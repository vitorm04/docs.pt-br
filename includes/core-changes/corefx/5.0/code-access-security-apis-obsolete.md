...
---
### <a name="most-code-access-security-apis-are-obsolete"></a>A maioria das APIs de segurança de acesso a código está obsoleta

A maioria dos tipos relacionados à CAS (segurança de acesso ao código) no .NET agora são obsoletos como aviso. Isso inclui atributos de CAS, como os <xref:System.Security.Permissions.SecurityPermissionAttribute> objetos de permissão CAS, como <xref:System.Net.SocketPermission> <xref:System.Security.Policy.EvidenceBase> tipos derivados e outras APIs de suporte.

#### <a name="change-description"></a>Descrição das alterações

No .NET Framework 2. x-4. x, os atributos e as APIs de CAS podem influenciar o curso de execução de código, incluindo garantir que a pilha de demanda por CAS tenha êxito ou falhe.
...
No .NET Core 2. x-3. x, o tempo de execução não respeita atributos de CAS ou APIs de CAS. O tempo de execução ignora atributos na entrada do método e a maioria das APIs programáticas não tem nenhum efeito.
...
Além disso, as chamadas programáticas para APIs mais extensas ( `Assert` ) sempre têm sucesso, enquanto chamadas programáticas para APIs restritivas ( `Deny` , `PermitOnly` ) sempre geram uma exceção em tempo de execução. ( <xref:System.Security.Permissions.PrincipalPermission> é uma exceção a essa regra. Consulte a seção [ação recomendada](#cas-action) abaixo.)
...
No .NET 5,0 e versões posteriores, a maioria das APIs relacionadas a CAS é obsoleta e produz um aviso de tempo de compilação `SYSLIB0003` .
...
Essa é uma alteração somente em tempo de compilação. Não há nenhuma alteração de tempo de execução de versões anteriores do .NET Core. Os métodos que não executam nenhuma operação no .NET Core 2. x-3. x continuarão a não executar nenhuma operação em tempo de execução no .NET 5,0 e posterior. Os métodos que lançam <xref:System.PlatformNotSupportedException> no .NET Core 2. x-3. x continuarão lançando um <xref:System.PlatformNotSupportedException> em tempo de execução no .NET 5,0 e posterior.

#### <a name="reason-for-change"></a>Motivo da alteração

A [CAS (segurança de acesso ao código)](../../../../docs/framework/misc/code-access-security.md) é uma tecnologia herdada sem suporte. A infraestrutura para habilitar o CAS existe somente no .NET Framework 2. x-4. x, mas foi preterida e não está recebendo correções de serviço ou de segurança.

Devido à reprovação do CAS, a [infra-estrutura de suporte não foi levada para o .NET Core](../../../../docs/core/porting/net-framework-tech-unavailable.md) ou o .NET 5.0 +. No entanto, as APIs foram encaminhadas para que os aplicativos pudessem compilar entre .NET Framework e o .NET Core. Isso levou a cenários de "falha aberta", onde existem algumas APIs relacionadas a CAS e que podem ser chamadas, mas não executam nenhuma ação em tempo de execução. Isso pode levar a problemas de segurança para os componentes que esperam que o tempo de execução obedeça atributos relacionados ao CAS ou chamadas de API programática. Para se comunicar melhor de que o tempo de execução não respeita esses atributos ou APIs, tornamos obsoleto a maioria deles no .NET 5,0.

#### <a name="version-introduced"></a>Versão introduzida

RC1 5,0

#### <a name=""></a><a id="cas-action">Ação recomendada</a>
...
- Se você estiver negando ou restringindo (via `PermitOnly` ) qualquer permissão, entre em contato com o seu supervisor de segurança. Como os atributos de CAS não são respeitados pelo tempo de execução do .NET 5.0 +, seu aplicativo poderá ter uma brecha de segurança se depender incorretamente da infraestrutura CAS para restringir o acesso a esses métodos.
...
- Se você estiver exigindo qualquer permissão (exceto <xref:System.Security.Permissions.PrincipalPermission> ), remova a demanda. Todas as demandas terão sucesso em tempo de execução.
...
- Se você estiver exigindo <xref:System.Security.Permissions.PrincipalPermission> , consulte as diretrizes para [PrincipalPermissionAttribute é obsoleto como erro](../../../../docs/core/compatibility/corefx.md#permission-action). Essa orientação se aplica para o <xref:System.Security.Permissions.PrincipalPermission> e o <xref:System.Security.Permissions.PrincipalPermissionAttribute> .

- Se for absolutamente necessário desabilitar esses avisos (o que não é recomendado), você poderá suprimir o `SYSLIB0003` aviso no código.
  ...
  > [!NOTE]
  > Suprimir `SYSLIB0003` desabilita apenas os avisos obsoletion relacionados ao CAS. Ele não desabilita nenhum outro aviso ou altera o comportamento do tempo de execução do .NET 5.0 +.
