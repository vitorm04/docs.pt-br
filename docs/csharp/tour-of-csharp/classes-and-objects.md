...
A memória ocupada por um objeto é recuperada automaticamente quando o objeto não está mais acessível. Não é necessário nem é possível desalocar explicitamente objetos no C#.
...
- `protected`
  - O acesso é limitado a essa classe ou classes derivadas desta classe.
- `internal`
  - O acesso é limitado ao assembly atual (. exe,. dll e assim por diante).
- `protected internal`
  - O acesso é limitado à classe que a contém, classes derivadas da classe que a contém ou classes dentro do mesmo assembly.
- `private`
  - O acesso é limitado a essa classe.
- `private protected`
  - O acesso é limitado à classe que a contém ou classes derivadas do tipo recipiente dentro do mesmo assembly.
...
Uma classe herda os membros de sua classe base. A herança significa que uma classe contém implicitamente todos os membros de sua classe base, exceto para a instância e os construtores estáticos, além dos finalizadores da classe base. Uma classe derivada pode adicionar novos membros a esses membros que ele herda, mas não pode remover a definição de um membro herdado. No exemplo anterior, `Point3D` herda os campos `x` e `y` de `Point` e cada instância `Point3D` contém três campos: `x`, `y` e `z`.
...
Os métodos podem ter uma lista de *parâmetros* que representam valores ou referências de variável passadas para o método, e um *tipo de retorno*, que especifica o tipo do valor calculado e retornado pelo método. O tipo de retorno de um método é `void` se ele não retornar um valor.
...
Uma *matriz de parâmetros* permite que um número variável de argumentos sejam passados para um método. Uma matriz de parâmetro é declarada com o modificador `params`. Somente o último parâmetro de um método pode ser uma matriz de parâmetros e o tipo de uma matriz de parâmetros deve ser um tipo de matriz unidimensional. Os métodos Write e WriteLine da classe <xref:System.Console?displayProperty=nameWithType> são bons exemplos de uso da matriz de parâmetros. Eles são declarados da seguinte maneira.

[!code-csharp[ConsoleExample](~/samples/snippets/csharp/tour/classes-and-objects/Program.cs#L78-L83)]

Dentro de um método que usa uma matriz de parâmetros, a matriz de parâmetros se comporta exatamente como um parâmetro regular de um tipo de matriz. No entanto, em uma invocação de um método com uma matriz de parâmetros, é possível passar um único argumento do tipo de matriz de parâmetro ou qualquer número de argumentos do tipo de elemento da matriz de parâmetros. No último caso, uma instância de matriz é automaticamente criada e inicializada com os argumentos determinados. Este exemplo
...
O C# requer que uma variável local seja *atribuída definitivamente* antes de seu valor poder ser obtido. Por exemplo, se a declaração da `i` anterior não incluísse um valor inicial, o compilador relataria um erro para os usos subsequentes de `i` porque `i` não seria definitivamente atribuído a esses pontos no programa.

Um método pode usar instruções `return` para retornar o controle é pelo chamador. Em um método que retorna `void`, `return` instruções não podem especificar uma expressão. Em um método que retorna não nulo, as instruções `return` devem incluir uma expressão que calcula o valor retornado.
...
Um método declarado sem um modificador estático é um *método de instância*. Um método de instância opera em uma instância específica e pode acessar membros estáticos e de instância. A instância em que um método de instância foi invocado pode ser explicitamente acessada como `this`. É um erro fazer referência a `this` em um método estático.

A seguinte classe `Entity` tem membros estáticos e de instância.

[!code-csharp[Entity](~/samples/snippets/csharp/tour/classes-and-objects/Entity.cs#L16-L36)]

Cada instância de `Entity` contém um número de série (e, presumivelmente, algumas outras informações que não são mostradas aqui). O construtor `Entity` (que é como um método de instância) inicializa a nova instância com o próximo número de série disponível. Como o construtor é um membro de instância, ele tem permissão para acessar o campo `serialNo` instância e o `nextSerialNo` campo estático.
...
Os métodos estáticos `SetNextSerialNo` e `GetNextSerialNo` são invocados na classe, enquanto o método de `GetSerialNo` instância é invocado em instâncias da classe.
...
O exemplo a seguir declara uma classe abstrata, `Expression`, que representa um nó de árvore de expressão e três classes derivadas, `Constant`, `VariableReference` e `Operation`, que implementam nós de árvore de expressão para operações aritméticas, referências de variável e constantes. (Este exemplo é semelhante a, mas não deve ser confundido com os tipos de árvore de expressão).
...
O exemplo a seguir mostra uma classe genérica chamada `MyList<T>`, que implementa uma lista de objetos que cresce. A classe contém vários exemplos dos tipos mais comuns de membros da função.
...
Ao contrário de outros membros, os construtores de instância não são herdados e uma classe não tem nenhum construtor de instância diferente daqueles construtores realmente declarados na classe. Se nenhum construtor de instância for fornecido para uma classe, então um construtor vazio sem parâmetros será fornecido automaticamente.
...
Para cenários avançados em que o controle do armazenamento subjacente de um evento é desejado, uma declaração de evento pode fornecer explicitamente `add` e `remove` acessadores, que são semelhantes ao acessador `set` de uma propriedade.
