...
*Correspondência de padrões* é um recurso que permite que você implemente a expedição do método em propriedades diferentes do tipo de um objeto. Você provavelmente já está familiarizado com a expedição de método com base no tipo de um objeto. Na programação orientada a objeto, os métodos virtuais e de substituição fornecem a sintaxe da linguagem para implementar a expedição de método com base no tipo de um objeto. As classes base e derivada fornecem implementações diferentes. As expressões de correspondência de padrões estendem esse conceito, de modo que você possa implementar com facilidade padrões de expedição semelhantes para elementos de dados e tipos que não são relacionados por meio de uma hierarquia de herança. 
