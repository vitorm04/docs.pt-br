...
Em versões anteriores do C#, era preciso fixar uma variável para acessar um dos números inteiros que fazem parte de `myFixedField`. Agora, o código abaixo é compilado em um contexto seguro:
...
Para saber mais, confira o artigo sobre [retornos `ref` e locais `ref`](../programming-guide/classes-and-structs/ref-returns.md) e o artigo sobre [`foreach`](../language-reference/keywords/foreach-in.md).
...
Para saber mais, confira o artigo [Instrução `stackalloc`](../language-reference/keywords/stackalloc.md) na referência da linguagem.
...
Para saber mais, confira o artigo [Instrução `fixed`](../language-reference/keywords/fixed-statement.md) na referência da linguagem.
...
Você também pode usar a nova restrição `unmanaged` para especificar que um parâmetro de tipo deve ser um **tipo não gerenciado**. Um **tipo não gerenciado** é um tipo que não é um tipo de referência e não contém campos de tipo de referência em nenhum nível de aninhamento.
...
Para saber mais, confira o artigo sobre o [modificador de parâmetro`in`](../language-reference/keywords/in-parameter-modifier.md).
