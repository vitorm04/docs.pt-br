...
- Começando com C# 8.0, <xref:System.Collections.Generic.IAsyncEnumerable%601>para um método de sincronia que retorna um fluxo *assíncrono*.

Para obter mais informações sobre os métodos assíncronos, consulte [Programação assíncrona com async e await (C#)](./index.md).  
  
## <a name="tasktresult-return-type"></a> Tipo de retorno Task\<TResult\>  
O <xref:System.Threading.Tasks.Task%601> tipo de retorno é usado para um método assíncrono que `TResult`contém uma declaração de [retorno](../../../language-reference/keywords/return.md) (C#) na qual o operand é .  
...
Você pode entender `await` melhor como recupera `Task<T>` o resultado de `GetLeisureHours` um separando a chamada para a aplicação de `await`, como mostra o código a seguir. Uma chamada ao método `GetLeisureHours` que não é aguardada imediatamente, retorna um `Task<int>`, como você esperaria da declaração do método. A tarefa é atribuída à variável `integerTask` no exemplo. Já que `integerTask` é um <xref:System.Threading.Tasks.Task%601>, ele contém uma propriedade <xref:System.Threading.Tasks.Task%601.Result> do tipo `TResult`. Nesse caso, `TResult` representa um tipo inteiro. Quando `await` é aplicado à `integerTask`, a expressão await é avaliada como o conteúdo da propriedade <xref:System.Threading.Tasks.Task%601.Result%2A> de `integerTask`. O valor é atribuído à variável `ret`.  
...
No exemplo a seguir, o método assíncrono `WaitAndApologize` não contém uma instrução `return`, então o método retorna um objeto <xref:System.Threading.Tasks.Task>. Devolver um `Task` `WaitAndApologize` permite ser aguardado. O <xref:System.Threading.Tasks.Task> tipo não inclui `Result` um imóvel porque não tem valor de retorno.  
...
## <a name="void-return-type"></a>Tipo de retorno do vazio

O tipo de retorno `void` é usado em manipuladores de eventos assíncronos, que exigem um tipo de retorno `void`. Para métodos diferentes de manipuladores de eventos que não retornam um valor, você deve retornar um <xref:System.Threading.Tasks.Task>, porque não é possível esperar por um método assíncrono que retorna `void`. Qualquer chamador de tal método deve continuar a ser concluído sem esperar que o chamado método assíncrono seja concluído. O chamador deve ser independente de quaisquer valores ou exceções que o método assíncrono gere.  
  
O chamador de um método de sincronismo de retorno de vazio não pode capturar exceções lançadas do método, e essas exceções não tratadas provavelmente causarão a falha do seu aplicativo. Se um método <xref:System.Threading.Tasks.Task> que <xref:System.Threading.Tasks.Task%601> retorna a ou lança uma exceção, a exceção é armazenada na tarefa retornada. A exceção é descartada quando a tarefa é aguardada. Portanto, verifique se qualquer método assíncrono que pode produzir uma exceção tem um tipo de retorno de <xref:System.Threading.Tasks.Task> ou <xref:System.Threading.Tasks.Task%601> e que as chamadas ao método são aguardadas.  
  
Para obter mais informações sobre como capturar exceções em métodos assíncronos, consulte a seção [Exceções na](../../../language-reference/keywords/try-catch.md#exceptions-in-async-methods) seção Métodos de Async do artigo [try-catch.](../../../language-reference/keywords/try-catch.md)  
  
O exemplo a seguir mostra o comportamento de um manipulador de eventos assíncrono. No código de exemplo, um manipulador de eventos assíncrono deve informar o segmento principal quando ele terminar. Em seguida, o thread principal pode aguardar um manipulador de eventos assíncronos ser concluído antes de sair do programa.
...
A partir de C# 8.0, um método assíncrono pode retornar um *fluxo assíncrono*, representado por <xref:System.Collections.Generic.IAsyncEnumerable%601>. Um fluxo assíncrono fornece uma maneira de enumerar itens lidos de um fluxo quando os elementos são gerados em pedaços com chamadas assíncronas repetidas. O exemplo a seguir mostra um método de assincronia que gera um fluxo assíncrono:

:::code language="csharp" source="./snippets/AsyncStreams.cs" id="SnippetGenerateAsyncStream":::

O exemplo anterior lê linhas de uma string assíncronamente. Uma vez que cada linha é lida, o código enumera cada palavra na seqüência. Os chamadores enumerariam `await foreach` cada palavra usando a declaração. O método aguarda quando precisa ler assíncronamente a próxima linha da seqüência de origem.
