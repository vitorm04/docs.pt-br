...
O arquivo XML a seguir é usado em vários exemplos de [!INCLUDE[sqltecxlinq](../../../../csharp/programming-guide/concepts/linq/includes/sqltecxlinq_md.md)] documentação. Esse arquivo contém clientes e pedidos.  
 ...
 O tópico [Arquivo XSD de exemplo: clientes e pedidos](../Topic/Sample%20XSD%20File:%20Customers%20and%20Orders1.md) contém um XSD que pode ser usado para validar esse documento. Ele usa o `xs:key` e `xs:keyref` recursos de XSD para estabelecer que o `CustomerID` atributo do `Customer` elemento é uma chave e estabelecer uma relação entre o `CustomerID` elemento em cada `Order` elemento e o `CustomerID` atributo em cada `Customer` elemento.  
  
 Para obter um exemplo de escrever consultas LINQ que tiram proveito dessa relação usando a `Join` cláusula, consulte [How to: Join Two Collections \(LINQ to XML\) \(C\#\)](../Topic/How%20to:%20Join%20Two%20Collections%20\(LINQ%20to%20XML\)%20\(C%23\).md).  
