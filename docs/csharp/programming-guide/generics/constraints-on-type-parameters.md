...
Algumas das restrições são mutuamente exclusivas. Todos os tipos de valor devem ter um construtor sem parâmetros acessível. A restrição `struct` implica que a restrição `new()` e a restrição `new()` não podem ser combinadas com a restrição `struct`. A restrição `unmanaged` implica a restrição `struct`. A restrição `unmanaged` não pode ser combinada com as restrições `struct` ou `new()`.
