...
As funções locais têm diferentes regras para atribuição definida em relação às expressões lambda. Uma declaração de função local pode ser referenciada em qualquer local do código em que ela esteja no escopo. Uma expressão lambda deve ser atribuída a uma variável delegada antes de ser acessada (ou chamada através do delegado fazendo referência à expressão lambda.) Observe que a versão que usa a expressão lambda `nthFactorial` deve declarar e inicializar a expressão lambda, antes de defini-la. Não fazer isso resulta em um erro em tempo de compilação para referenciar `nthFactorial` antes de atribuí-lo.
