---
description: Erro do Compilador CS1588
title: Erro do Compilador CS1588
ms.date: 07/20/2015
f1_keywords:
- CS1588
helpviewer_keywords:
- CS1588
ms.assetid: ffcfd6fa-fb0b-4fad-a7f3-79be00fa2c96
ms.openlocfilehash: dc208b28207f8b441ac03f3554c042a925b60274
ms.sourcegitcommit: d579fb5e4b46745fd0f1f8874c94c6469ce58604
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/30/2020
ms.locfileid: "89140029"
---
# <a name="compiler-error-cs1588"></a>Erro do Compilador CS1588
Não é possível determinar o diretório Common Language Runtime--' Reason '  
  
 O compilador não pode recuperar o diretório no qual o Common Language Runtime de .NET Framework está instalado. Esse erro indica que o Common Language Runtime está instalado incorretamente.
