...
Iteradores não podem ter em ref ou parâmetros de saída  
  
 Esse erro ocorrerá se um método iterador usa uma `in`, `ref`, ou `out` parâmetro. Para evitar esse erro, remova os `in`, `ref`, ou `out` palavra-chave de assinatura do método.  
