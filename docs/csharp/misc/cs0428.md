...
Não é possível converter o grupo de métodos 'Identifier' para 'type' do tipo não delegado.  Você pretendia invocar o método?  
  
 Esse erro ocorre ao converter um grupo de métodos em um tipo não delegado ou a tentativa de invocar um método sem o uso de parênteses.  
