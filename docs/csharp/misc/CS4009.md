...
Se você usar C# 7,0 ou inferior, remova a palavra-chave `async` da assinatura do ponto de entrada do aplicativo. Remova também quaisquer `await` palavras-chave que você usou para aguardar métodos assíncronos no ponto de entrada do aplicativo. 
...
Para aguardar um método que retorna um <xref:System.Threading.Tasks.Task>, chame seu método <xref:System.Threading.Tasks.Task.Wait%2A>, como ilustra o exemplo a seguir:

[!code-csharp[CS4009](~/samples/snippets/csharp/misc/cs4009-2.cs)]

Para aguardar um método que retorna um <xref:System.Threading.Tasks.Task%601>, recupere o valor de sua propriedade <xref:System.Threading.Tasks.Task%601.Result>, como o exemplo a seguir:
...
- [Operador `await`](../language-reference/operators/await.md)
