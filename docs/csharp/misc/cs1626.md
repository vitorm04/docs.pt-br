...
Não é possível usar a instrução yield no corpo de um bloco try com uma cláusula catch  
 ...
 Uma instrução yield não é permitida em um bloco try, se houver uma cláusula catch associada com o bloco try. Para evitar esse erro, mova a instrução yield fora do bloco try/catch/finally ou remova o bloco catch.
