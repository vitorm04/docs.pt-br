...
A classe base ou a interface ' class ' no assembly ' assembly ' referenciada pelo tipo ' type ' não pôde ser resolvida  
 ...
 Para obter mais informações, consulte [caixa de diálogo Adicionar referência](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager) e [/Reference (opções doC# compilador)](../language-reference/compiler-options/reference-compiler-option.md).  
...
## <a name="example"></a>Exemplo  
 O terceiro arquivo substitui a DLL criada pela primeira etapa e omite a definição da classe `B`interna.  
...
## <a name="example"></a>Exemplo  
 Por fim, o quarto arquivo faz referência `C` à classe definida no segundo exemplo, que é derivada da `B`classe e que agora está ausente.  
