...
Uma instrução `if` identifica qual instrução executar com base no valor de uma expressão booliana. No exemplo a seguir, a variável `bool` `result` deve ser definida como `true` e, em seguida, verificada na instrução `if`. A saída é `The variable is set to true.`.
