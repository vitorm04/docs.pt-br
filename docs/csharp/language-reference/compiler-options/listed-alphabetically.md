...
|-checksumalgorithm:\<alg>|Especifica o algoritmo para calcular a soma de verificação do arquivo de origem armazenada no PDB.  Os valores compatíveis são: SHA256 (padrão) ou SHA1.<br>Em razão de problemas de colisão com SHA1, a Microsoft recomenda SHA256. |
...
|[-platform](platform-compiler-option.md)|Limita as plataformas em que este código pode ser executado: x86, Itanium, x64, anycpu ou anycpu32bitpreferred. O padrão é anycpu.|
...
|[-reference](reference-compiler-option.md)|Referencia metadados dos arquivos do assembly especificado.|
