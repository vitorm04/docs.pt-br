...
```
 ...
 `i = lng;` faz uma atribuição, mas os tipos de dados das variáveis dos lados esquerdo e direito do operador de atribuição não são correspondentes. Antes de fazer a atribuição, o compilador está implicitamente convertendo a variável `lng`, que é do tipo longo, para um int. Isso está implícito porque nenhum código instruiu explicitamente o compilador a realizar essa conversão. O problema com esse código é que isso é considerado uma conversão de estreitamento e o compilador não permite conversões de estreitamento implícitas porque pode haver uma perda de dados.
