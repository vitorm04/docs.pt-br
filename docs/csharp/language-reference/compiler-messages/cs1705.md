...
- Um cliente que faz referência somente à versão 1.0 das DLLs idênticas, mas acessa uma classe da versão 2.0.  
 ...
 O código a seguir cria a primeira das DLLs idênticas. Para obter informações sobre como gerar um arquivo de chave, consulte [-keyfileC# (opções do compilador)](../compiler-options/keyfile-compiler-option.md).  
