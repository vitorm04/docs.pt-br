...
O tipo TypeName1 existe no TypeName2 e no TypeName3  
 ...
 Para resolver esse erro, use o recurso alias da opção de compilador [-referência (C# Compiler Options)](../compiler-options/reference-compiler-option.md) ou não faça referência a um de seus conjuntos.  
