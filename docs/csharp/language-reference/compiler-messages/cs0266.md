...
Não é possível converter implicitamente o tipo 'type1' para 'type2'. Existe uma conversão explícita (há uma conversão ausente?)  
 ...
 Esse erro ocorre quando o código tenta converter entre dois tipos que não podem ser convertidos implicitamente, mas nos quais há uma conversão explícita. Para obter mais informações, consulte [Conversões e conversões de Tipo](../../programming-guide/types/casting-and-type-conversions.md).  
