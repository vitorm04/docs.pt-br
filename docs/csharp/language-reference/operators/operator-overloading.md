...
Um tipo definido pelo usuário pode sobrecarregar um operador C# predefinido. Ou seja, um tipo pode fornecer a implementação personalizada de uma operação no caso de um ou ambos os operands serem desse tipo. A seção [Operadores sobrecarregáveis](#overloadable-operators) mostra quais operadores do C# podem ser sobrecarregados.
...
- Um operador não-unary tem um parâmetro de entrada. Um operador binário tem dois parâmetros de entrada. Em cada caso, pelo menos um parâmetro deve ter o tipo `T` ou `T?`, em que `T` é o tipo que contém a declaração do operador.
...
Você pode estender o exemplo anterior `int` [definindo uma conversão implícita](user-defined-conversion-operators.md) de . `Fraction` Em seguida, os operadores sobrecarregados seriam compatíveis com os argumentos desses dois tipos. Ou seja, tornaria-se possível adicionar um inteiro a uma fração e obter uma fração como um resultado.
...
|[x && y](boolean-logical-operators.md#conditional-logical-and-operator-), [x &#124;&#124; y](boolean-logical-operators.md#conditional-logical-or-operator-)|Operadores lógicos condicionais não podem ser sobrecarregados. No entanto, se um tipo com os `&` [ `true` operadores sobrecarregados e `false` os operadores](true-false-operators.md) também sobrecarregar o operador ou <code>&#124;</code> operador de uma determinada forma, o `&&` ou <code>&#124;&#124;</code> operador, respectivamente, pode ser avaliado para os operands desse tipo. Para obter mais informações, veja a seção [Operadores lógicos condicionais definidos pelo usuário](~/_csharplang/spec/expressions.md#user-defined-conditional-logical-operators) na [especificação da linguagem C#](~/_csharplang/spec/introduction.md).|
...
|[(T)x](type-testing-and-cast.md#cast-expression)|O operador de elenco não pode ser sobrecarregado, mas você pode definir conversões de tipo personalizadas que podem ser executadas por uma expressão de elenco. Para saber mais, confira [Operadores de conversão definidos pelo usuário](user-defined-conversion-operators.md).|
...
- [Diretrizes de design - Sobrecargas de operadores](../../../standard/design-guidelines/operator-overloads.md)
