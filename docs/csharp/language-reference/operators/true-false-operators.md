...
O operador `true` retorna o valor [bool](../keywords/bool.md) `true` para indicar que seu operando é definitivamente verdadeiro. O operador `false` retorna o valor de `bool` `true` para indicar que seu operando é definitivamente falso. Não há garantia de que os operadores `true` e `false` se complementarão. Ou seja, ambos os operadores `true` e `false` podem retornar o valor `bool` `false` para o mesmo operando. Se um tipo define um dos dois operadores, ele também deve definir outro operador.
