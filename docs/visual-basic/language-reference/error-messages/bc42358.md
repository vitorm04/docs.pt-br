...
---
# <a name="bc42358-because-this-call-is-not-awaited-execution-of-the-current-method-continues-before-the-call-is-completed"></a>BC42358: como essa chamada não é esperada, a execução do método atual continua antes que a chamada seja concluída
...
Um problema igualmente importante é o que acontece com as exceções que são geradas no método assíncrono chamado. Uma exceção lançada em um método que retorna uma <xref:System.Threading.Tasks.Task> ou <xref:System.Threading.Tasks.Task%601> é armazenada na tarefa retornada. Se você não aguardar a tarefa ou verificar explicitamente se há exceções, a exceção será perdida. Se você aguardar a tarefa, a exceção será relançada.
