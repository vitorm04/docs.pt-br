...
> [!NOTE]
> Como é o padrão, você não precisa especificar explicitamente a `ByVal` palavra-chave em assinaturas de método. Ele tende a produzir código barulhento e, muitas vezes, leva à palavra `ByRef` -chave não-padrão que está sendo ignorada.
