...
|Termo|Definição|  
|---|---|  
|`testexpression`|Necessário. Expressão. É necessário avaliar um dos tipos de dados elementares (`Boolean`, `Byte`, `Char`, `Date`, `Double`, `Decimal`, `Integer`, `Long`, `Object`, `SByte`, `Short`, `Single`, `String`, `UInteger`, `ULong`e `UShort`).|  
...
|`End Select`|Encerra a definição da construção de `Select`...`Case`.|  
