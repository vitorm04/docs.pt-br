...
|Termo|Definição|  
|---|---|  
|`condition`|Necessária. Expressão `Boolean`. Se `condition` for `Nothing`, Visual Basic o tratará como `False`.|  
...
|`End While`|Necessária. Finaliza a definição do bloco `While`.|  
