---
title: "Instru&#231;&#227;o Enum (Visual Basic) | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
helpviewer_keywords: 
  ...
  - "instrução Enum"
  - "Palavra-chave Private, instruções Enum"
  - "Palavra-chave Public, na instrução Enum"
  - "variáveis [Visual Basic], enumeração"
  - "constantes enumeradas"
...
-   `enumerationname`  
     ...
     Obrigatório. Nome da enumeração. Para obter informações sobre nomes válidos, consulte [nomes de elemento declarado](../../../visual-basic/programming-guide/language-features/declared-elements/declared-element-names.md).  
...
-   `memberlist`  
     ...
     Obrigatório. Lista de constantes de membro sendo declarada nessa instrução. Vários membros aparecem em linhas de código fonte individual.  
    ...
    |-|-|  
    |Parte|Descrição|  
    |`membername`|Obrigatório. Nome desse membro.|  
...
## <a name="initializing-members"></a>Inicializando membros  
 ...
 Se você não especificar `initializer` para um membro, Visual Basic o inicializa com zero (se ele for o primeiro `member` em `memberlist`), ou para um valor maior do que o precede imediatamente `member`.  
...
## <a name="assigning-multiple-values"></a>Atribuindo valores múltiplos  
 Enumerações geralmente representam valores mutuamente exclusivos. Incluindo o <xref:System.FlagsAttribute> atributo o `Enum` declaração, você pode em vez disso, atribuir vários valores para uma instância da enumeração. O <xref:System.FlagsAttribute> atributo especifica que a enumeração ser tratado como um campo de bits, ou seja, um conjunto de sinalizadores. Eles são chamados de *bit a bit* enumerações.  
  
 Quando você declara uma enumeração usando o <xref:System.FlagsAttribute> atributo, é recomendável que você use potência de 2, que é, 1, 2, 4, 8, 16 e assim por diante, para os valores. Também recomendamos que "Nenhum" seja o nome de um membro cujo valor é 0. Para obter diretrizes adicionais, consulte <xref:System.FlagsAttribute> e <xref:System.Enum>.  
...
## <a name="example"></a>Exemplo  
 No exemplo a seguir, um `As` cláusula é usada para especificar o `datatype` de uma enumeração.  
