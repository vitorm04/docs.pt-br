...
## <a name="writing-to-the-file"></a>Gravando no arquivo  
 Para adicionar a capacidade de gravar em um arquivo por meio do aplicativo, use a classe <xref:System.IO.StreamWriter>. <xref:System.IO.StreamWriter> foi criado para a saída de caracteres em uma determinada codificação, enquanto a classe <xref:System.IO.Stream> foi criada para entrada e saída em bytes. Use <xref:System.IO.StreamWriter> para gravar linhas de informações em um arquivo de texto padrão. Para obter mais informações sobre a classe <xref:System.IO.StreamWriter>, consulte <xref:System.IO.StreamWriter>.  
...
4.  Por fim, anexe o código para limpar o <xref:System.Windows.Forms.TextBox>. Adicione o seguinte código ao evento <xref:System.Windows.Forms.Control.Click> do botão `Clear`.  
...
## <a name="adding-display-features-to-the-diary"></a>Adicionando recursos de exibição ao diário  
 Nesta seção, você adiciona um recurso que exibe a última entrada em `DisplayEntry`<xref:System.Windows.Forms.TextBox>. Você também pode adicionar um <xref:System.Windows.Forms.ComboBox> que exibe várias entradas e em que um usuário pode selecionar uma entrada para exibição em `DisplayEntry`<xref:System.Windows.Forms.TextBox>. Uma instância da classe <xref:System.IO.StreamReader> lê de `MyDiary.txt`. Assim como a classe <xref:System.IO.StreamWriter>, <xref:System.IO.StreamReader> é destinado a uso com arquivos de texto.  
...
1.  A `PickEntries`<xref:System.Windows.Forms.ComboBox> é usada para exibir as datas em que um usuário envia cada entrada, para que o usuário possa selecionar uma entrada de uma data específica. Crie um manipulador de eventos <xref:System.Windows.Forms.Control.Click> para o botão `GetEntries` e adicione o código a seguir.  
...
2.  Para testar seu código, pressione F5 para compilar o aplicativo e, então, clique em **Obter entradas**. Clique na seta suspensa na <xref:System.Windows.Forms.ComboBox> para exibir as datas das entradas.  
...
2.  Para testar seu código, pressione F5 para compilar o aplicativo e, então, envie uma entrada. Clique em **Obter entradas**, selecione uma entrada da <xref:System.Windows.Forms.ComboBox> e, em seguida, clique em **Exibir**. O conteúdo da entrada selecionada aparecerá na `DisplayEntry`<xref:System.Windows.Forms.TextBox>.  
...
1.  Adicione o seguinte código ao evento <xref:System.Windows.Forms.Control.Click> do botão `Display`, após `DisplayEntry.Text = ReadString`.  
...
3.  Quando um usuário exibe uma entrada, o botão `EditEntry` fica habilitado. Adicione o seguinte código ao evento <xref:System.Windows.Forms.Control.Click> do botão `Display`, após `DisplayEntry.Text = ReadString`.  
...
5.  Crie um manipulador de eventos <xref:System.Windows.Forms.Control.Click> para o botão `SubmitEdit` e adicione o código a seguir  
 ...
 Para testar seu código, pressione F5 para compilar o aplicativo. Clique em **Obter Entradas**, selecione uma entrada e clique em **Exibir**. A entrada aparece no em `DisplayEntry`<xref:System.Windows.Forms.TextBox>. Clique em **Editar entrada**. A entrada aparece no em `Entry`<xref:System.Windows.Forms.TextBox>. Edite a entrada em `Entry`<xref:System.Windows.Forms.TextBox> e clique em **Enviar edição**. Abra o arquivo `MyDiary.txt` para confirmar a correção. Agora, selecione uma entrada e clique em **Excluir entrada**. Quando o <xref:System.Windows.Forms.MessageBox> solicitar confirmação, clique em **OK**. Feche o aplicativo e abra `MyDiary.txt` para confirmar a exclusão.  
