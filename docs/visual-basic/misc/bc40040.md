...
Um tipo genérico está marcado como `<CLSCompliant(True)>`, mas uma restrição em um dos seus parâmetros de tipo Especifica um tipo que está marcado como `<CLSCompliant(False)>`, não é marcada ou não se qualifica porque ele é um tipo incompatível.  
 ...
 Para um tipo para ser compatível com o [independência da linguagem e componentes independentes da linguagem](../../../docs/standard/language-independence-and-language-independent-components.md) (CLS), ele deve usar somente tipos compatíveis com CLS. Isso também se aplicam às restrições em parâmetros de tipo de um tipo genérico.  
