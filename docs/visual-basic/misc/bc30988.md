...
Quando você declara uma instância de uma classe, a palavra-chave `New` deve ser seguida por um nome de tipo ou por `With`. Por exemplo, as instruções a seguir declaram `client` como uma instância da classe `Customer`. O nome do tipo `Customer` segue `New`.  
