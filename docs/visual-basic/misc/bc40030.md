...
Uma classe ou interface está marcado como `<CLSCompliant(True)>` quando ele está aninhado em um tipo que está marcado como `<CLSCompliant(False)>` ou não está marcado.  
 ...
 Para uma classe ou interface seja compatível com o [independência da linguagem e componentes independentes da linguagem](https://msdn.microsoft.com/library/12a7a7h3) (CLS), sua hierarquia de confinamento inteira deve ser compatível. Isso significa que cada tipo no qual ele está aninhado deve ser compatível.  
