...
1.  Determinar se existe uma conversão de qualquer tipo de `<type1>` para `<type2>`. Se ambos estiverem [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] tipos elementares, ou se ambos forem instâncias de classes, você normalmente pode fazer essa determinação consultando a tabela em [Widening e conversões de estreitamento](../../visual-basic/programming-guide/language-features/data-types/widening-and-narrowing-conversions.md).  
