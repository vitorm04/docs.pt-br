...
Se você não quiser compilar os aplicativos por conta própria, poderá baixar "exemplo assíncrono: Acessando a orientaçãoC# da Web (e Visual Basic) "de [exemplos de código do desenvolvedor](https://code.msdn.microsoft.com/Async-Sample-Accessing-the-9c10497f).
...
2. Adicione as instruções `Imports` a seguir na parte superior do arquivo de código se elas ainda não estiverem presentes.
...
3. O código para a solução síncrona contém os quatro métodos a seguir:
    ...
    Copie os quatro métodos a seguir e cole- `startButton_Click` os no manipulador de eventos em MainWindow. XAML. vb:
...
2. `GetResponseAsync` retorna um <xref:System.Threading.Tasks.Task%601>. Nesse caso, a *variável de retorno de tarefa*, `TResult`, tem o tipo <xref:System.Net.WebResponse>. A tarefa é uma promessa de produzir um objeto `WebResponse` verdadeiro após os dados solicitados terem sido baixados e a tarefa ter sido executada até a conclusão.
    ...
    Para recuperar o `WebResponse` valor da tarefa, aplique um operador [Await](../../../../visual-basic/language-reference/operators/await-operator.md) à chamada para `GetResponseAsync`, como mostra o código a seguir.
    ...
    A chamada para `webReq.GetResponseAsync` retorna um `Task(Of WebResponse)` ou `Task<WebResponse>`. Em seguida `Await` , um operador é aplicado à tarefa para recuperar `WebResponse` o valor.

    Se o método Async tiver trabalho para fazer isso não depende da conclusão da tarefa, o método poderá continuar com esse trabalho entre essas duas instruções, após a chamada para o método Async e antes que o operador Await seja aplicado. Para ver mais exemplos, veja [Como: Faça várias solicitações da Web em paralelo usando Async e Await (Visual Basic)](../../../../visual-basic/programming-guide/concepts/async/how-to-make-multiple-web-requests-in-parallel-by-using-async-and-await.md) e [como: Estenda a instrução assíncrona usando Task. WhenAll (Visual Basic)](../../../../visual-basic/programming-guide/concepts/async/how-to-extend-the-async-walkthrough-by-using-task-whenall.md).
...
4. Tudo o que resta fazer em `GetURLContents` é ajustar a assinatura do método. Você pode usar o `Await` operador somente em métodos que são marcados com o modificador [assíncrono](../../../../visual-basic/language-reference/modifiers/async.md) . Adicione o modificador para marcar o método como um *método assíncrono*, como mostra o código a seguir.

    ```vb
    Private Async Function GetURLContents(url As String) As Byte()
    ```

5. O tipo de retorno de um método assíncrono só pode <xref:System.Threading.Tasks.Task>ser <xref:System.Threading.Tasks.Task%601>,. Em Visual Basic, o método deve ser um `Function` que retorna um `Task` ou um `Task(Of T)`, ou o método deve ser um `Sub`. Normalmente, um `Sub` método é usado apenas em um manipulador de eventos assíncrono, `Sub` onde é necessário. Em outros casos, você usará `Task(T)` se o método Completed tiver uma instrução [Return](../../../../visual-basic/language-reference/statements/return-statement.md) que retorna um valor do tipo T, e você `Task` usará se o método Completed não retornar um valor significativo.
...
## <a name="convert-startbuttonclick-to-an-asynchronous-method"></a>Converter startButton_Click em um método assíncrono
...
4. Por fim, adicione o modificador `Async` à declaração de modo que o manipulador de eventos pode esperar `SumPagSizesAsync`.
    ...
    Normalmente, os nomes dos manipuladores de eventos não são alterados. O tipo de retorno não é `Task` alterado para porque os manipuladores `Sub` de eventos devem ser procedimentos em Visual Basic.
...
1. O .NET Framework fornece muitos métodos assíncronos que você pode usar. Um deles, o <xref:System.Net.Http.HttpClient.GetByteArrayAsync%28System.String%29?displayProperty=nameWithType> método, faz exatamente o que você precisa para este passo a passos. Você pode usá-lo em vez do método `GetURLContentsAsync` que criou em um procedimento anterior.
    ...
    A primeira etapa é criar um <xref:System.Net.Http.HttpClient> objeto `SumPageSizesAsync` no método. Adicione a declaração a seguir ao início do método.
