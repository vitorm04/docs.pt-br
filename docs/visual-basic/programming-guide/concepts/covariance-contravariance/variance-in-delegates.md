...
No exemplo de código a seguir `SampleGenericDelegate(Of String)` , não pode ser explicitamente `SampleGenericDelegate(Of Object)`convertido em `String` , `Object`embora herde. Você pode corrigir esse problema marcando o parâmetro genérico `T` com a palavra-chave `out`.
...
A variação para parâmetros de tipo genérico tem suporte apenas para tipos de referência. Por exemplo, `DVariant(Of Int)`não pode ser convertido implicitamente `DVariant(Of Object)` em `DVariant(Of Long)`ou, porque Integer é um tipo de valor.
