...
## <a name="unary-logical-operator"></a>Operador lógico unário  
 O [operador Not](../../../../visual-basic/language-reference/operators/not-operator.md) executa lógica *negação* em um `Boolean` expressão. Retorna a lógica oposta seu operando. Se a expressão for avaliada como `True`, em seguida, `Not` retorna `False`; se a expressão for avaliada como `False`, em seguida, `Not` retorna `True`. O exemplo a seguir ilustra essa situação.  
...
## <a name="binary-logical-operators"></a>Operadores lógicos binários  
 O [operador e](../../../../visual-basic/language-reference/operators/and-operator.md) executa lógica *conjunto* em dois `Boolean` expressões. Se as duas expressões são `True`, em seguida, `And` retorna `True`. Se pelo menos uma das expressões for avaliada como `False`, em seguida, `And` retorna `False`.  
...
## <a name="short-circuiting-logical-operations"></a>Operações de lógicas do curto-circuito  
 O [operador AndAlso](../../../../visual-basic/language-reference/operators/andalso-operator.md) é muito semelhante do `And` operador, que também executa conjunção lógica em duas `Boolean` expressões. A principal diferença entre as duas é que `AndAlso` exibe *curto-circuito* comportamento. Se a primeira expressão em uma `AndAlso` expressão é avaliada como `False`, em seguida, a segunda expressão é avaliada não porque ele não é possível alterar o resultado final, e `AndAlso` retorna `False`.  
  
 Da mesma forma, o [operador OrElse](../../../../visual-basic/language-reference/operators/orelse-operator.md) executa uma disjunção lógica em duas do curto-circuito `Boolean` expressões. Se a primeira expressão em uma `OrElse` expressão é avaliada como `True`, em seguida, a segunda expressão é avaliada não porque ele não é possível alterar o resultado final, e `OrElse` retorna `True`.  
  
### <a name="short-circuiting-trade-offs"></a>Vantagens e desvantagens do curto-circuito  
 Curto-circuito pode melhorar o desempenho, não avaliando uma expressão que não é possível alterar o resultado da operação lógica. No entanto, se essa expressão executar ações adicionais, curto-circuito ignora essas ações. Por exemplo, se a expressão inclui uma chamada para um `Function` procedimento, o procedimento não é chamado se a expressão é curto-circuitada e qualquer código adicional contidas a `Function` não é executado. Portanto, a função possam ser executadas apenas ocasionalmente e não pode ser testada corretamente. Ou a lógica do programa pode depender do código de `Function`.  
 ...
 No exemplo anterior, observe que alguns códigos importante dentro `checkIfValid()` não é executado quando a chamada é curto-circuitada. A primeira `If` chamadas de instrução `checkIfValid()` , embora `12 > 45` retorna `False`, pois `And` não curto-circuito. A segunda `If` não chama a instrução `checkIfValid()`, porque quando `12 > 45` retorna `False`, `AndAlso` reduz a segunda expressão. O terceiro `If` chamadas de instrução `checkIfValid()` , embora `12 < 45` retorna `True`, pois `Or` não curto-circuito. O quarto `If` não chama a instrução `checkIfValid()`, porque quando `12 < 45` retorna `True`, `OrElse` reduz a segunda expressão.  
  
## <a name="bitwise-operations"></a>Operações bit a bit  
 Operações bit a bit avaliam dois valores integrais em formato binário (base 2). Eles comparam os bits em posições correspondentes e, em seguida, atribuem valores com base na comparação. O exemplo a seguir ilustra o `And` operador.  
...
-   O `And` operador compara as representações binárias, uma posição binária (bits) por vez. Se ambos os bits na posição especificada forem 1, 1 é colocado na posição no resultado. Se qualquer bit for 0, 0 é colocado na posição no resultado. No exemplo anterior isso funciona da seguinte maneira:  
...
-   O resultado é tratado como decimal. O valor 001 é a representação binária de 1, portanto `x` = 1.  
 ...
 Bit a bit `Or` operação é semelhante, exceto que um 1 será atribuído para o bit de resultado se um ou ambos os bits comparados é 1. `Xor`atribui um 1 para o bit de resultado se exatamente um dos bits comparados (não ambos) é 1. `Not`receba um operando único e inverte a todos os bits, incluindo o bit de sinal e atribui o valor para o resultado. Isso significa que, para assinado números positivos, `Not` sempre retorna um valor negativo e para números negativos, `Not` sempre retorna um positivo ou o valor zero.  
  
 O `AndAlso` e `OrElse` operadores não oferecem suporte a operações bit a bit.  
  
> [!NOTE]
>  Operações bit a bit podem ser executadas em tipos integrais somente. Valores de ponto flutuante devem ser convertidos para tipos integrais antes de prosseguir com a operação bit a bit.  
