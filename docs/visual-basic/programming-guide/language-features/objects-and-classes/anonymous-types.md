...
O Visual Basic dá suporte a tipos anônimos, que permitem criar objetos sem gravar uma definição de classe para o tipo de dados. Em vez disso, o compilador gera uma classe para você. A classe não tem nome utilizável, herda diretamente de <xref:System.Object>e contém as propriedades que você especifica ao declarar o objeto. Como o nome do tipo de dados não é especificado, ele é conhecido como um *tipo anônimo*.  
 ...
 Nos exemplos a seguir, `products` é uma lista de objetos Product, cada um dos quais tem muitas propriedades. A variável `namePriceQuery` mantém a definição de uma consulta que, quando executada, retorna uma coleção de instâncias de um tipo anônimo que tem duas propriedades, `Name` e `Price`.  
  
 [!code-vb[VbVbalrAnonymousTypes#2](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrAnonymousTypes/VB/Class1.vb#2)]  
  
 A variável `nameQuantityQuery` mantém a definição de uma consulta que, quando executada, retorna uma coleção de instâncias de um tipo anônimo que tem duas propriedades, `Name` e `OnHand`.  
...
## <a name="declaring-an-anonymous-type"></a>Declarando um tipo anônimo  
 ...
 Para obter mais informações sobre outras maneiras de especificar propriedades de tipo anônimo, consulte [como: inferir nomes e tipos de propriedade em declarações de tipo anônimo](../../../../visual-basic/programming-guide/language-features/objects-and-classes/how-to-infer-property-names-and-types-in-anonymous-type-declarations.md).  
...
### <a name="read-only-values"></a>Valores somente leitura  
 Os valores das propriedades de chave não podem ser alterados. Por exemplo, em `prod8` no exemplo anterior, os campos `Name` e `Price` são `read-only`, mas `OnHand` podem ser alterados.  
...
## <a name="an-anonymous-type-definition"></a>Uma definição de tipo anônimo  
 ...
 Definições de tipo anônimo que têm pelo menos uma propriedade de chave também implementam a interface <xref:System.IEquatable%601?displayProperty=nameWithType>, em que `T` é o tipo do tipo anônimo.  
