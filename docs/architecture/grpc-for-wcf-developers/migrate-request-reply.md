...
Se você seguiu as instruções na seção anterior, deve ter um projeto gRPC com um arquivo `portfolios.proto` semelhante a este:
...
A classe `PortfolioItem` será convertida primeiro em uma mensagem Protobuf, pois a classe `Portfolio` depende dela. A classe é simples e três das propriedades são mapeadas diretamente para os tipos de dados gRPC. A propriedade `Cost`, que representa o preço pago pelos compartilhamentos na compra, é um campo de `decimal`. o gRPC dá suporte apenas a `float` ou `double` para números reais, que não são adequados para a moeda. Como os preços de compartilhamento variam em um mínimo de uma centésimo, o custo pode ser expresso como um `int32` de centavos.

> [!NOTE]
> Lembre-se de usar camelCase para nomes de campo no arquivo de `.proto`. O C# gerador de código irá convertê-los em PascalCase para você, e os usuários de outras linguagens lhe agradecerão por respeitar seus diferentes padrões de codificação.
...
O método `GetAll` do WCF usa apenas um único parâmetro, `traderId`, portanto, pode parecer que você pode especificar `string` como o tipo de parâmetro. Mas gRPC requer um tipo de mensagem definido. Esse requisito ajuda a impor a prática de usar mensagens personalizadas para todas as entradas e saídas, para compatibilidade com versões anteriores futuras.

O método WCF também retorna um `List<Portfolio>`, mas, pelo mesmo motivo, ele não permite tipos de parâmetro simples, gRPC não permitirá `repeated Portfolio` como um tipo de retorno. Em vez disso, crie um tipo de `GetAllResponse` para encapsular a lista.

> [!WARNING]
> Você pode estar tentado a criar uma mensagem `PortfolioList` ou algo semelhante e usá-la em vários métodos de serviço, mas deve resistir a essa tentação. É impossível saber como os vários métodos em um serviço irão evoluir, portanto, mantenha suas mensagens específicas e separadas de forma limpa.
...
Depois de criar a biblioteca e adicioná-la à solução, exclua o arquivo `Class1.cs` gerado e copie os arquivos da biblioteca da solução WCF para a pasta da nova biblioteca de classes, mantendo a estrutura de pastas:
...
Os projetos do estilo SDK do .NET incluem automaticamente quaisquer `.cs` arquivos em ou sob seu próprio diretório, para que você não precise adicioná-los explicitamente ao projeto. A única etapa restante é remover os atributos `DataContract` e `DataMember` das classes `Portfolio` e `PortfolioItem` para que eles sejam classes simples e C# antigas:
...
Comece implementando o método `Get`. A substituição padrão é semelhante a este exemplo:
...
O primeiro problema é que `request.TraderId` é uma cadeia de caracteres e o serviço requer uma `Guid`. Embora o formato esperado para a cadeia de caracteres seja `UUID`, o código precisa lidar com a possibilidade de um chamador ter enviado um valor inválido e responder adequadamente. O serviço pode responder com erros lançando um `RpcException` e usar o código de status de `InvalidArgument` padrão para expressar o problema:
...
Depois que houver um valor de `Guid` apropriado para `traderId`, você poderá usar o repositório para recuperar o portfólio e retorná-lo para o cliente:
...
Agora que você tem o código de conversão em vigor, você pode concluir a implementação do método de `Get`:
...
A implementação do método `GetAll` é semelhante. Observe que os campos de `repeated` em mensagens Protobuf são gerados como propriedades `readonly` do tipo `RepeatedField<T>`, portanto, você precisa adicionar itens a eles usando o método `AddRange`, como neste exemplo:
...
Crie um .NET Standard biblioteca de classes na mesma solução para conter o cliente. Isso é basicamente um exemplo de criação de código de cliente, mas você pode empacotar essa biblioteca usando o NuGet e distribuí-la em um repositório interno para que outras equipes do .NET consumam. Vá em frente e adicione uma nova biblioteca de classes .NET Standard chamada `TraderSys.Portfolios.Client` à solução e exclua o arquivo `Class1.cs`.
...
Navegue até o arquivo de `portfolios.proto` no projeto `TraderSys.Portfolios`, deixe **cliente** em **selecionar o tipo de classe a ser gerado**e, em seguida, selecione **OK**:
...
> [!TIP]
> Se você não estiver usando o Visual Studio ou preferir trabalhar na linha de comando, poderá usar o `dotnet-grpc` ferramenta global para gerenciar referências Protobuf em um projeto .NET gRPC. Para obter mais informações, consulte a [documentação do`dotnet-grpc`](/aspnet/core/grpc/dotnet-grpc).
