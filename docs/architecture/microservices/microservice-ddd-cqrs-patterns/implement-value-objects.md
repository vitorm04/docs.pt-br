...
Com o EF Core 2.0 e posterior, existem novas e melhores maneiras de persistir objetos de valor.

## <a name="persist-value-objects-as-owned-entity-types-in-ef-core-20-and-later"></a>Persista objetos de valor como tipos de entidades próprias no EF Core 2.0 e posterior

Mesmo com algumas lacunas entre o padrão de objeto de valor canônico no DDD e o tipo de entidade própria no EF Core, é atualmente a melhor maneira de persistir objetos de valor com o EF Core 2.0 e posterior. Você pode ver as limitações no final desta seção.
...
É possível acorrentar o `OwnsOne` método em um mapeamento fluente. No exemplo hipotético a seguir, `OrderDetails` tem `BillingAddress` e `ShippingAddress`, que são tipos `Address`. Então `OrderDetails` pertence ao tipo `Order`.
...
- Pode ser configurado `[Owned]`com atributo, usando o EF Core 2.1 e posterior.

- Pode lidar com coleções de tipos próprios (usando a versão 2.2 e posterior).

#### <a name="owned-entities-limitations"></a>Limitações de entidades próprias

- Você não pode `DbSet<T>` criar um tipo de propriedade (por design).
...
- A divisão da tabela é opcional, ou seja, elas podem, opcionalmente, ser mapeadas para uma tabela separada e ainda serem de propriedade de tipos.

- Eles podem referenciar outras entidades (ou seja, podem atuar como o lado dependente das relações com outros tipos não pertencentes).
...
- **Vaughn Vernon. Implementando o design orientado por domínio.** (Livro; inclui uma discussão sobre objetos de valor) \
  <https://www.amazon.com/Implementing-Domain-Driven-Design-Vaughn-Vernon/dp/0321834577/>

- **Tipos de entidades próprias** \
