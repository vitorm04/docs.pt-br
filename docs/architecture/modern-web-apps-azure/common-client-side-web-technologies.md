---
title: Tecnologias web comuns do lado do cliente
description: Arquitetura Aplicações Web Modernas com ASP.NET Core e Azure | Tecnologias web comuns do lado do cliente
author: ardalis
ms.author: wiwagn
ms.date: 12/04/2019
...
---
# <a name="common-client-side-web-technologies"></a>Tecnologias web comuns do lado do cliente
...
Embora o HTML e o CSS sejam relativamente estáveis, o JavaScript, por meio das estruturas do aplicativo e dos utilitários com os quais os desenvolvedores trabalham para criar aplicativos baseados na Web, está evoluindo em uma velocidade vertiginosa. Este capítulo analisa algumas maneiras pelas quais o JavaScript é usado por desenvolvedores web e fornece uma visão geral de alto nível das bibliotecas angular e react do lado do cliente.

> [!NOTE]
> O Blazor fornece uma alternativa às estruturas JavaScript para construir interfaces de usuário cliente ricas e interativas. O suporte blazor do lado do cliente ainda está em pré-visualização, então por enquanto está fora de escopo para este capítulo.
...
A maioria dos recursos que o jQuery não tem intrinsecamente pode ser adicionada com a adição de outras bibliotecas. No entanto, uma estrutura de SPA como o Angular fornece esses recursos de forma mais integrada, pois foi projetado com todos eles em mente, desde o início. Além disso, jQuery é uma biblioteca imperativa, o que significa que você precisa chamar funções jQuery para fazer qualquer coisa com jQuery. Grande parte do trabalho e da funcionalidade fornecida pelas estruturas de SPA pode ser feita de forma declarativa, sem a necessidade de codificação real.

A associação de dados é um ótimo exemplo disso. No jQuery, geralmente é preciso apenas uma linha de código para obter o valor de um elemento DOM ou para definir o valor de um elemento. No entanto, você tem que escrever este código sempre que precisar alterar o valor do elemento, e às vezes isso ocorrerá em múltiplas funções em uma página. Outro exemplo comum é a visibilidade do elemento. Em jQuery, pode haver muitos lugares diferentes onde você escreveria código para controlar se certos elementos eram visíveis. Em cada um desses casos, ao usar a associação de dados, nenhuma codificação precisa ser feita. Você simplesmente vincularia o valor ou a visibilidade dos elementos em questão a um *modelo de exibição* na página, e as alterações nesse modelo de visualização seriam automaticamente refletidas nos elementos vinculados.

### <a name="angular-spas"></a>SPAs do Angular

Angular continua sendo uma das estruturas JavaScript mais populares do mundo. Desde angular 2, a equipe reconstruiu a estrutura do zero (usando [TypeScript](https://www.typescriptlang.org/)) e rebranded a partir do nome AngularJS original para simplesmente Angular. Agora com vários anos de idade, o Angular redesenhado continua a ser uma estrutura robusta para a construção de aplicativos de página única.
...
Os componentes são definidos com a função de decorador @Component, que usa metadados sobre o componente. A propriedade seletor identifica o ID do elemento na página onde este componente será exibido. A propriedade de modelo é um modelo HTML simples que inclui um espaço reservado que corresponde à propriedade de nome do componente, definida na última linha.
...
Você pode desenvolver aplicativos Angular usando um CLI. Para começar o desenvolvimento local com o Angular (supondo que você já tenha instalado o GIT e o npm) basta clonar um repositório do GitHub e executar `npm install` e `npm start`. Além disso, a Angular envia sua própria CLI, que pode criar projetos, adicionar arquivos e ajudar em tarefas de teste, agrupamento e implantação. Essa simpatia cli torna angular especialmente compatível com ASP.NET Core, que também possui ótimo suporte à CLI.
...
A partir de seu guia de início, "O Vue é uma estrutura progressiva para a construção de interfaces de usuário. Ao contrário de outras estruturas monolíticas, o Vue foi projetado do zero para ser incrementalmente adotável. A biblioteca central é focada apenas na camada de exibição, e é fácil de pegar e se integrar com outras bibliotecas ou projetos existentes. Por outro lado, o Vue é perfeitamente capaz de alimentar aplicativos sofisticados de página única quando usado em combinação com ferramentas modernas e bibliotecas de suporte."

Começar com o Vue simplesmente requer incluir seu script dentro de um arquivo HTML:
...
Com a estrutura adicionada, você pode tornar declarativamente os dados para o DOM usando a sintaxe de templating direta do Vue:
...
Isso é o suficiente para renderizar "Hello Vue!" na página. Observe, no entanto, que o Vue não está simplesmente prestando a mensagem para a div uma vez. Ele suporta vinculação de dados e atualizações `message` dinâmicas de `<div>` tal forma que, se o valor das alterações, o valor no é imediatamente atualizado para refleti-lo.

Claro, isso só arranha a superfície do que o Vue é capaz. Ganhou muita popularidade nos últimos anos e tem uma grande comunidade. Há uma [lista enorme e crescente de componentes de suporte e bibliotecas](https://github.com/vuejs/awesome-vue#redux) que trabalham com o Vue para ampliá-lo também. Se você está procurando adicionar comportamento do lado do cliente ao seu aplicativo web ou considerando a construção de um SPA completo, vale a pena investigar o Vue.
...
- Quão ativa é sua comunidade? Novos projetos estão sendo construídos com ele?
...
> - **Desenvolvimento do lado do cliente no núcleo ASP.NET**  
...
> - **Vue**  
> <https://vuejs.org/>
> - **Angular vs React vs Vue: qual quadro escolher em 2020**
> <https://www.codeinwp.com/blog/angular-vs-vue-vs-react/>
> - **Os principais javascript frameworks para desenvolvimento front-end em 2020**  
