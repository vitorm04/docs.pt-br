...
- Se você tiver `Microsoft.NETCore.App` ou `NETStandard.Library` itens de `<PackageReference>` em seu projeto, remova-os. Essas referências do pacote estão [implícitas no SDK](https://aka.ms/sdkimplicitrefs).
...
- Remova os [globs](https://en.wikipedia.org/wiki/Glob_(programming)) que estão [implícitos no SDK](../project-sdk/overview.md#default-compilation-includes). Deixar esses globs em seu projeto causará um erro no build, uma vez que os itens de compilação serão duplicados.
