...
| dnu restore                       | dotnet restore    | Restaure as dependências especificadas no seu project.json. ([veja observação](#dotnet-restore-note))                                                               |
...
A partir do v1, as ferramentas da CLI do .NET Core não têm um comando `install` para instalar dependências. Para instalar um pacote NuGet, você precisará adicioná-lo como uma dependência ao arquivo `project.json` e, em seguida, executar `dotnet restore` ([veja observação](#dotnet-restore-note)). 
...
Se você estiver usando outros destinos `dnx` como o `dnx451`, será necessário alterá-los também. `dnx451` deve ser alterado para `net451`. Consulte o tópico [.NET Standard](../../standard/net-standard.md) para saber mais. 
