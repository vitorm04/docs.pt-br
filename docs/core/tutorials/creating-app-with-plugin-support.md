...
Este tutorial mostra como criar <xref:System.Runtime.Loader.AssemblyLoadContext> um personalizado para carregar plugins. Um <xref:System.Runtime.Loader.AssemblyDependencyResolver> é usado para resolver as dependências do plugin. O tutorial isola corretamente as dependências do plugin do aplicativo de hospedagem. Você aprenderá a:
...
2. Para facilitar a construção do projeto, crie um arquivo de solução visual studio na mesma pasta. Execute o comando a seguir:

    ```dotnetcli
    dotnet new sln
    ```

3. Execute o seguinte comando para adicionar o projeto do aplicativo à solução:
...
Agora, o aplicativo pode carregar corretamente e instanciar comandos a partir de conjuntos de plugins carregados, mas ainda não é capaz de carregar os conjuntos de plugins. Crie um arquivo chamado *PluginLoadContext.cs* na pasta *AppWithPlugin* com o seguinte conteúdo:
...
## <a name="simple-plugin-with-no-dependencies"></a>Plugin simples sem dependências

Novamente na pasta raiz, faça o seguinte:

1. Execute o seguinte comando para criar `HelloPlugin`um novo projeto de biblioteca de classes chamado :

    ```dotnetcli
    dotnet new classlib -o HelloPlugin
    ```

2. Execute o seguinte comando para `AppWithPlugin` adicionar o projeto à solução:
...
O `<Private>false</Private>` elemento é importante. Ele informa ao MSBuild que ele não deve copiar o *PluginBase.dll* para o diretório de saída do HelloPlugin. Se o assembly *PluginBase.dll* estiver presente no diretório de saída, o `PluginLoadContext` encontrará o assembly lá e o carregará ao carregar o assembly *HelloPlugin.dll*. Neste ponto, o tipo `HelloPlugin.HelloCommand` implementará a interface `ICommand` do *PluginBase.dll* no diretório de saída do projeto `HelloPlugin`, não a interface `ICommand` que é carregada no contexto de carregamento padrão. Uma vez que o tempo de execução vê esses `AppWithPlugin.Program.CreateCommands` dois tipos como tipos diferentes de conjuntos diferentes, o método não encontrará os comandos. Como resultado, os metadados `<Private>false</Private>` serão necessários para a referência ao assembly que contém as interfaces de plug-in.

Da mesma `<ExcludeAssets>runtime</ExcludeAssets>` forma, o elemento `PluginBase` também é importante se as referências a outros pacotes. Esta configuração tem `<Private>false</Private>` o mesmo efeito que `PluginBase` funciona em referências de pacote que o projeto ou uma de suas dependências pode incluir.

Agora que `HelloPlugin` o projeto está concluído, você deve atualizar o `AppWithPlugin` projeto para saber onde o `HelloPlugin` plugin pode ser encontrado. Após o comentário `// Paths to plugins to load`, adicione `@"HelloPlugin\bin\Debug\netcoreapp3.0\HelloPlugin.dll"` como um elemento da matriz `pluginPaths`.

## <a name="plugin-with-library-dependencies"></a>Plugin com dependências de biblioteca

Quase todos os plug-ins são mais complexos do que um simples "Olá, Mundo", e muitos plug-ins têm dependências de outras bibliotecas. Os projetos de plug-in `JsonPlugin` e `OldJson` no exemplo mostram dois exemplos de plug-ins com dependências de pacotes NuGet em `Newtonsoft.Json`. Os próprios arquivos do projeto não têm nenhuma informação especial para as referências `pluginPaths` do projeto e (depois de adicionar os caminhos de plugin à matriz) os plugins funcionam perfeitamente, mesmo que sejam executados na mesma execução do aplicativo AppWithPlugin. No entanto, esses projetos não copiam as montagens referenciadas para o diretório de saída, então as montagens precisam estar presentes na máquina do usuário para que os plugins funcionem. Há duas maneiras de contornar esse problema. A primeira opção é usar o comando `dotnet publish` para publicar a biblioteca de classes. Como alternativa, se você quer usar a saída do `dotnet build` para seu plug-in, adicione a propriedade `<CopyLocalLockFileAssemblies>true</CopyLocalLockFileAssemblies>` entre as marcas `<PropertyGroup>` no arquivo de projeto do plug-in. Veja o projeto de plug-in `XcopyablePlugin` como exemplo.

## <a name="other-examples-in-the-sample"></a>Outros exemplos na amostra
...
## <a name="reference-a-plugin-interface-from-a-nuget-package"></a>Consulte uma interface de plugin de um pacote NuGet
