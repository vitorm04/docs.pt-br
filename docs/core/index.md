...
O .NET Core é uma plataforma de desenvolvimento de uso geral mantida pela Microsoft e pela comunidade .NET no [GitHub](https://github.com/dotnet/core). Ele é multiplataforma e dá suporte ao Windows, macOS e Linux, e pode ser usado em dispositivos, na nuvem e em cenários inseridos/IoT.

As seguintes características são as que melhor definem o .NET Core:

- **Implantação flexível:** o .NET Core pode ser incluído no seu aplicativo ou instalado lado a lado no usuário ou em todos os computadores.
- **Multiplataforma:** o .NET Core pode ser executado em Windows, macOS e Linux e pode ser transferido para outros sistemas operacionais. Os [sistemas operacionais com suporte](https://github.com/dotnet/core/blob/master/roadmap.md), as CPUs e os cenários de aplicação serão ampliados ao longo do tempo, fornecidos pela Microsoft, outras empresas e outros indivíduos.
...
- **Suporte da Microsoft:** o .NET Core é mantido pela Microsoft, pelo [Suporte do .NET Core](https://www.microsoft.com/net/core/support/)

## <a name="composition"></a>Composição

O .NET Core é composto pelas seguintes partes:

- Um [tempo de execução .NET](https://github.com/dotnet/coreclr) que oferece sistema de tipos, carregamento de assembly, coletor de lixo, interoperabilidade nativa e outros serviços básicos.
...
- Um [conjunto de ferramentas de SDK](https://github.com/dotnet/cli) e [compiladores de linguagem](https://github.com/dotnet/roslyn) que possibilitam a experiência de desenvolvedor básica, disponíveis no [SDK do .NET Core](sdk.md).
- O host de aplicativo “dotnet” usado para iniciar aplicativos .NET Core. Ele seleciona e hospeda o tempo de execução, fornece políticas de carregamento de assembly e inicia o aplicativo. Esse host também é usado para abrir as ferramentas de SDK da mesma maneira.

### <a name="languages"></a>Linguagens

As linguagens C# e F# (o Visual Basic será lançado em breve) podem ser usadas para criar aplicativos e bibliotecas para .NET Core. Os compiladores são executados no .NET Core, permitindo que você desenvolva soluções para .NET Core em qualquer lugar que ele seja executado. Em geral, você não usará os compiladores diretamente, mas sim indiretamente, usando as ferramentas de SDK.
...
O .NET Core pode ser pensado como uma versão multiplataforma do .NET Framework, na camada da BCL (Biblioteca de Classes Base) do .NET Framework. Ele implementa a especificação [.NET Standard](../standard/net-standard.md). O .NET Core fornece um subconjunto das APIs que estão disponíveis no .NET Framework ou no Mono/Xamarin. Em alguns casos, os tipos não são implementados completamente (alguns membros não estão disponíveis ou foram movidos).
...
O [.NET Standard](../standard/net-standard.md) é uma especificação de API que descreve o conjunto coerente de APIs .NET que os desenvolvedores podem esperar em cada implementação do .NET. Implementações do .NET precisam implementar essa especificação para serem consideradas em conformidade com o .NET Standard e para dar suporte a bibliotecas voltadas para o .NET Standard. 
...
- [Xamarin.Forms quando direcionando para UWP](https://www.xamarin.com/forms)

### <a name="open-source"></a>Software Livre

O [.NET Core](https://github.com/dotnet/core) é um software livre (licença MIT) e foi concedido à [.NET Foundation](https://dotnetfoundation.org) pela Microsoft em 2014. Agora ele é um dos projetos mais ativos do .NET Foundation. Ele pode ser adotado livremente por pessoas e empresas, incluindo para fins pessoais, comerciais ou acadêmicos. Várias empresas usam o .NET Core como parte de aplicativos, ferramentas, novas plataformas e serviços de hospedagem. Algumas dessas empresas fazem contribuições significativas para o .NET Core no GitHub e fornecem diretrizes sobre a direção do produto como parte do [.NET Foundation Technical Steering Group](https://dotnetfoundation.org/blog/tsg-welcome).

## <a name="acquisition"></a>Aquisição

O .NET Core é distribuído de duas maneiras principais, como pacotes no NuGet.org e como distribuições _standalone_.

### <a name="distributions"></a>Distribuições

Você pode baixar o .NET Core na página [Introdução ao .NET Core](https://www.microsoft.com/net/core).

- A distribuição *Microsoft .NET Core* inclui o _runtime_ do CoreCLR, bibliotecas associadas, um host de aplicativo de console e o inicializador de aplicativos `dotnet`. Ele é descrito pelo metapacote [`Microsoft.NETCore.App`](https://www.nuget.org/packages/Microsoft.NETCore.App).
...
Normalmente, você primeiro instala o SDK do .NET Core para dar início ao desenvolvimento com .NET Core. Você pode optar por instalar builds adicionais (talvez pré-lançamento) do .NET Core.

### <a name="packages"></a>Pacotes

- [Pacotes do .NET Core](packages.md) contêm o _runtime_ e as bibliotecas do .NET Core (implementações e assemblies de referência). Por exemplo, [System.Net.Http](https://www.nuget.org/packages/System.Net.Http/).
...
O .NET Core é uma implementação multiplataforma do .NET. As principais questões de arquitetura exclusivas do .NET Core são referentes a implementações específicas de plataforma para as plataformas com suporte.

### <a name="environments"></a>Ambientes

A Microsoft disponibiliza o .NET Core no Windows, no macOS e no Linux. No Linux, a Microsoft dá suporte principalmente ao .NET Core executado em famílias de distribuição do RHEL (Red Hat Enterprise Linux) ou Debian.

Atualmente o .NET Core dá suporte a CPUs X64. No Windows, também há suporte para X86. O suporte para ARM64 e ARM32 está em andamento.

O [Roteiro do .NET Core](https://github.com/dotnet/core/blob/master/roadmap.md) fornece informações mais detalhadas sobre a carga de trabalho e suporte e planos para ambientes de sistemas operacionais e de CPU.
...
O .NET Core foi criado como um produto muito semelhante, porém único, em relação a outros produtos .NET. Ele foi projetado para possibilitar ampla adaptabilidade a novas plataformas, para novas cargas de trabalho e novas _toolchains_ de compiladores. Ele possui vários _ports_ para sistemas operacionais e CPU em andamento e pode ser portado para muitos outros. Um exemplo é o projeto [LLILC](https://github.com/dotnet/llilc), que é um protótipo inicial de compilação nativa para .NET Core por meio do compilador [LLVM](http://llvm.org/).

O produto é dividido em várias partes, permitindo que essas sejam adaptadas para novas plataformas em diferentes cronogramas. O _runtime_ e as bibliotecas fundamentais específicas de plataformas devem ser portados como uma unidade. Bibliotecas independentes de plataforma devem funcionar no estado em que se encontram em todas as plataformas como-está em todas as plataformas, por padrão. Há uma tendência de projeto em reduzir as implementações específicas de plataforma para aumentar a eficiência dos desenvolvedores, preferindo código C# neutro com relação à plataforma sempre que um algoritmo ou API puder ser implementado dessa maneira de forma total ou parcial.

As pessoas geralmente perguntam como o .NET Core é implementado para dar suporte a vários sistemas operacionais. Elas normalmente perguntam se há implementações separadas ou se a [compilação condicional](https://en.wikipedia.org/wiki/Conditional_compilation) é usada. A resposta é ambas, com uma forte tendência para a compilação condicional.
...
O [Mono](http://www.mono-project.com/) é a implementação .NET multiplataforma e de [software livre](https://github.com/mono/mono) original, fornecido pela primeira vez em 2004. Ele pode ser considerado um clone de comunidade do .NET Framework. A equipe de projeto do Mono se baseava nos [padrões .NET](https://github.com/dotnet/coreclr/blob/master/Documentation/project-docs/dotnet-standards.md) (especificamente o ECMA 335) publicados pela Microsoft a fim de fornecer uma implementação compatível.
