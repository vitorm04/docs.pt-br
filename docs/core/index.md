...
> Confira os [Tutoriais de "Introdução"](getting-started.md) para aprender a criar um aplicativo .NET Core simples. Bastam apenas alguns minutos para colocar seu primeiro aplicativo em funcionamento.
...
### <a name="relationship-to-the-net-standard-library"></a>Relações com a .NET Standard Library

A [.NET Standard Library](../standard/library.md) é uma especificação da API que descreve o conjunto consistente de APIs do .NET que os desenvolvedores podem esperar em cada implementação do .NET. Implementações do .NET precisam implantar essa especificação para serem considerados compatíveis com a .NET Standard Library e para dar suporte a bibliotecas voltadas para a .NET Standard Library. 

O .NET Core implementa a .NET Standard Library e, portanto, dá suporte a bibliotecas padrão do .NET.
...
Você pode ver no gráfico abaixo que a grande maioria dos [CoreFX](https://github.com/dotnet/corefx) neutros em relação à plataforma são compartilhados entre todas as plataformas. O código neutro com relação à plataforma pode ser implementado como um único assembly portátil que pode ser usado em todas as plataformas.
