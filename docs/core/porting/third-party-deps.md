...
Para fazer a portabilidade de seu código para o .NET Core ou .NET Standard, é preciso compreender suas dependências. As dependências externas são os pacotes NuGet ou os arquivos `.dll` que você faz referência em seu projeto, mas que você não se compila.
...
Depois de migrar seu projeto para o formato de `PackageReference`, verifique se seus pacotes são compatíveis com o .NET Core.
...
>[Bibliotecas de portas](libraries.md)
