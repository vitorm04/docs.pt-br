...
- <span data-ttu-id="bb36b-124">Você tem um esquema para sua fonte de informações?</span><span class="sxs-lookup"><span data-stu-id="bb36b-124">Do you have a schema for your information source?</span></span> <span data-ttu-id="bb36b-125">Nesse caso, o que é o mapeamento para o F # e sistema de tipos do .NET?</span><span class="sxs-lookup"><span data-stu-id="bb36b-125">If so, what’s the mapping into the F# and .NET type system?</span></span>
...
<span data-ttu-id="bb36b-146">Para depurar este provedor usando as instruções print, fazer com que um script que expõe um problema com o provedor e, em seguida, use o seguinte código:</span><span class="sxs-lookup"><span data-stu-id="bb36b-146">To debug this provider by using print statements, make a script that exposes a problem with the provider, and then use the following code:</span></span>
...
- <span data-ttu-id="bb36b-175">Isso proporcionou tipo é apagado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-175">This provided type is erased.</span></span>  <span data-ttu-id="bb36b-176">Porque você indica que o tipo base é `obj`, instâncias serão exibidos como valores do tipo [obj](https://msdn.microsoft.com/library/dcf2430f-702b-40e5-a0a1-97518bf137f7) no código compilado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-176">Because you indicate that the base type is `obj`, instances will appear as values of type [obj](https://msdn.microsoft.com/library/dcf2430f-702b-40e5-a0a1-97518bf137f7) in compiled code.</span></span>
...
- <span data-ttu-id="bb36b-214">Quando você estiver escrevendo um provedor para um espaço de informações que contém somente dados e métodos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-214">When you are writing a provider for an information space that contains only data and methods.</span></span>
...
### <a name="choosing-representations-for-erased-provided-types"></a><span data-ttu-id="bb36b-228">Escolher representações para apagados tipos fornecidos</span><span class="sxs-lookup"><span data-stu-id="bb36b-228">Choosing Representations for Erased Provided Types</span></span>
...
<span data-ttu-id="bb36b-251">Nesse caso, você pode (opcionalmente) usar esse tipo como a eliminação de tipo ao especificar esse tipo como o `baseType` ao construir o `ProvidedTypeDefinition`:</span><span class="sxs-lookup"><span data-stu-id="bb36b-251">In this case, you may (optionally) use this type as the type erasure by specifying this type as the `baseType` when constructing the `ProvidedTypeDefinition`:</span></span>
...
- <span data-ttu-id="bb36b-260">Verificando se uma expressão regular é válida.</span><span class="sxs-lookup"><span data-stu-id="bb36b-260">Verifying whether a regular expression is valid.</span></span>
...
|<span data-ttu-id="bb36b-318">Distância (medidor)</span><span class="sxs-lookup"><span data-stu-id="bb36b-318">Distance (meter)</span></span>|<span data-ttu-id="bb36b-319">Tempo (segundos)</span><span class="sxs-lookup"><span data-stu-id="bb36b-319">Time (second)</span></span>|
|----------------|-------------|
...
<span data-ttu-id="bb36b-343">Observe os seguintes pontos sobre a implementação:</span><span class="sxs-lookup"><span data-stu-id="bb36b-343">Note the following points about the implementation:</span></span>
...
<span data-ttu-id="bb36b-359">Observe as convenções a seguir ao criar provedores de tipos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-359">Observe the following conventions when authoring type providers.</span></span>
...
<span data-ttu-id="bb36b-369">Para obter mais informações, consulte o `GetConnection` projetar convenção que é descrita posteriormente neste tópico.</span><span class="sxs-lookup"><span data-stu-id="bb36b-369">For more information, see the `GetConnection` design convention that's described later in this topic.</span></span>

### <a name="design-patterns-for-type-providers"></a><span data-ttu-id="bb36b-370">Padrões de design para provedores de tipos</span><span class="sxs-lookup"><span data-stu-id="bb36b-370">Design Patterns for Type Providers</span></span>

<span data-ttu-id="bb36b-371">As seções a seguir descrevem padrões de design, que você pode usar ao criar provedores de tipos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-371">The following sections describe design patterns you can use when authoring type providers.</span></span>

#### <a name="the-getconnection-design-pattern"></a><span data-ttu-id="bb36b-372">O padrão de Design GetConnection</span><span class="sxs-lookup"><span data-stu-id="bb36b-372">The GetConnection Design Pattern</span></span>

<span data-ttu-id="bb36b-373">A maioria dos provedores de tipo deve ser escrito para usar o `GetConnection` padrão que é usado pelos provedores de tipo em FSharp.Data.TypeProviders.dll, como mostra o exemplo a seguir:</span><span class="sxs-lookup"><span data-stu-id="bb36b-373">Most type providers should be written to use the `GetConnection` pattern that's used by the type providers in FSharp.Data.TypeProviders.dll, as the following example shows:</span></span>
...
#### <a name="type-providers-backed-by-remote-data-and-services"></a><span data-ttu-id="bb36b-374">Provedores de tipos com suporte pelos serviços e dados remotos</span><span class="sxs-lookup"><span data-stu-id="bb36b-374">Type Providers Backed By Remote Data and Services</span></span>

<span data-ttu-id="bb36b-375">Antes de criar um provedor de tipo que é apoiado por serviços e dados remotos, você deve considerar uma variedade de problemas inerentes à programação conectada.</span><span class="sxs-lookup"><span data-stu-id="bb36b-375">Before you create a type provider that's backed by remote data and services, you must consider a range of issues that are inherent in connected programming.</span></span> <span data-ttu-id="bb36b-376">Esses problemas incluem as seguintes considerações:</span><span class="sxs-lookup"><span data-stu-id="bb36b-376">These issues include the following considerations:</span></span>

- <span data-ttu-id="bb36b-377">mapeamento de esquema</span><span class="sxs-lookup"><span data-stu-id="bb36b-377">schema mapping</span></span>

- <span data-ttu-id="bb36b-378">alocação e a invalidação na presença de alteração de esquema</span><span class="sxs-lookup"><span data-stu-id="bb36b-378">liveness and invalidation in the presence of schema change</span></span>

- <span data-ttu-id="bb36b-379">cache de esquemas</span><span class="sxs-lookup"><span data-stu-id="bb36b-379">schema caching</span></span>

- <span data-ttu-id="bb36b-380">implementações assíncronas de operações de acesso de dados</span><span class="sxs-lookup"><span data-stu-id="bb36b-380">asynchronous implementations of data access operations</span></span>

- <span data-ttu-id="bb36b-381">suporte a consultas, incluindo consultas LINQ</span><span class="sxs-lookup"><span data-stu-id="bb36b-381">supporting queries, including LINQ queries</span></span>

- <span data-ttu-id="bb36b-382">autenticação e credenciais</span><span class="sxs-lookup"><span data-stu-id="bb36b-382">credentials and authentication</span></span>

<span data-ttu-id="bb36b-383">Este tópico não explorar ainda mais esses problemas.</span><span class="sxs-lookup"><span data-stu-id="bb36b-383">This topic doesn't explore these issues further.</span></span>

### <a name="additional-authoring-techniques"></a><span data-ttu-id="bb36b-384">Técnicas de criação adicionais</span><span class="sxs-lookup"><span data-stu-id="bb36b-384">Additional Authoring Techniques</span></span>

<span data-ttu-id="bb36b-385">Quando você escrever seus próprios provedores de tipos, você talvez queira usar as seguintes técnicas adicionais.</span><span class="sxs-lookup"><span data-stu-id="bb36b-385">When you write your own type providers, you might want to use the following additional techniques.</span></span>

### <a name="creating-types-and-members-on-demand"></a><span data-ttu-id="bb36b-386">Criação de tipos e membros sob demanda</span><span class="sxs-lookup"><span data-stu-id="bb36b-386">Creating Types and Members On-Demand</span></span>

<span data-ttu-id="bb36b-387">A API ProvidedType adiou a versões do AddMember.</span><span class="sxs-lookup"><span data-stu-id="bb36b-387">The ProvidedType API has delayed versions of AddMember.</span></span>
...
<span data-ttu-id="bb36b-388">Essas versões são usadas para criar espaços de demanda de tipos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-388">These versions are used to create on-demand spaces of types.</span></span>

### <a name="providing-array-types-and-generic-type-instantiations"></a><span data-ttu-id="bb36b-389">Fornecendo instanciações de tipo genérico e tipos de matriz</span><span class="sxs-lookup"><span data-stu-id="bb36b-389">Providing Array types and Generic Type Instantiations</span></span>

<span data-ttu-id="bb36b-390">Tornar membros fornecidos (cujas assinaturas incluem tipos de matriz e tipos byref instanciações de tipos genéricos) usando o vetor perpendicular `MakeArrayType`, `MakePointerType`, e `MakeGenericType` em qualquer instância do <xref:System.Type>, incluindo `ProvidedTypeDefinitions`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-390">You make provided members (whose signatures include array types, byref types, and instantiations of generic types) by using the normal `MakeArrayType`, `MakePointerType`, and `MakeGenericType` on any instance of <xref:System.Type>, including `ProvidedTypeDefinitions`.</span></span>

> [!NOTE]
> <span data-ttu-id="bb36b-391">Em alguns casos você talvez precise usar o auxiliar em `ProvidedTypeBuilder.MakeGenericType`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-391">In some cases you may have to use the helper in `ProvidedTypeBuilder.MakeGenericType`.</span></span>  <span data-ttu-id="bb36b-392">Consulte a [documentação do SDK do provedor de tipo](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) para obter mais detalhes.</span><span class="sxs-lookup"><span data-stu-id="bb36b-392">See the [Type Provider SDK documentation](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) for more details.</span></span>

### <a name="providing-unit-of-measure-annotations"></a><span data-ttu-id="bb36b-393">Fornecendo a unidade de medida anotações</span><span class="sxs-lookup"><span data-stu-id="bb36b-393">Providing Unit of Measure Annotations</span></span>

<span data-ttu-id="bb36b-394">A API ProvidedTypes fornece auxiliares para fornecer anotações de medida.</span><span class="sxs-lookup"><span data-stu-id="bb36b-394">The ProvidedTypes API provides helpers for providing measure annotations.</span></span> <span data-ttu-id="bb36b-395">Por exemplo, para fornecer o tipo `float<kg>`, use o seguinte código:</span><span class="sxs-lookup"><span data-stu-id="bb36b-395">For example, to provide the type `float<kg>`, use the following code:</span></span>
...
```
  ...
  <span data-ttu-id="bb36b-396">Para fornecer o tipo `Nullable<decimal<kg/m^2>>`, use o seguinte código:</span><span class="sxs-lookup"><span data-stu-id="bb36b-396">To provide the type `Nullable<decimal<kg/m^2>>`, use the following code:</span></span>
...
### <a name="accessing-project-local-or-script-local-resources"></a><span data-ttu-id="bb36b-397">Acessando recursos do projeto Local ou Local de Script</span><span class="sxs-lookup"><span data-stu-id="bb36b-397">Accessing Project-Local or Script-Local Resources</span></span>

<span data-ttu-id="bb36b-398">Cada instância de um provedor de tipo pode ser fornecida um `TypeProviderConfig` valor durante a construção.</span><span class="sxs-lookup"><span data-stu-id="bb36b-398">Each instance of a type provider can be given a `TypeProviderConfig` value during construction.</span></span> <span data-ttu-id="bb36b-399">Esse valor contém a pasta"Resolução" para o provedor (ou seja, a pasta de projeto para a compilação ou o diretório que contém um script), a lista de assemblies referenciados e outras informações.</span><span class="sxs-lookup"><span data-stu-id="bb36b-399">This value contains the "resolution folder" for the provider (that is, the project folder for the compilation or the directory that contains a script), the list of referenced assemblies, and other information.</span></span>

### <a name="invalidation"></a><span data-ttu-id="bb36b-400">Invalidação</span><span class="sxs-lookup"><span data-stu-id="bb36b-400">Invalidation</span></span>

<span data-ttu-id="bb36b-401">Provedores podem gerar sinais de invalidação para notificar o serviço de linguagem F # que as suposições de esquema podem ter sido alterado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-401">Providers can raise invalidation signals to notify the F# language service that the schema assumptions may have changed.</span></span> <span data-ttu-id="bb36b-402">Quando ocorre a invalidação, um typecheck é refeito se o provedor está sendo hospedado no Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="bb36b-402">When invalidation occurs, a typecheck is redone if the provider is being hosted in Visual Studio.</span></span> <span data-ttu-id="bb36b-403">Esse sinal será ignorada quando o provedor estiver hospedado em F # interativo ou pelo compilador F # (fsc.exe).</span><span class="sxs-lookup"><span data-stu-id="bb36b-403">This signal will be ignored when the provider is hosted in F# Interactive or by the F# Compiler (fsc.exe).</span></span>

### <a name="caching-schema-information"></a><span data-ttu-id="bb36b-404">Armazenar em cache informações de esquema</span><span class="sxs-lookup"><span data-stu-id="bb36b-404">Caching Schema Information</span></span>

<span data-ttu-id="bb36b-405">Provedores devem armazenar em cache geralmente acesso às informações de esquema.</span><span class="sxs-lookup"><span data-stu-id="bb36b-405">Providers must often cache access to schema information.</span></span> <span data-ttu-id="bb36b-406">Os dados armazenados em cache devem ser armazenados usando um nome de arquivo que é fornecido como um parâmetro estático ou como dados de usuário.</span><span class="sxs-lookup"><span data-stu-id="bb36b-406">The cached data should be stored by using a file name that's given as a static parameter or as user data.</span></span> <span data-ttu-id="bb36b-407">Um exemplo de cache de esquema é o `LocalSchemaFile` os provedores de tipo no parâmetro o `FSharp.Data.TypeProviders` assembly.</span><span class="sxs-lookup"><span data-stu-id="bb36b-407">An example of schema caching is the `LocalSchemaFile` parameter in the type providers in the `FSharp.Data.TypeProviders` assembly.</span></span> <span data-ttu-id="bb36b-408">Na implementação desses provedores, esse parâmetro estático instrui o provedor de tipo para usar as informações de esquema no arquivo local especificado em vez de acessar a fonte de dados pela rede.</span><span class="sxs-lookup"><span data-stu-id="bb36b-408">In the implementation of these providers, this static parameter directs the type provider to use the schema information in the specified local file instead of accessing the data source over the network.</span></span> <span data-ttu-id="bb36b-409">Para usar as informações de esquema em cache, você também deve definir o parâmetro static `ForceUpdate` para `false`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-409">To use cached schema information, you must also set the static parameter `ForceUpdate` to `false`.</span></span> <span data-ttu-id="bb36b-410">Você pode usar uma técnica semelhante para habilitar o acesso de dados online e offline.</span><span class="sxs-lookup"><span data-stu-id="bb36b-410">You could use a similar technique to enable online and offline data access.</span></span>

### <a name="backing-assembly"></a><span data-ttu-id="bb36b-411">Assembly de suporte</span><span class="sxs-lookup"><span data-stu-id="bb36b-411">Backing Assembly</span></span>

<span data-ttu-id="bb36b-412">Quando você compila um `.dll` ou `.exe` arquivo, o arquivo. dll de suporte para tipos gerados está vinculada estaticamente o assembly resultante.</span><span class="sxs-lookup"><span data-stu-id="bb36b-412">When you compile a `.dll` or `.exe` file, the backing .dll file for generated types is statically linked into the resulting assembly.</span></span> <span data-ttu-id="bb36b-413">Esse link é criado, copiando as definições de tipo de linguagem intermediária (IL) e todos os recursos gerenciados do conjunto de backup no assembly final.</span><span class="sxs-lookup"><span data-stu-id="bb36b-413">This link is created by copying the Intermediate Language (IL) type definitions and any managed resources from the backing assembly into the final assembly.</span></span> <span data-ttu-id="bb36b-414">Ao usar o F # interativo, o arquivo. dll de backup não sejam copiado e em vez disso, é carregado diretamente ao processo de F # interativo.</span><span class="sxs-lookup"><span data-stu-id="bb36b-414">When you use F# Interactive, the backing .dll file isn't copied and is instead loaded directly into the F# Interactive process.</span></span>

### <a name="exceptions-and-diagnostics-from-type-providers"></a><span data-ttu-id="bb36b-415">Exceções e diagnóstico de provedores de tipos</span><span class="sxs-lookup"><span data-stu-id="bb36b-415">Exceptions and Diagnostics from Type Providers</span></span>

<span data-ttu-id="bb36b-416">Todos os usos de todos os membros de tipos fornecidos podem lançar exceções.</span><span class="sxs-lookup"><span data-stu-id="bb36b-416">All uses of all members from provided types may throw exceptions.</span></span> <span data-ttu-id="bb36b-417">Em todos os casos, se um provedor de tipos gera uma exceção, o compilador de host atributos o erro para um provedor de tipo específico.</span><span class="sxs-lookup"><span data-stu-id="bb36b-417">In all cases, if a type provider throws an exception, the host compiler attributes the error to a specific type provider.</span></span>

- <span data-ttu-id="bb36b-418">Tipo de provedor exceções nunca deve resultar em erros internos do compilador.</span><span class="sxs-lookup"><span data-stu-id="bb36b-418">Type provider exceptions should never result in internal compiler errors.</span></span>

- <span data-ttu-id="bb36b-419">Provedores de tipos não podem relatar avisos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-419">Type providers can't report warnings.</span></span>

- <span data-ttu-id="bb36b-420">Quando um provedor de tipo é hospedado no compilador F #, um ambiente de desenvolvimento em F # ou F # interativo, todas as exceções do provedor são capturadas.</span><span class="sxs-lookup"><span data-stu-id="bb36b-420">When a type provider is hosted in the F# compiler, an F# development environment, or F# Interactive, all exceptions from that provider are caught.</span></span> <span data-ttu-id="bb36b-421">A propriedade de mensagem sempre é o texto de erro e nenhum rastreamento de pilha é exibida.</span><span class="sxs-lookup"><span data-stu-id="bb36b-421">The Message property is always the error text, and no stack trace appears.</span></span> <span data-ttu-id="bb36b-422">Se você pretende lançar uma exceção, você pode lançar os exemplos a seguir: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-422">If you’re going to throw an exception, you can throw the following examples: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span></span>

#### <a name="providing-generated-types"></a><span data-ttu-id="bb36b-423">Fornecimento de tipos gerados</span><span class="sxs-lookup"><span data-stu-id="bb36b-423">Providing Generated Types</span></span>

<span data-ttu-id="bb36b-424">Até agora, este documento explicou como fornecer tipos apagados.</span><span class="sxs-lookup"><span data-stu-id="bb36b-424">So far, this document has explained how to provide erased types.</span></span> <span data-ttu-id="bb36b-425">Você também pode usar o mecanismo de provedor de tipo em F # para fornecer tipos gerados, que são adicionados como definições de tipo real do .NET no programa dos usuários.</span><span class="sxs-lookup"><span data-stu-id="bb36b-425">You can also use the type provider mechanism in F# to provide generated types, which are added as real .NET type definitions into the users' program.</span></span> <span data-ttu-id="bb36b-426">Você deve se referir a gerado tipos fornecidos por meio de uma definição de tipo.</span><span class="sxs-lookup"><span data-stu-id="bb36b-426">You must refer to generated provided types by using a type definition.</span></span>
...
<span data-ttu-id="bb36b-427">O código do auxiliar de 0,2 ProvidedTypes que faz parte da versão 3.0 do F # tem somente suporte limitado para fornecer tipos gerados.</span><span class="sxs-lookup"><span data-stu-id="bb36b-427">The ProvidedTypes-0.2 helper code that is part of the F# 3.0 release has only limited support for providing generated types.</span></span> <span data-ttu-id="bb36b-428">As instruções a seguir devem ser verdadeiras para uma definição de tipo gerado:</span><span class="sxs-lookup"><span data-stu-id="bb36b-428">The following statements must be true for a generated type definition:</span></span>

- <span data-ttu-id="bb36b-429">`isErased` deve ser definido como `false`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-429">`isErased` must be set to `false`.</span></span>

- <span data-ttu-id="bb36b-430">O tipo gerado deve ser adicionado a um recentemente construído `ProvidedAssembly()`, que representa um contêiner para fragmentos de código gerado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-430">The generated type must be added to a newly constructed `ProvidedAssembly()`, which represents a container for generated code fragments.</span></span>

- <span data-ttu-id="bb36b-431">O provedor deve ter um assembly que tem um arquivo. dll do .NET de backup real com um arquivo. dll correspondente no disco.</span><span class="sxs-lookup"><span data-stu-id="bb36b-431">The provider must have an assembly that has an actual backing .NET .dll file with a matching .dll file on disk.</span></span>

## <a name="rules-and-limitations"></a><span data-ttu-id="bb36b-432">Regras e limitações</span><span class="sxs-lookup"><span data-stu-id="bb36b-432">Rules and Limitations</span></span>

<span data-ttu-id="bb36b-433">Quando você escrever provedores de tipos, tenha as seguintes regras e limitações em mente.</span><span class="sxs-lookup"><span data-stu-id="bb36b-433">When you write type providers, keep the following rules and limitations in mind.</span></span>

### <a name="provided-types-must-be-reachable"></a><span data-ttu-id="bb36b-434">Tipos fornecidos devem ser acessíveis</span><span class="sxs-lookup"><span data-stu-id="bb36b-434">Provided types must be reachable</span></span>

<span data-ttu-id="bb36b-435">Todos fornecidos tipos devem ser acessíveis a partir de tipos não aninhadas.</span><span class="sxs-lookup"><span data-stu-id="bb36b-435">All provided types should be reachable from the non-nested types.</span></span> <span data-ttu-id="bb36b-436">Os tipos aninhados não são fornecidos na chamada para o `TypeProviderForNamespaces` construtor ou uma chamada para `AddNamespace`.</span><span class="sxs-lookup"><span data-stu-id="bb36b-436">The non-nested types are given in the call to the `TypeProviderForNamespaces` constructor or a call to `AddNamespace`.</span></span> <span data-ttu-id="bb36b-437">Por exemplo, se o provedor fornece um tipo `StaticClass.P : T`, você deve garantir que T é um tipo não aninhado ou aninhados em um.</span><span class="sxs-lookup"><span data-stu-id="bb36b-437">For example, if the provider provides a type `StaticClass.P : T`, you must ensure that T is either a non-nested type or nested under one.</span></span>

<span data-ttu-id="bb36b-438">Por exemplo, alguns provedores têm uma classe estática, como `DataTypes` que contêm esses `T1, T2, T3, ...` tipos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-438">For example, some providers have a static class such as `DataTypes` that contain these `T1, T2, T3, ...` types.</span></span> <span data-ttu-id="bb36b-439">Caso contrário, o erro afirma que uma referência ao tipo T em um assembly foi encontrada, mas não foi possível encontrar o tipo nesse assembly.</span><span class="sxs-lookup"><span data-stu-id="bb36b-439">Otherwise, the error says that a reference to type T in assembly A was found, but the type couldn't be found in that assembly.</span></span> <span data-ttu-id="bb36b-440">Se esse erro aparecer, verifique se todos os seus subtipos podem ser acessados entre os tipos de provedor.</span><span class="sxs-lookup"><span data-stu-id="bb36b-440">If this error appears, verify that all your subtypes can be reached from the provider types.</span></span> <span data-ttu-id="bb36b-441">Observação: Essas `T1, T2, T3...` tipos são chamados da *em interrupções* tipos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-441">Note: These `T1, T2, T3...` types are referred to as the *on-the-fly* types.</span></span> <span data-ttu-id="bb36b-442">Lembre-se de colocá-los em um namespace acessível ou um tipo de pai.</span><span class="sxs-lookup"><span data-stu-id="bb36b-442">Remember to put them in an accessible namespace or a parent type.</span></span>

### <a name="limitations-of-the-type-provider-mechanism"></a><span data-ttu-id="bb36b-443">Limitações do mecanismo de provedor de tipo</span><span class="sxs-lookup"><span data-stu-id="bb36b-443">Limitations of the Type Provider Mechanism</span></span>

<span data-ttu-id="bb36b-444">O mecanismo de provedor de tipo em F # tem as seguintes limitações:</span><span class="sxs-lookup"><span data-stu-id="bb36b-444">The type provider mechanism in F# has the following limitations:</span></span>

- <span data-ttu-id="bb36b-445">A infraestrutura subjacente para provedores de tipos em F # não oferece suporte fornecido genérico tipos ou fornecidos métodos genéricos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-445">The underlying infrastructure for type providers in F# doesn't support provided generic types or provided generic methods.</span></span>

- <span data-ttu-id="bb36b-446">O mecanismo não dá suporte a tipos aninhados com parâmetros estáticos.</span><span class="sxs-lookup"><span data-stu-id="bb36b-446">The mechanism doesn't support nested types with static parameters.</span></span>

## <a name="development-tips"></a><span data-ttu-id="bb36b-447">Dicas de desenvolvimento</span><span class="sxs-lookup"><span data-stu-id="bb36b-447">Development Tips</span></span>

<span data-ttu-id="bb36b-448">Você pode achar as seguintes dicas úteis durante o processo de desenvolvimento.</span><span class="sxs-lookup"><span data-stu-id="bb36b-448">You might find the following tips helpful during the development process.</span></span>

### <a name="run-two-instances-of-visual-studio"></a><span data-ttu-id="bb36b-449">Executar duas instâncias do Visual Studio</span><span class="sxs-lookup"><span data-stu-id="bb36b-449">Run Two Instances of Visual Studio</span></span>

<span data-ttu-id="bb36b-450">Você pode desenvolver o provedor de tipo em uma instância e testar o provedor nos outros porque o IDE de teste tem um bloqueio no arquivo. dll que impede que o provedor de tipo que está sendo recriado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-450">You can develop the type provider in one instance and test the provider in the other because the test IDE will take a lock on the .dll file that prevents the type provider from being rebuilt.</span></span> <span data-ttu-id="bb36b-451">Portanto, você deve fechar a segunda instância do Visual Studio enquanto o provedor é criado na primeira instância e, em seguida, você deverá reabri-la a segunda instância depois que o provedor é compilado.</span><span class="sxs-lookup"><span data-stu-id="bb36b-451">Thus, you must close the second instance of Visual Studio while the provider is built in the first instance, and then you must reopen the second instance after the provider is built.</span></span>

### <a name="debug-type-providers-by-using-invocations-of-fscexe"></a><span data-ttu-id="bb36b-452">Provedores de tipos de depuração por meio de invocações de fsc.exe</span><span class="sxs-lookup"><span data-stu-id="bb36b-452">Debug type providers by using invocations of fsc.exe</span></span>

<span data-ttu-id="bb36b-453">Você pode invocar provedores de tipos, usando as seguintes ferramentas:</span><span class="sxs-lookup"><span data-stu-id="bb36b-453">You can invoke type providers by using the following tools:</span></span>

- <span data-ttu-id="bb36b-454">FSC.exe (compilador de linha de comando do F #)</span><span class="sxs-lookup"><span data-stu-id="bb36b-454">fsc.exe (The F# command line compiler)</span></span>

- <span data-ttu-id="bb36b-455">FSI.exe (compilador do F # interativo)</span><span class="sxs-lookup"><span data-stu-id="bb36b-455">fsi.exe (The F# Interactive compiler)</span></span>

- <span data-ttu-id="bb36b-456">devenv.exe (Visual Studio)</span><span class="sxs-lookup"><span data-stu-id="bb36b-456">devenv.exe (Visual Studio)</span></span>

<span data-ttu-id="bb36b-457">Muitas vezes você pode depurar provedores de tipos com mais facilidade usando fsc.exe em um arquivo de script de teste (por exemplo, script.fsx).</span><span class="sxs-lookup"><span data-stu-id="bb36b-457">You can often debug type providers most easily by using fsc.exe on a test script file (for example, script.fsx).</span></span> <span data-ttu-id="bb36b-458">Você pode iniciar um depurador em um prompt de comando.</span><span class="sxs-lookup"><span data-stu-id="bb36b-458">You can launch a debugger from a command prompt.</span></span>

```
  devenv /debugexe fsc.exe script.fsx
```

  <span data-ttu-id="bb36b-459">Você pode usar o log de impressão para stdout.</span><span class="sxs-lookup"><span data-stu-id="bb36b-459">You can use print-to-stdout logging.</span></span>

## <a name="see-also"></a><span data-ttu-id="bb36b-460">Consulte também</span><span class="sxs-lookup"><span data-stu-id="bb36b-460">See also</span></span>

- [<span data-ttu-id="bb36b-461">Provedores de Tipos</span><span class="sxs-lookup"><span data-stu-id="bb36b-461">Type Providers</span></span>](index.md)
- [<span data-ttu-id="bb36b-462">O SDK do provedor de tipo</span><span class="sxs-lookup"><span data-stu-id="bb36b-462">The Type Provider SDK</span></span>](https://github.com/fsprojects/FSharp.TypeProviders.SDK)
