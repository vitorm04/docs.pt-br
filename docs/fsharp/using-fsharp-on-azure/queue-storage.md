...
Os exemplos neste artigo podem ser usados em um F# aplicativo ou um F# script. Para criar um F# script, crie um arquivo com a `.fsx` extensão, por exemplo `queues.fsx`, em seu F# ambiente de desenvolvimento.
...
Adicione as seguintes `open` instruções à parte superior `queues.fsx` do arquivo:
...
A `CloudQueueClient` classe permite que você recupere filas armazenadas no armazenamento de filas. Aqui está uma maneira de criar o cliente de serviço:
...
Para inserir uma mensagem em uma fila existente, primeiro crie uma nova `CloudQueueMessage`. Em seguida, chame `AddMessage` o método. Um `CloudQueueMessage` pode ser criado por meio de uma cadeia de caracteres (em formato UTF-8 `byte` ) ou de uma matriz, como esta:
...
Posteriormente, você indicará o processamento bem-sucedido da mensagem `DeleteMessage`usando.
...
Você pode obter uma estimativa do número de mensagens em uma fila. O `FetchAttributes` método solicita ao serviço fila recuperar os atributos da fila, incluindo a contagem de mensagens. A `ApproximateMessageCount` propriedade retorna o último valor recuperado `FetchAttributes` pelo método, sem chamar o serviço fila.
