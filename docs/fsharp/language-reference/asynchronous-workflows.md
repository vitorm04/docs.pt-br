...
Usar métodos assíncronos no .NET Framework que são projetados para outros modelos assíncronos com o F# modelo de programação assíncrona, você cria uma função que retorna um F# `Async` objeto. O F# biblioteca tem funções que tornam isso mais fácil de fazer.
