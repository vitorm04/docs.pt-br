...
No contexto do F# idioma, uma *módulo* é um agrupamento de F# de código, como valores, tipos e valores de função em um F# programa. O agrupamento de código em módulos ajuda a manter junto o código relacionado e ajuda a evitar conflitos de nome em seu programa.
...
Você pode abrir o módulo ou um ou mais namespaces para simplificar o código. Para obter mais informações sobre namespaces de abertura e de módulos, consulte [declarações de importação: O `open` palavra-chave](import-declarations-the-open-keyword.md).
