...
Este tópico descreve as opções de linha de comando com F# suporte do `fsi.exe`Interactive,. F#O Interactive aceita muitas das mesmas opções de linha de comando F# que o compilador, mas também aceita algumas opções adicionais.
...
A extensão de arquivo F# para arquivos de `.fsx`script é.
...
|**--reference:&lt;filename&gt;**<br /><br />**-r:&lt;nome de arquivo&gt;**|O mesmo que a opção de compilador **FSC. exe** . Para obter mais informações, consulte [Opções do compilador](compiler-options.md).|
