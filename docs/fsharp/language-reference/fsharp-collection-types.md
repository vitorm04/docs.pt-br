...
Ao analisar este tópico, você pode determinar qual F# tipo de coleção melhor atende a uma necessidade específica. Esses tipos de coleção diferem dos tipos de coleção no .NET Framework, como aqueles no namespace `System.Collections.Generic`, nos quais os F# tipos de coleção são criados a partir de uma perspectiva de programação funcional em vez de uma perspectiva orientada a objeto. Mais especificamente, somente a coleção de matrizes tem elementos mutáveis. Portanto, ao modificar uma coleção, você cria uma instância da coleção modificada em vez de alterar a coleção original.
...
A tabela a seguir F# mostra os tipos de coleção.

|{1&gt;Tipo&lt;1}|Descrição|Links Relacionados|
|----|-----------|-------------|
|[List](https://msdn.microsoft.com/library/c627b668-477b-4409-91ed-06d7f1b3e4a7)|Uma série ordenada e imutável de elementos do mesmo tipo. Implementado como uma lista vinculada.|[Listas](lists.md)<br /><br />[Módulo de lista](https://msdn.microsoft.com/library/a2264ba3-2d45-40dd-9040-4f7aa2ad9788)|
...
|[Set](https://msdn.microsoft.com/library/50cebdce-0cd7-4c5c-8ebc-f3a9e90b38d8)|Um conjunto imutável com base em árvores binárias, em que comparação é a F# função de comparação estrutural, que potencialmente usa implementações da interface `System.IComparable` em valores de chave.|[Definir módulo](https://msdn.microsoft.com/library/61efa732-d55d-4c32-993f-628e2f98e6a0)|

### <a name="table-of-functions"></a>Tabela de funções

Esta seção compara as funções que estão disponíveis em F# tipos de coleção. A complexidade computacional da função é determinada, em que N é o tamanho da primeira coleção e M é o tamanho da segunda coleção, se houver. Um traço (-) indica que essa função não está disponível na coleção. Como as sequências são avaliadas lentamente, uma função como Seq. Distinct pode ser O (1) porque ela retorna imediatamente, embora ainda afete o desempenho da sequência quando enumerada.

|Função|Matriz|Lista|Sequence|Mapa|Set|Descrição|
|--------|-----|----|--------|---|---|-----------|
|acrescentar|O (M)|O (N)|O (N)|-|-|Retorna uma nova coleção que contém os elementos da primeira coleção seguida por elementos da segunda coleção.|
...
|escolha|O (N)|O (N)|O (N)|-|-|Aplica a função fornecida `f` a cada elemento `x` da lista. Retorna a lista que contém os resultados para cada elemento em que a função retorna `Some(f(x))`.|
|recolher|O (N)|O (N)|O (N)|-|-|Aplica a função fornecida a cada elemento da coleção, concatena todos os resultados e retorna a lista combinada.|
...
|{1&gt;count&lt;1}|-|-|-|-|O (N)|Retorna o número de elementos no conjunto.|
...
|criar|O (N)|-|-|-|-|Cria uma matriz de elementos inteiros que são inicialmente o valor determinado.|
|delay|-|-|O(1)|-|-|Retorna uma sequência que é criada a partir da especificação atrasada especificada de uma sequência.|
...
|filtrar|O (N)|O (N)|O (N)|O (N)|O (N)|Retorna uma nova coleção que contém somente os elementos da coleção para os quais o predicado fornecido retorna `true`.|
|find|O (N)|O (N)|O (N)|O (log N)|-|Retorna o primeiro elemento para o qual a função fornecida retorna `true`. Retornará `System.Collections.Generic.KeyNotFoundException` se nenhum elemento desse tipo existir.|
...
|formam|-|-|-|-|O (log N &#42; log M)|Computa a interseção de dois conjuntos.|
...
|{1&gt;length&lt;1}|O(1)|O (N)|O (N)|-|-|Retorna o número de elementos na coleção.|
|map|O (N)|O (N)|O(1)|-|-|Cria uma coleção cujos elementos são os resultados da aplicação da função determinada em cada elemento da matriz.|
...
|partição|O (N)|O (N)|-|O (N)|O (N)|Divide a coleção em duas coleções. A primeira coleção contém os elementos para os quais o predicado fornecido retorna `true`e a segunda coleção contém os elementos para os quais o predicado fornecido retorna `false`.|
...
|{1&gt;remove&lt;1}|-|-|-|O (log N)|O (log N)|Remove um elemento do domínio do mapa. Nenhuma exceção será gerada se o elemento não estiver presente.|
...
|skipWhile|-|-|O (N)|-|-|Retorna uma sequência que, quando iterada, ignora os elementos da sequência subjacente, enquanto o predicado fornecido retorna `true` e, em seguida, produz os elementos restantes da sequência.|
...
|união|-|-|-|-|O (M &#42; log N)|Computa a União dos dois conjuntos.|
