...
|----|-----------|-------------|
|[Lista](https://msdn.microsoft.com/library/c627b668-477b-4409-91ed-06d7f1b3e4a7)|Uma série ordenada e imutável de elementos do mesmo tipo. Implementado como uma lista vinculada.|[Listas](lists.md)<br /><br />[Módulo de lista](https://msdn.microsoft.com/library/a2264ba3-2d45-40dd-9040-4f7aa2ad9788)|
...
|[Configurar](https://msdn.microsoft.com/library/50cebdce-0cd7-4c5c-8ebc-f3a9e90b38d8)|Um conjunto imutável com base em árvores binárias, em que comparação é a F# função de comparação estrutural, que potencialmente usa implementações da interface `System.IComparable` em valores de chave.|[Definir módulo](https://msdn.microsoft.com/library/61efa732-d55d-4c32-993f-628e2f98e6a0)|
...
|choose|O (N)|O (N)|O (N)|-|-|Aplica a função fornecida `f` a cada elemento `x` da lista. Retorna a lista que contém os resultados para cada elemento em que a função retorna `Some(f(x))`.|
...
|filtro|O (N)|O (N)|O (N)|O (N)|O (N)|Retorna uma nova coleção que contém somente os elementos da coleção para os quais o predicado fornecido retorna `true`.|
|localizar|O (N)|O (N)|O (N)|O (log N)|-|Retorna o primeiro elemento para o qual a função fornecida retorna `true`. Retornará `System.Collections.Generic.KeyNotFoundException` se nenhum elemento desse tipo existir.|
...
|principal|-|O(1)|O(1)|-|-|Retorna o primeiro elemento da coleção.|
...
|partition|O (N)|O (N)|-|O (N)|O (N)|Divide a coleção em duas coleções. A primeira coleção contém os elementos para os quais o predicado fornecido retorna `true`e a segunda coleção contém os elementos para os quais o predicado fornecido retorna `false`.|
|permudo|O (N)|O (N)|-|-|-|Retorna uma matriz com todos os elementos sem mudo de acordo com a permutação especificada.|
