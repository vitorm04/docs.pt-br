...
Você pode definir valores em expressões conhecidas como *expressões de registro*. O compilador infere o tipo dos rótulos usados (se os rótulos forem suficientemente distintos dos outros tipos de registro). As chaves ({}) incluem a expressão de registro. O código a seguir mostra uma expressão de registro que inicializa um registro com três elementos float `x`com `y` rótulos `z`e.
...
Os rótulos do tipo declarado mais recentemente têm precedência sobre aqueles do tipo declarado anteriormente, portanto, no exemplo anterior, `mypoint3D` é inferido `Point3D`como. Você pode especificar explicitamente o tipo de registro, como no código a seguir.
...
A definição de registros recursivos mutuamente é feita `and` com a palavra-chave. Isso permite que você vincule 2 ou mais tipos de registro juntos.

Por exemplo, o código a seguir define `Person` um `Address` e tipo como recursivo mutuamente:
...
Se você escrever o mesmo código com classes, os dois objetos de classe seriam desiguais porque os dois valores representariam dois objetos no heap e apenas os endereços seriam comparados (a menos que o tipo de `System.Object.Equals` classe substitua o método).

Se você precisar de igualdade de referência para registros, adicione `[<ReferenceEquality>]` o atributo acima do registro.
