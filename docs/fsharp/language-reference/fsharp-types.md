...
Alguns tipos são considerados *tipos primitivos*, como o tipo `bool` booliano e os tipos de ponto flutuante e integral de vários tamanhos, que incluem tipos de bytes e caracteres. Esses tipos são descritos em [tipos primitivos](primitive-types.md).
...
|array|*nome do tipo* [] ou<br /><br />matriz *de nome de tipo*|`int[]`<br /><br />`array<int>`<br /><br />`int array`|
|matriz bidimensional|*nome do tipo* [,]|`int[,]`<br /><br />`float[,]`|
|matriz tridimensional|*type-name*[,,]|`float[,,]`|
|serem|*digite-Nome1* &#42; *digite-nome2* ...|Por exemplo, `(1,'b',3)` tem tipo`int * char * int`|
...
|tipo de função que tem um único parâmetro|*parameter-type1* -&gt; *return-type*|Uma função que usa um `int` e retorna um `string` tipo tem`int -> string`|
|tipo de função que tem vários parâmetros|*parâmetro-type1*  - Parameter- - type2&gt; ...-tipoderetorno&gt; &gt;|Uma função que usa um `int` e um `float` e retorna um `string` tipo`int -> float -> string`|
...
|Tópico|Descrição|
|-----|-----------|
|[Tipos Primitivos](primitive-types.md)|Descreve os tipos simples internos, como tipos integrais, o tipo booliano e os tipos de caracteres.|
