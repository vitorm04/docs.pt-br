...
## <a name="example"></a>Exemplo  
 O exemplo a seguir configura um serviço para expor metadados usando o \<elemento > de serviços de metadados. Ele também configura um ponto de extremidade para expor o `IMetadataExchange` contrato como uma implementação de um protocolo de WS-MetadataExchange (MEX). O exemplo usa o `mexHttpBinding`, que é uma associação padrão de conveniência equivalente `wsHttpBinding` ao com o modo de segurança definido como `None`. Um endereço relativo de "MEX" é usado no ponto de extremidade, que, quando resolvido em relação ao endereço base de serviços, resulta `http://localhost/servicemodelsamples/service.svc/mex`em um endereço de ponto de extremidade de.  
