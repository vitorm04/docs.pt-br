...
## <a name="remarks"></a>Comentários  
 ...
 A configuração padrão do `NetNamedPipeBinding` é semelhante à configuração fornecida `NetTcpBinding`pelo, mas é mais simples porque a implementação do WCF destina-se apenas ao uso no computador e, consequentemente, há menos recursos expostos. A diferença mais notável é que a `securityMode` configuração só oferece as `None` opções `Transport` e. O suporte à segurança SOAP não é uma opção incluída. O comportamento de segurança é configurável usando o `securityMode` atributo opcional.  
...
## <a name="example"></a>Exemplo  
 ...
 A associação é especificada nos arquivos de configuração para o cliente e o serviço. O tipo de associação é especificado no `binding` atributo `<endpoint>` do elemento. Se você quiser configurar a associação netNamedPipeBinding e alterar algumas de suas configurações, deverá definir uma configuração de associação. O ponto de extremidade deve referenciar a configuração de associação `bindingConfiguration` por nome com um atributo. Neste exemplo, a configuração de associação é denominada Binding1.  
