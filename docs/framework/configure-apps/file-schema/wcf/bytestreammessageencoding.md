...
|Atributo|Descrição|  
|---------------|-----------------|  
|messageVersion|Especifica a versão SOAP das mensagens enviadas usando a associação. Essa propriedade só pode ser definida como o valor de versão da mensagem de <xref:System.ServiceModel.Channels.MessageVersion.None%2A>. O codificador de mensagens de fluxo de bytes não oferece suporte a outras versões de mensagem.<br /><br /> Este atributo é do tipo <xref:System.ServiceModel.Channels.MessageVersion>.|  
