...
|contextProtectionLevel|Um valor <xref:System.Net.Security.ProtectionLevel> válido que especifica o nível de proteção desejado do cabeçalho SOAP usado para propagar as informações de contexto.  O valor padrão é <xref:System.Net.Security.ProtectionLevel.Sign>.|  
