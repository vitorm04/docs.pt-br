...
|`closeTimeout`|Um valor <xref:System.TimeSpan> que especifica o intervalo de tempo para a conclusão de uma operação de fechamento. Esse valor deve ser maior ou igual a <xref:System.TimeSpan.Zero>. O padrão é 00:01:00.|  
...
|`textEncoding`|Especifica a codificação do conjunto de caracteres a ser usada para emitir mensagens na associação. Os valores válidos incluem o seguinte:<br /><br /> -   `UnicodeFffeTextEncoding`: codificação Unicode big endian.<br />-   `Utf16TextEncoding`: codificação de 16 bits.<br />-   `Utf8TextEncoding`: codificação de 8 bits.<br /><br /> O padrão é `Utf8TextEncoding`.<br /><br /> Este atributo é do tipo <xref:System.Text.Encoding>.|  
