...
|queueTransferProtocol|Especifica o transporte de canal de comunicação enfileirado que essa associação usa. Os valores válidos são<br /><br /> Forma  Use o protocolo nativo do MSMQ.<br />SRMP  Use o protocolo SRMP (Soap Reliable Messaging Protocol).<br />- SrmpSecure: Use o transporte SRMPS (Soap Reliable Messaging Protocol Secure).<br /><br /> Esse atributo é do tipo <xref:System.ServiceModel.QueueTransferProtocol>.<br /><br /> Como o MSMQ não dá suporte ao endereçamento de Active Directory ao usar o protocolo de mensagens confiáveis SOAP, você não deve definir esse atributo `useActiveDirectory` como Srmp ou `true`srmps quando é definido como.|  
