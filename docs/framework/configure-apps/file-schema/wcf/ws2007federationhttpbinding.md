...
|`textEncoding`|Define a codificação do conjunto de caracteres a ser usada para emitir mensagens na associação. Os valores válidos incluem o seguinte:<br /><br /> BigEndianUnicode Codificação Big Endian Unicode.<br />Unicode codificação de 16 bits.<br />UTF8 codificação de 8 bits.<br /><br /> O padrão é UTF8. Esse atributo é do tipo <xref:System.Text.Encoding>.|
|`transactionFlow`|Um valor que especifica se a associação dá suporte ao fluxo de WS-Transactions. O padrão é `false`.|
|`useDefaultWebProxy`|Um valor que indica se o proxy HTTP autoconfigurado do sistema é usado. O endereço de proxy deve `null` ser (ou seja, não definido) se esse atributo `true`for. O padrão é `true`.|
