...
|[> de \<de portais](servicebehaviors.md)|Esta seção de configuração representa todos os comportamentos definidos para um serviço específico.|  
...
## <a name="remarks"></a>Comentários  
 Você pode usar o elemento `<remove>` para remover um comportamento específico da coleção. Para fazer isso, basta fornecer o nome do comportamento a ser removido no atributo `name` do elemento `<remove>`.  Você também pode usar o elemento `<clear>` para garantir que uma coleção de comportamentos comece vazia limpando todo o conteúdo da coleção.  
