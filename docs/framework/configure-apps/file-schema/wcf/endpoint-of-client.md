...
|contrato|Atributo de cadeia de caracteres obrigatória.<br /><br /> Uma cadeia de caracteres que indica qual contrato esse ponto de extremidade está expondo. O assembly deve implementar o tipo de contrato.|  
...
|tipo|Uma cadeia de caracteres que especifica o tipo de ponto de extremidade padrão aplicado. O tipo deve ser registrado no `<extensions>` seção ou em Machine. config. Se nada for especificado, um ponto de extremidade de canal comum é criado.|  
