...
|serializationFormat|Especifica o formatador que é usado para serializar objetos que são enviados como parte de uma mensagem MSMQ. Os valores válidos são<br /><br /> -ActiveX: O formatador ActiveX é usado ao serializar objetos COM.<br />-Binary: Serializa o objeto em um pacote de binário.<br />-ByteArray: Serializa o objeto em uma matriz de bytes.<br />-Stream: Serializa o objeto em um fluxo.<br />-Xml: Serializa o objeto em um pacote XML. O padrão é XML.<br /><br /> Esse atributo é do tipo <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>.|  
