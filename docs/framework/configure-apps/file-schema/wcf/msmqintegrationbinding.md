...
|receiveRetryCount|Um inteiro que especifica o número máximo de tentativas imediatas que o Gerenciador de fila deve tentar se a transmissão de uma mensagem da fila do aplicativo para o aplicativo falhar.<br /><br /> Se o número máximo de tentativas de entrega for atingido e a mensagem não for acessada pelo aplicativo, a mensagem será enviada a uma fila de repetição para entrega novamente mais tarde. O período de tempo antes que a mensagem seja transferida de volta para a fila de `retryCycleDelay`envio é controlada pelo. Se os ciclos de repetição `maxRetryCycles` atingirem o valor, a mensagem será enviada para a fila de mensagens suspeitas ou uma confirmação negativa será enviada de volta ao remetente.|  
...
## <a name="remarks"></a>Comentários  
 Esse elemento de associação pode ser usado para permitir que aplicativos Windows Communication Foundation (WCF) enviem mensagens para e recebam mensagens de aplicativos MSMQ existentes que usam com, APIs nativas do MSMQ ou os tipos <xref:System.Messaging?displayProperty=nameWithType> definidos no namespace que você pode usar este elemento de configuração para especificar maneiras de endereçar a fila, garantir a transferência, se as mensagens devem ser permanentemente armazenadas e como as mensagens devem ser protegidas e autenticadas. Para obter mais informações, confira [Como: Trocar mensagens com pontos de extremidade WCF e aplicativos](../../../wcf/feature-details/how-to-exchange-messages-with-wcf-endpoints-and-message-queuing-applications.md)de enfileiramento de mensagens.  
