...
|[\<enableWebScript>](enablewebscript.md)|Habilita o comportamento do ponto de extremidade que torna possível consumir o serviço de páginas da Web do ASP.NET AJAX. O comportamento só deve ser usado em conjunto com a associação \<de WebHttpBinding > padrão ou com o \<elemento de associação de > webMessageEncoding.|  
