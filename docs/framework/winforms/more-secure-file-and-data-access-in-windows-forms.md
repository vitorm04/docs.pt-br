...
> [!NOTE]
> A permissão específica não <xref:System.Windows.Forms.OpenFileDialog.OpenFile%2A> é solicitada até que o método seja realmente chamado.  
  
 A permissão para exibir uma caixa de diálogo de <xref:System.Windows.Forms.FileDialog>arquivo <xref:System.Windows.Forms.OpenFileDialog>não <xref:System.Windows.Forms.SaveFileDialog> concede ao seu aplicativo acesso total a todos os membros das classes e classes. Para obter as permissões exatas necessárias para chamar cada método, consulte o tópico de referência para esse método na documentação da biblioteca de classe .NET Framework.  
  
 O exemplo de <xref:System.Windows.Forms.OpenFileDialog.OpenFile%2A> código a seguir usa o método <xref:System.Windows.Forms.RichTextBox> para abrir um arquivo especificado pelo usuário em um controle. O exemplo <xref:System.Security.Permissions.FileDialogPermission> requer <xref:System.Security.Permissions.FileDialogPermissionAttribute.Open%2A> e o valor de enumeração associado. O exemplo demonstra como <xref:System.Security.SecurityException> lidar com o para determinar se o recurso de salvamento deve ser desativado. Este exemplo requer <xref:System.Windows.Forms.Form> que <xref:System.Windows.Forms.Button> o `ButtonOpen`seu <xref:System.Windows.Forms.RichTextBox> tenha `RtfBoxMain`um controle chamado , e um controle chamado .  
...
## <a name="database-access"></a>Acesso a banco de dados  
...
- [Segurança de formulários do Windows](windows-forms-security.md)
