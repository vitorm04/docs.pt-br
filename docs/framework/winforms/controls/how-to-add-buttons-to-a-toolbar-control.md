...
> O controle <xref:System.Windows.Forms.ToolStrip> substitui e adiciona funcionalidade ao controle <xref:System.Windows.Forms.ToolBar>, no entanto, o controle <xref:System.Windows.Forms.ToolBar> é mantido para compatibilidade com versões anteriores e para uso futuro, se desejado.  
 ...
 Uma parte integrante <xref:System.Windows.Forms.ToolBar> do controle são os botões que você adiciona a ele. Eles podem ser usados para fornecer acesso fácil aos comandos de menu ou, como alternativa, podem ser colocados em outra área da interface do usuário do seu aplicativo para expor comandos para os usuários que não estão disponíveis na estrutura do menu.  
  
 Os exemplos abaixo supõem que um <xref:System.Windows.Forms.ToolBar> controle`Form1`foi adicionado a um formulário do Windows ( ).  
  
### <a name="to-add-buttons-programmatically"></a>Para adicionar botões programáticamente  
  
1. Em um procedimento, crie botões de barra <xref:System.Windows.Forms.ToolBar.Buttons%2A?displayProperty=nameWithType> de ferramentas adicionando-os à coleção.  
  
2. Especifique as configurações de propriedade para um <xref:System.Windows.Forms.ToolBar.Buttons%2A> botão individual passando o índice do botão através da propriedade.  
  
     O exemplo abaixo assume um <xref:System.Windows.Forms.ToolBar> formulário com um controle já adicionado.  
  
    > [!NOTE]
    > A <xref:System.Windows.Forms.ToolBar.Buttons%2A?displayProperty=nameWithType> coleção é uma coleção baseada em zero, então o código deve proceder em conformidade.  
