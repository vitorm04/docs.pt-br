...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>'\<Nome da guia de caixa de ferramentas > ' não é uma categoria de caixa de ferramentas
...
Mensagem de erro: "Um analisador de idioma solicitado não está instalado. O nome do analisador de idioma{0}é ' '.
...
### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Ocorreu uma exceção ao tentar criar uma instância do '\<nome do objeto > '

Mensagem de erro: "Ocorreu uma exceção ao tentar criar uma instância de '\<nome do objeto > '. A exceção era "\<cadeia de\>caracteres de exceção".
...
### <a name="array-rank-rank-in-array-is-too-high"></a>A classificação de\<matriz ' Rank na matriz > ' é muito alta
...
Mensagem de erro: Não foi possível\<abrir o "assembly" do nome do assembly > '. Verifique se o arquivo ainda existe. "
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Não é possível nomear\<o objeto ' name > ' porque ele já\<é chamado ' name > '
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>A categoria\<' nome da guia de caixa de ferramentas > ' não tem uma\<ferramenta para a classe ' nome da classe > '
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>O componente\<"nome do componente >" não chamou contêiner. Add () em seu construtor
...
### <a name="delegate-class-class-name-has-no-invoke-method-is-this-class-a-delegate"></a>A classe de\<representante ' Class Name > ' não tem um método Invoke. Essa classe é um delegado?
...
Esse erro ocorre porque uma variável de membro foi declarada duas vezes (por exemplo, dois `Button1` controles nomeados são declarados no código). Os nomes devem ser exclusivos em formulários herdados. Além disso, nomes não podem diferir apenas por maiúsculas e minúsculas.

### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Erro ao ler os recursos do arquivo de recursos para a\<cultura ' nome da cultura > '
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Erro ao ler os recursos do arquivo de recursos para o '\<nome de cultura > ' da cultura padrão
...
Mensagem de erro: "Falha ao analisar o método\<' nome do método > '. O analisador relatou o seguinte erro:\<' Error String > '. Consulte a Lista de Tarefas para obter possíveis erros ".
...
Quando você define uma classe em vários arquivos usando a palavra-chave [partial](/dotnet/csharp/language-reference/keywords/partial-type) , só pode ter uma definição parcial em cada arquivo.
...
Mensagem de erro: "Não foi possível\<encontrar o" nome do assembly > ' do assembly. Verifique se o assembly é referenciado. Se o assembly fizer parte do projeto de desenvolvimento atual, verifique se o projeto foi criado. "

Esse erro é semelhante a "o tipo\<nome do tipo >" não pôde ser encontrado ", mas esse erro geralmente ocorre devido a um atributo de metadados. Para corrigir esse erro, verifique se todos os assemblies usados por atributos são referenciados.
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>Não foi possível carregar\<a classe base ' Class Name > '
...
### <a name="the-designer-for-base-class-class-name-is-not-installed"></a>O designer para a classe base\<' Class Name > ' não está instalado
...
Esse erro ocorreu porque a classe base do objeto que está sendo passado para o designer é [abstrata](/dotnet/csharp/language-reference/keywords/abstract), o que não é permitido.
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>O '\<nome do serviço > ' do serviço é necessário, mas não foi localizado

Mensagem de erro: "O Service Name\<> ' do serviço é necessário, mas não foi localizado. Pode haver um problema com a instalação do Visual Studio. "
...
Mensagem de erro: "Não foi possível recuperar o\<item da caixa de ferramentas para ' nome do componente > ' na caixa de ferramentas. Verifique se o assembly que contém o item da caixa de ferramentas está instalado corretamente. O item da caixa de ferramentas gerou o \<seguinte erro: cadeia de caracteres de erro >. "

O componente em questão emitiu uma exceção quando o Visual Studio o acessou. Contate o fornecedor do componente.

### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Não foi possível recuperar o\<item da caixa de ferramentas ' nome do item de caixa de ferramentas > ' da caixa de ferramentas

Mensagem de erro: "O item da caixa de\<ferramentas para ' nome do item de caixa de ferramentas > ' não pôde ser recuperado da caixa de ferramentas. Tente remover o item da caixa de ferramentas e adicioná-lo de volta. "
...
### <a name="the-type-type-name-could-not-be-found"></a>Não foi possível\<encontrar o tipo ' nome do tipo > '

Mensagem de erro: "Não foi possível\<encontrar o tipo ' nome do tipo > '. Verifique se o assembly que contém o tipo é referenciado. Se o assembly fizer parte do projeto de desenvolvimento atual, verifique se o projeto foi criado. "
...
### <a name="there-is-already-a-component-named-component-name"></a>Já existe um componente chamado '\<nome do componente > '
...
Mensagem de erro: "Não é possível adicionar o\<nome de referência ' reference ' > ' ao aplicativo atual. Verifique se uma versão diferente do '\<nome de referência > ' já não está referenciada. "
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Não é possível localizar a página\<chamada ' nome da guia da caixa de diálogo ' Opções > '
...
### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>O Visual Studio não pode abrir o\<' nome do documento > ' em modo de exibição de design

Mensagem de erro: "O Visual Studio não pode abrir o\<' nome do documento > ' em modo de exibição de design. Nenhum analisador está instalado para o tipo de arquivo. "
