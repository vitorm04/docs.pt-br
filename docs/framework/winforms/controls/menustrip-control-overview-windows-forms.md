...
|<xref:System.Windows.Forms.ToolStripMenuItem.ShowShortcutKeys%2A>|Obtém ou define um valor que indica se as teclas de atalho que estão associados a <xref:System.Windows.Forms.ToolStripMenuItem> são exibidos ao lado de <xref:System.Windows.Forms.ToolStripMenuItem>.|  
