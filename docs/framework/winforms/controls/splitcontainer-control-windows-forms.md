...
> [!NOTE]
> Na **caixa de ferramentas**, esse controle substitui o controle de <xref:System.Windows.Forms.Splitter> que estava lá na versão anterior do Visual Studio. O controle de `SplitContainer` é muito preferível ao controle de <xref:System.Windows.Forms.Splitter>. A classe <xref:System.Windows.Forms.Splitter> ainda está incluída no .NET Framework para compatibilidade com os aplicativos existentes, mas é altamente recomendável que você use o controle de `SplitContainer` para novos projetos.  
