...
O [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] ajuda a detectar quando você está acessando os controles de uma maneira que não é thread-safe. Quando você estiver executando seu aplicativo no depurador e um thread diferente daquele que criou um controle tenta chamar o controle, o depurador gera uma <xref:System.InvalidOperationException> com a mensagem "controle *nome do controle* acessados de um thread diferente do thread que foi criado."
