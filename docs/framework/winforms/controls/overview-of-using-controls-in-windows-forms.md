...
- Uma ou mais classes <xref:System.Windows.Forms.Form?displayProperty=nameWithType>que derivam de .  
  
- Um `Main` método que `static` invoca`shared` o método <xref:System.Windows.Forms.Application.Run%2A> (no <xref:System.Windows.Forms.Form> Visual Basic) e passa uma instância para ele. O <xref:System.Windows.Forms.Application.Run%2A> método processa mensagens do sistema operacional para o aplicativo.  
