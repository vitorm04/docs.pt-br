---
title: Controle FlowLayoutPanel (Windows Forms)
ms.date: 03/30/2017
helpviewer_keywords:
- FlowLayoutPanel control [Windows Forms]
- layout [Windows Forms]
- Windows Forms, arranging contents
ms.assetid: 630c9305-9ecf-4f99-94f9-ddf06de82713
ms.openlocfilehash: c164a77314f7127056e2367e7e9c4fac331d723d
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 05/04/2018
---
# <a name="flowlayoutpanel-control-windows-forms"></a>Controle FlowLayoutPanel (Windows Forms)
O `FlowLayoutPanel` controle organiza seu conteúdo em uma direção de fluxo horizontal ou vertical. Seu conteúdo pode ser encapsulado de uma linha à outra ou de uma coluna à próxima. Como alternativa, seu conteúdo pode ser recortado, em vez de encapsulado.  
  
 Os tópicos nesta seção descrevem os conceitos e técnicas que permitem que você crie `FlowLayoutPanel` recursos em seus aplicativos.  
  
## <a name="in-this-section"></a>Nesta seção  
 [Visão geral do controle FlowLayoutPanel](../../../../docs/framework/winforms/controls/flowlayoutpanel-control-overview.md)  
 Apresenta os conceitos gerais do `FlowLayoutPanel` controle, que permite que você crie um layout de fluxo horizontal ou verticalmente.  
  
 [Como ancorar e encaixar controles filho em um controle FlowLayoutPanel](../../../../docs/framework/winforms/controls/how-to-anchor-and-dock-child-controls-in-a-flowlayoutpanel-control.md)  
 Explica como usar o <xref:System.Windows.Forms.Control.Anchor%2A> e <xref:System.Windows.Forms.Control.Dock%2A> propriedades para ancorar e encaixar controles filho em um <xref:System.Windows.Forms.FlowLayoutPanel> controle.  
  
 Consulte também [Explicação passo a passo: organizando controles nos Windows Forms utilizando um FlowLayoutPanel](http://msdn.microsoft.com/library/z9w7ek2f\(v=vs.110\)).  
  
## <a name="reference"></a>Referência  
 <xref:System.Windows.Forms.FlowLayoutPanel>  
 Fornece documentação de referência para o <xref:System.Windows.Forms.FlowLayoutPanel> controle.
