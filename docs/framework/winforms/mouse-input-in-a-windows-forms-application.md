---
title: Entrada do mouse
ms.date: 03/30/2017
helpviewer_keywords:
- Windows Forms, mouse input
ms.assetid: 743c2f3c-219e-4a52-b6b8-2657096a2da6
ms.openlocfilehash: e0b43fa5f98a23b1cde882154ca1d7ec7542cdc0
ms.sourcegitcommit: de17a7a0a37042f0d4406f5ae5393531caeb25ba
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 01/24/2020
ms.locfileid: "76727905"
---
# <a name="mouse-input-in-a-windows-forms-application"></a>Entrada do mouse em um aplicativo do Windows Forms
O Windows Forms inclui uma variedade de eventos de mouse e suporte adicional para cursores personalizados de mouse, captura de mouse e comportamento de arrastar e soltar.  
  
## <a name="in-this-section"></a>Nesta seção  
 [Como a entrada do mouse funciona nos Windows Forms](how-mouse-input-works-in-windows-forms.md)  
 Fornece informações sobre os eventos do mouse e como obter informações atuais e configurações do sistema para o mouse.  
  
 [Eventos do mouse nos Windows Forms](mouse-events-in-windows-forms.md)  
 Fornece informações sobre a ordem na qual os eventos do mouse ocorrem e como os eventos do mouse são gerados dentro de controles específicos.  
  
 [Como distinguir entre cliques e cliques duplos](how-to-distinguish-between-clicks-and-double-clicks.md)  
 Demonstra como usar cliques únicos e duplos para iniciar ações incompatíveis.  
  
 [Ponteiros do mouse nos Windows Forms](mouse-pointers-in-windows-forms.md)  
 Descreve como alterar o cursor do mouse.  
  
 [Captura do mouse nos Windows Forms](mouse-capture-in-windows-forms.md)  
 Descreve como um controle pode capturar o mouse.  
  
 [Funcionalidade do tipo "arrastar e soltar" nos Windows Forms](drag-and-drop-functionality-in-windows-forms.md)  
 Descreve como implementar o comportamento de arrastar e soltar.  
  
## <a name="related-sections"></a>Seções Relacionadas  
 [Acessando o Mouse](../../visual-basic/developing-apps/programming/computer-resources/accessing-the-mouse.md)  
 Lista os tópicos para acessar o mouse usando Visual Basic.
