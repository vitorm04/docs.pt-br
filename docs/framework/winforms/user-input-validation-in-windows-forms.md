...
- Se o código postal deve ser um CEP válido dos Estados Unidos, você poderia chamar um serviço Web de código postal para validar os dados inseridos pelo usuário.  
 ...
 O <xref:System.Windows.Forms.Control.Validating> evento é fornecido <xref:System.ComponentModel.CancelEventArgs>como um objeto de tipo. Se você determinar que os dados do controle não <xref:System.Windows.Forms.Control.Validating> são válidos, <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> você `true`pode cancelar o evento definindo a propriedade deste objeto como . Se você não <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> definir a propriedade, o Windows Forms assumirá que <xref:System.Windows.Forms.Control.Validated> a validação foi bem sucedida para esse controle e aumentará o evento.  
  
 Para um exemplo de código que <xref:System.Windows.Controls.TextBox>valida <xref:System.Windows.Forms.Control.Validating>um endereço de e-mail em um , veja .  
...
### <a name="data-binding-and-event-driven-validation"></a>Vinculação de dados e validação controlada por evento  
 ...
 Quando você usa vinculação de dados, os dados em seu <xref:System.Windows.Forms.Control.Validating> controle são sincronizados com a fonte de dados durante a execução do evento. Se você <xref:System.Windows.Forms.Control.Validating> cancelar o evento, os dados não serão sincronizados com a fonte de dados.  
  
> [!IMPORTANT]
> Se você tiver validação personalizada <xref:System.Windows.Forms.Control.Validating> que ocorra após o evento, ela não afetará a vinculação de dados. Por exemplo, se você <xref:System.Windows.Forms.Control.Validated> tiver código em um caso que tente cancelar a vinculação de dados, a vinculação de dados ainda ocorrerá. Neste caso, para realizar <xref:System.Windows.Forms.Control.Validated> a validação no evento, altere a propriedade **do Modo** de Atualização de Fonte de Dados do controle **(em (Databindings)**\\ **(Avançado)**) de **OnValidation** para **Never**, e adicione *Control*`.DataBindings["`*\<YOURFIELD>* `"].WriteValue()` ao seu código de validação.  
...
#### <a name="implicit-validation"></a>Validação implícita  
 ...
 Se você quiser usar validação implícita para um controle, <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A> você <xref:System.Windows.Forms.AutoValidate.EnablePreventFocusChange> <xref:System.Windows.Forms.AutoValidate.EnableAllowFocusChange>deve definir a propriedade desse controle para ou . Se você <xref:System.Windows.Forms.Control.Validating> cancelar o evento, o comportamento do controle será determinado <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A>pelo valor a que você atribuiu . Se você <xref:System.Windows.Forms.AutoValidate.EnablePreventFocusChange>atribuiu, cancelar o <xref:System.Windows.Forms.Control.Validated> evento fará com que o evento não ocorra. O foco de entrada permanecerá no controle atual até que o usuário altere os dados para uma entrada válida. Se você <xref:System.Windows.Forms.AutoValidate.EnableAllowFocusChange>atribuído, <xref:System.Windows.Forms.Control.Validated> o evento não ocorrerá quando você cancelar o evento, mas o foco ainda mudará para o próximo controle.  
  
 Atribuir <xref:System.Windows.Forms.AutoValidate.Disable> à <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A> propriedade impede completamente a validação implícita. Para validar seus controles, você precisará usar a validação explícita.  
...
- Ligue <xref:System.Windows.Forms.ContainerControl.Validate%2A> para validar o último controle que perdeu o foco.  
  
- Chamada <xref:System.Windows.Forms.ContainerControl.ValidateChildren%2A> para validar todos os controles de crianças em um formulário ou controle de contêiner.  
...
#### <a name="default-implicit-validation-behavior-for-windows-forms-controls"></a>Comportamento de validação implícita padrão de controles dos Windows Forms  
 Diferentes controles do Windows Forms <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A> têm padrões diferentes para sua propriedade. A tabela a seguir mostra os controles mais comuns e seus valores padrão.  
