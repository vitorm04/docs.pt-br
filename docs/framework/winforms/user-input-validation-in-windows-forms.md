...
- Se o código postal deve ser um CEP válido dos Estados Unidos, você poderia chamar um serviço Web de código postal para validar os dados inseridos pelo usuário.  
 ...
 O evento <xref:System.Windows.Forms.Control.Validating> é fornecido como um objeto do tipo <xref:System.ComponentModel.CancelEventArgs>. Se você determinar que os dados do controle não são válidos, poderá cancelar o evento <xref:System.Windows.Forms.Control.Validating> definindo a propriedade <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> do objeto como `true`. Se você não definir a propriedade <xref:System.ComponentModel.CancelEventArgs.Cancel%2A>, Windows Forms irá pressupor que a validação foi bem-sucedida para esse controle e gerar o evento <xref:System.Windows.Forms.Control.Validated>.  
...
### <a name="data-binding-and-event-driven-validation"></a>Vinculação de dados e validação controlada por evento  
 ...
 Quando você usa a vinculação de dados, os dados em seu controle são sincronizados com a fonte de dados durante a execução do evento de <xref:System.Windows.Forms.Control.Validating>. Se você cancelar o evento <xref:System.Windows.Forms.Control.Validating>, os dados não serão sincronizados com a fonte de dados.  
  
> [!IMPORTANT]
> Se você tiver uma validação personalizada que ocorre após o evento <xref:System.Windows.Forms.Control.Validating>, ela não afetará a associação de dados. Por exemplo, se você tiver um código em um evento <xref:System.Windows.Forms.Control.Validated> que tente cancelar a associação de dados, a vinculação de dados ainda ocorrerá. Nesse caso, para executar a validação no evento <xref:System.Windows.Forms.Control.Validated>, altere a propriedade do **modo de atualização da fonte de dados** do controle (**em (DataBindings)** \\ **(avançado)** ) de **OnValidation** para **Never**e adicione *Control*`.DataBindings["` *\<o* >Field`"].WriteValue()` ao seu código de validação.  
...
#### <a name="implicit-validation"></a>Validação implícita  
 ...
 Se você quiser usar a validação implícita para um controle, deverá definir a propriedade <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A> do controle como <xref:System.Windows.Forms.AutoValidate.EnablePreventFocusChange> ou <xref:System.Windows.Forms.AutoValidate.EnableAllowFocusChange>. Se você cancelar o evento <xref:System.Windows.Forms.Control.Validating>, o comportamento do controle será determinado pelo valor que você atribuiu a <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A>. Se você atribuiu <xref:System.Windows.Forms.AutoValidate.EnablePreventFocusChange>, cancelar o evento fará com que o evento de <xref:System.Windows.Forms.Control.Validated> não ocorra. O foco de entrada permanecerá no controle atual até que o usuário altere os dados para uma entrada válida. Se você atribuiu <xref:System.Windows.Forms.AutoValidate.EnableAllowFocusChange>, o evento <xref:System.Windows.Forms.Control.Validated> não ocorrerá quando você cancelar o evento, mas o foco ainda será alterado para o próximo controle.  
  
 A atribuição de <xref:System.Windows.Forms.AutoValidate.Disable> à propriedade <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A> impede completamente a validação implícita. Para validar seus controles, você precisará usar a validação explícita.  
...
- Chame <xref:System.Windows.Forms.ContainerControl.Validate%2A> para validar o último controle para ter perdido o foco.  
...
#### <a name="default-implicit-validation-behavior-for-windows-forms-controls"></a>Comportamento de validação implícita padrão de controles dos Windows Forms  
 Diferentes Windows Forms controles têm padrões diferentes para sua propriedade <xref:System.Windows.Forms.ContainerControl.AutoValidate%2A>. A tabela a seguir mostra os controles mais comuns e seus valores padrão.  
...
- Chamando o método <xref:System.Windows.Forms.Form.Close%2A> programaticamente.  
  
 No entanto, em alguns casos, você talvez queira permitir que o usuário feche o formulário independentemente se os valores nos controles são válidos. Você pode substituir a validação e fechar um formulário que ainda contém dados inválidos criando um manipulador para o evento de <xref:System.Windows.Forms.Form.FormClosing> do formulário. No evento, defina a propriedade <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> como `false`. Isso força o formulário a fechar. Para obter mais informações e um exemplo, consulte <xref:System.Windows.Forms.Form.FormClosing?displayProperty=nameWithType>.  
...
- [Exemplos de expressões regulares](../../standard/base-types/regular-expression-examples.md)
