...
## <a name="description"></a>Descrição  
 Esse evento é emitido quando padrão do modelo de serviço `OperationInvoker` encontrou uma exceção ao invocar seu método. Observe que as exceções que derivam de `FaultException` causar esse evento não seja emitido.  
  
## <a name="message"></a>Mensagem  
 O método '%1' gerou uma exceção sem tratamento quando chamado pelo OperationInvoker. A duração da chamada de método era '%2' ms.  
