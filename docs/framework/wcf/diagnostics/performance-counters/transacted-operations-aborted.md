---
title: Operações de transação abortadas
ms.date: 03/30/2017
ms.assetid: d0ff844c-a33c-4ecf-8be5-b5b33635914a
ms.openlocfilehash: f227197a5c2d1b96cc200ca598ba54641dc0bcf1
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 05/04/2018
---
# <a name="transacted-operations-aborted"></a><span data-ttu-id="565d8-102">Operações de transação abortadas</span><span class="sxs-lookup"><span data-stu-id="565d8-102">Transacted Operations Aborted</span></span>
<span data-ttu-id="565d8-103">Nome do contador: Operações transacionadas anuladas.</span><span class="sxs-lookup"><span data-stu-id="565d8-103">Counter Name: Transacted Operations Aborted.</span></span>  
  
## <a name="description"></a><span data-ttu-id="565d8-104">Descrição</span><span class="sxs-lookup"><span data-stu-id="565d8-104">Description</span></span>  
 <span data-ttu-id="565d8-105">Número de operações transacionais anulados neste serviço.</span><span class="sxs-lookup"><span data-stu-id="565d8-105">Number of transactional operations that have been aborted in this service.</span></span>
