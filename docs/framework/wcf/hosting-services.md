...
|Serviço de Ativação de Processos do Windows (WAS)|Windows Vista, Windows Server 2008|HTTP<br /><br /> net.tcp,<br /><br /> NET. pipe,<br /><br /> net.msmq|Sim|
 ...
 É importante observar que a execução de um serviço ou de qualquer extensão de um host não confiável compromete a segurança. Além disso, ao abrir um <xref:System.ServiceModel.ServiceHost> em representação, um aplicativo deve garantir que o usuário não seja desconectado, por exemplo, armazenando em cache a <xref:System.Security.Principal.WindowsIdentity> do usuário.
