...
3.  Falhas que são direcionadas a uma única camada da pilha, por exemplo erros como falhas de número de sequência WS-RM.  
 ...
 Categoria de 1. Falhas geralmente são WS-Addressing e falhas de SOAP. A base de `FaultConverter` classe fornecida pelo [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] converte erros correspondentes para mensagens de falha especificada pelo WS-Addressing e SOAP para que você não precisa lidar com conversão dessas exceções por conta própria.  
...
-   Além dos componentes principais, você pode usar o [ferramenta de Visualizador de rastreamento de serviço (SvcTraceViewer.exe)](../../../../docs/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe.md) para exibir e pesquisar [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] rastreamentos. A ferramenta é projetada especificamente para arquivos de rastreamento gerados por [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] e gravadas usando <xref:System.Diagnostics.XmlWriterTraceListener>. A figura a seguir mostra os vários componentes envolvidos no rastreamento.  
