...
Este exemplo demonstra como os pontos de extensibilidade do Windows Communication Foundation (WCF) podem ser usados para permitir dados de mensagem em um formato diferente do esperado pelo WCF. Por padrão, os formatadores do WCF esperam que os `soap:body` parâmetros do método sejam incluídos no elemento. O exemplo mostra como implementar um formatador de operação personalizada que analisa dados de parâmetro de uma cadeia de caracteres de consulta HTTP GET e invoca métodos que usam esses dados.  
  
 O exemplo se baseia na [introdução](../../../../docs/framework/wcf/samples/getting-started-sample.md), que implementa o contrato `ICalculator` de serviço. Ele mostra como as mensagens adicionar, subtrair, multiplicar e dividir podem ser alteradas para usar HTTP GET para solicitações de cliente para servidor e HTTP POST com mensagens POX para respostas de servidor para cliente.  
...
- `IClientMessageFormatter.SerializeRequest` No método no cliente, o formatador cria um URI com o apropriado para endereçar e acrescentar o nome da operação como um sufixo. Esse nome é usado para enviar para a operação apropriada no servidor. Em seguida, ele pega a matriz de objetos de parâmetro e serializa os dados de parâmetro para a cadeia de caracteres de consulta de URI usando nomes <xref:System.ComponentModel.TypeConverter> de parâmetro e os valores convertidos pela classe. As <xref:System.ServiceModel.Channels.MessageHeaders.To%2A> propriedades <xref:System.ServiceModel.Channels.MessageProperties.Via%2A> e são então definidas para esse URI. <xref:System.ServiceModel.Channels.MessageProperties>é acessado <xref:System.ServiceModel.Channels.Message.Properties%2A> por meio da propriedade.  
  
- No método no servidor, o formatador recupera o `Via` URI nas propriedades da mensagem de solicitação de entrada. `IDispatchMessageFormatter.DeserializeRequest` Ele analisa os pares de nome-valor na cadeia de caracteres de consulta de URI em valores e nomes de parâmetros e usa os nomes e valores de parâmetro para preencher a matriz de parâmetros passada para o método. Observe que a expedição da operação já ocorreu, portanto, o sufixo do nome da operação é ignorado nesse método.  
...
### <a name="installing-operation-formatters"></a>Instalando formatadores de operação  
 ...
 Isso deve ser feito antes de `CreateChannel`chamar.  
...
- Isso deve ser feito antes <xref:System.ServiceModel.Channels.CommunicationObject.Open%2A> de o ser chamado. Neste exemplo, mostramos como o formatador é modificado manualmente antes de <xref:System.ServiceModel.Channels.CommunicationObject.Open%2A>chamar. Outra maneira de obter a mesma coisa é derivar uma classe de <xref:System.ServiceModel.ServiceHost> que faça as `EnableHttpGetRequestsBehavior.ReplaceFormatterBehavior` chamadas antes de abrir (consulte a documentação e os exemplos de hospedagem para obter exemplos).  
  
### <a name="user-experience"></a>Experiência do usuário  
 No servidor:  
  
- A implementação `ICalculator` do servidor não precisa ser alterada.  
  
- O app. config do serviço deve usar uma associação Pox personalizada que define o `messageVersion` atributo `textMessageEncoding` do elemento como `None`.  
...
- Adicione os formatadores de operação <xref:System.ServiceModel.Channels.CommunicationObject.Open%2A>antes de chamar.  
  
 No cliente:  
  
- A implementação do cliente não precisa ser alterada.  
  
- O app. config do cliente deve usar uma associação Pox personalizada que define o `messageVersion` atributo `textMessageEncoding` do elemento como `None`. Uma diferença do serviço é que o cliente deve habilitar o endereçamento manual para que o endereço de saída possa ser modificado.  
...
- Adicione os formatadores de operação <xref:System.ServiceModel.ChannelFactory%601.CreateChannel>antes de chamar.  
