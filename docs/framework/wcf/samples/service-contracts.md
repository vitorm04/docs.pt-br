---
title: Contratos de serviço
ms.date: 03/30/2017
ms.assetid: 9e34b694-aa56-45b3-8dd1-2616705b4fc5
ms.openlocfilehash: 3d6172b4f7976a93898a29058ba0445616dbd8f5
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33500913"
---
# <a name="service-contracts"></a>Contratos de serviço
Esta seção contém exemplos que demonstram os contratos de serviço no Windows Communication Foundation (WCF).  
  
## <a name="in-this-section"></a>Nesta seção  
 [Duplex](../../../../docs/framework/wcf/samples/duplex.md)  
 Demonstra como definir e implementar um contrato duplex.  
  
 [Contrato de falha](../../../../docs/framework/wcf/samples/fault-contract.md)  
 Demonstra como comunicar informações de erro de um serviço para um cliente.  
  
 [Unidirecional](../../../../docs/framework/wcf/samples/one-way.md)  
 Demonstra um contato de serviço com operações de serviço unidirecional.  
  
 [Sessão](../../../../docs/framework/wcf/samples/session.md)  
 Demonstra como implementar um contrato que requer uma sessão.  
  
 [Fluxo](../../../../docs/framework/wcf/samples/stream.md)  
 Demonstra o uso de fluxo de comunicação de modo de transferência.  
  
 [Assíncrono](http://msdn.microsoft.com/library/833db946-f511-4f64-a26f-2759a11217c7)  
 Demonstra como um cliente pode acessar uma operação de serviço assíncrona e como um serviço pode implementar suas operações de forma assíncrona.  
  
 [Falhas de XmlSerializer](../../../../docs/framework/wcf/samples/xmlserializer-faults.md)  
 Demonstra como comunicar informações de erro de um serviço para um cliente usando o <xref:System.Xml.Serialization.XmlSerializer>.
