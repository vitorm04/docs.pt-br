...
Este exemplo demonstra como um aplicativo cliente pode criar um canal com a classe <xref:System.ServiceModel.ChannelFactory> em vez de um cliente gerado. Este exemplo é baseado no [introdução](../../../../docs/framework/wcf/samples/getting-started-sample.md) que implementa um serviço de calculadora.

> [!NOTE]
> O procedimento de instalação e as instruções de Build para este exemplo estão localizados no final deste tópico.

Este exemplo usa a classe <xref:System.ServiceModel.ChannelFactory%601> para criar um canal para um ponto de extremidade de serviço. Normalmente, para criar um canal para um ponto de extremidade de serviço, você gera um tipo de cliente com a [ferramenta de utilitário de metadados ServiceModel (svcutil. exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) e cria uma instância do tipo gerado. Você também pode criar um canal usando a classe <xref:System.ServiceModel.ChannelFactory%601>, conforme demonstrado neste exemplo. O serviço criado pelo código de exemplo a seguir é idêntico ao serviço no [introdução](../../../../docs/framework/wcf/samples/getting-started-sample.md).
...
Para fechar o canal, primeiro ele deve ser convertido em uma interface <xref:System.ServiceModel.IClientChannel>. Isso ocorre porque o canal como gerado é declarado no aplicativo cliente usando a interface `ICalculator`, que tem métodos como `Add` e `Subtract`, mas não `Close`. O método `Close` se origina na interface <xref:System.ServiceModel.ICommunicationObject>.
