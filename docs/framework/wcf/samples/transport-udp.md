...
4. Adicione um [ \<](../../configure-apps/file-schema/wcf/bindings.md) elemento de>de associação que adiciona o transporte personalizado a uma pilha de canais. Para obter mais informações, consulte [adicionando um elemento de associação](#AddingABindingElement).  
...
- Abrindo: a transição de objetos para esse <xref:System.ServiceModel.ICommunicationObject.Open%2A> estado quando é chamado. Neste ponto, as propriedades se tornam imutáveis e a entrada/saída pode começar. Essa transição é válida somente a partir do estado criado.  
...
- Fechamento: a transição de objetos para esse <xref:System.ServiceModel.ICommunicationObject.Close%2A> estado quando é chamado para um desligamento normal. Essa transição é válida somente a partir do estado aberto.  
...
## <a name="adding-a-binding-element"></a>Adicionando um elemento de associação  
 Agora que as fábricas e os canais são criados, devemos expô-los ao tempo de execução de ServiceModel por meio de uma associação. Uma associação é uma coleção de elementos de associação que representa a pilha de comunicação associada a um endereço de serviço. Cada elemento na pilha é representado por um [ \<](../../configure-apps/file-schema/wcf/bindings.md) elemento de>de associação.  
...
#### <a name="wsdl-export"></a>Exportação de WSDL  
 Para exportar informações de endereçamento `UdpTransportBindingElement` , `IWsdlExportExtension` o implementa a interface. O `ExportEndpoint` método adiciona as informações de endereçamento corretas à porta WSDL.  
...
### <a name="adding-a-custom-standard-binding-importer"></a>Adicionando um importador de associação padrão personalizado  
 ...
 Um importador de associação padrão personalizado implementa `ImportEndpoint` o método na `IWsdlImportExtension` interface para examinar a `CustomBinding` instância importada de metadados para ver se ela pode ter sido gerada por uma associação padrão específica.  
