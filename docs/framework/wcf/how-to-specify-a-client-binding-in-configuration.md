...
4.  Svcutil.exe também gera a configuração do cliente que usa o <xref:System.ServiceModel.BasicHttpBinding> classe. Ao usar [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)], nomeie esse arquivo App. config. Observe que o endereço e as informações de associação não estão especificados em qualquer lugar dentro da implementação do serviço. Além disso, o código não tem a ser gravado para recuperar informações do arquivo de configuração.  
