...
1. Em seu contrato de serviço, declare um par de métodos assíncronos de acordo com as diretrizes de design assíncrono do .NET. O método `Begin` usa um parâmetro, um objeto de retorno de chamada e um objeto de estado e retorna um <xref:System.IAsyncResult?displayProperty=nameWithType> e um método `End` correspondente que usa um <xref:System.IAsyncResult?displayProperty=nameWithType> e retorna o valor de retorno. Para obter mais informações sobre chamadas assíncronas, consulte [padrões de design de programação assíncrona](../../standard/asynchronous-programming-patterns/event-based-asynchronous-pattern-eap.md).  
