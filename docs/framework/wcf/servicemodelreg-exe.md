...
## <a name="fixing-the-fileloadexception-error"></a>Corrigindo o erro fileuploadexception  
 Se você instalou versões anteriores do WCF em seu computador, poderá obter um erro de `FileLoadFoundException` ao executar a ferramenta ServiceModelReg para registrar uma nova instalação. Isso pode acontecer mesmo que você tenha removido manualmente os arquivos da instalação anterior, mas deixou as configurações de Machine. config intactas.  
