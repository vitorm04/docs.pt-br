...
|Nenhuma transação|Permitidos|N/D|Processo|  
...
## <a name="enabling-transaction-flow-at-the-endpoint-level"></a>Habilitando o fluxo de transações no nível do ponto de extremidade  
 ...
 A maioria das ligações que o WCF fornece contém os `transactionFlow` atributos `transactionProtocol` e para permitir que você configure a associação específica para aceitar as transações de entrada. Para obter mais informações sobre como definir os elementos de configuração, consulte [ \<Binding >](../../../../docs/framework/misc/binding.md).  
