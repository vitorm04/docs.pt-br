...
|Metadados|Mecanismo WS-PolicyAttachment|[Mecanismo WS-PolicyAttachment](http://go.microsoft.com/fwlink/?LinkId=95328)<br /><br /> [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]implementa um mecanismo WS-PolicyAttachment para anexar a expressões de política em vários escopos no WSDL Web Services Description Language ().|  
...
|Transações|Coordenação WS|[Coordenação WS](http://go.microsoft.com/fwlink/?LinkId=95324)<br /><br /> Usar o fluxo do contexto de transação quando o `flowTransactions` atributo é definido como "Permitido" ou "Necessário".<br /><br /> `<wsHttpBinding>   <binding transactionFlow="true"/> </wsHttpBinding>`|  
