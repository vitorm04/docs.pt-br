...
## <a name="service-discovery"></a>Descoberta de serviço  
 Um aplicativo cliente pode usar a <xref:System.ServiceModel.Discovery.DiscoveryClient> classe para localizar serviços. O desenvolvedor cria uma instância da <xref:System.ServiceModel.Discovery.DiscoveryClient> classe que passa em um ponto de extremidade de descoberta que especifica onde enviar `Resolve` `Probe` ou mensagens. Em seguida, o <xref:System.ServiceModel.Discovery.DiscoveryClient.Find%2A> cliente chama que especifica os critérios <xref:System.ServiceModel.Discovery.FindCriteria> de pesquisa dentro de uma instância. Se os serviços correspondentes forem encontrados <xref:System.ServiceModel.Discovery.DiscoveryClient.Find%2A> , o retornará uma <xref:System.ServiceModel.Discovery.EndpointDiscoveryMetadata>coleção de. O código a seguir mostra como chamar o `Find` método e, em seguida, conectar-se a um serviço descoberto.  
