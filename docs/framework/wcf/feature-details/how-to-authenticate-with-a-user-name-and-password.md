...
1.  Criar uma instância do <xref:System.ServiceModel.WSHttpBinding>, defina o modo de segurança da associação com `SecurityMode.Message`, defina o `ClientCredentialType` da associação com `MessageCredentialType.UserName`e adicione um ponto de extremidade de serviço usando a associação configurada para o host de serviço, conforme mostrado no código a seguir:  
