...
Uma *coleção* é uma lista de itens de um determinado tipo. No .NET Framework, essas listas podem ser representadas usando matrizes ou uma variedade de outros tipos (lista genérica, <xref:System.ComponentModel.BindingList%601>genérica <xref:System.Collections.Specialized.StringCollection>, ou <xref:System.Collections.ArrayList>). Por exemplo, uma coleção pode conter uma lista de endereços para um determinado cliente. Essas coleções são chamadas de *coleções de listas*, independentemente do seu tipo real.
...
Os tipos contidos em coleções devem ser tipos de contrato de dados ou ser serializáveis de outra forma. Para obter mais informações, consulte [tipos com suporte no serializador de contrato de dados](../../../../docs/framework/wcf/feature-details/types-supported-by-the-data-contract-serializer.md).
...
Semelhante às coleções de lista, todas as coleções de dicionário que têm os mesmos tipos de chave e valor são consideradas como tendo o mesmo contrato de dados <xref:System.Runtime.Serialization.CollectionDataContractAttribute> (a menos que sejam personalizadas pelo atributo).
...
As coleções não genéricas são consideradas como tendo o mesmo contrato de dados que as coleções genéricas `Object`do tipo. (Por exemplo, os contratos <xref:System.Collections.ArrayList> de dados e `Object` genéricos <xref:System.Collections.Generic.List%601> são os mesmos.)
...
Você pode personalizar os tipos de coleção usando <xref:System.Runtime.Serialization.CollectionDataContractAttribute> o atributo, que tem vários usos.
...
As regras para os tipos de coleção de nomenclatura são semelhantes às de nomenclatura de tipos de contrato de dados regulares, conforme descrito em [nomes de contrato de dados](../../../../docs/framework/wcf/feature-details/data-contract-names.md), embora existam algumas diferenças importantes:

- O <xref:System.Runtime.Serialization.CollectionDataContractAttribute> atributo é usado para personalizar o nome, em vez <xref:System.Runtime.Serialization.DataContractAttribute> do atributo. O <xref:System.Runtime.Serialization.CollectionDataContractAttribute> atributo também tem `Name` propriedades `Namespace` e.
...
- Você pode usar as `Name` propriedades `Namespace` e para personalizar ainda mais a nomenclatura. Consulte a classe a seguir.
...
`CustomerList` Nos exemplos, as coleções continham cadeias de caracteres. O nome do contrato de dados para o tipo primitivo da cadeia de caracteres é "String", portanto\<, o elemento repetitivo era "String >".

No entanto, <xref:System.Runtime.Serialization.CollectionDataContractAttribute.ItemName%2A> usando a propriedade <xref:System.Runtime.Serialization.CollectionDataContractAttribute> no atributo, esse nome de elemento repetitivo pode ser personalizado. Para obter um exemplo, consulte o seguinte tipo.
...
Não é necessário adicionar tipos de coleção a tipos conhecidos quando usados polimorficamente no lugar de outras coleções ou interfaces de coleção. Por exemplo, se você declarar um membro de dados do <xref:System.Collections.IEnumerable> tipo e usá-lo para enviar uma <xref:System.Collections.ArrayList>instância do, não será necessário adicionar <xref:System.Collections.ArrayList> a tipos conhecidos.

Quando você usa coleções polimorficamente em vez de tipos que não são de coleção, elas devem ser adicionadas a tipos conhecidos. Por exemplo, se você declarar um membro de dados do `Object` tipo e usá-lo para enviar uma <xref:System.Collections.ArrayList>instância do <xref:System.Collections.ArrayList> , adicione a tipos conhecidos.
...
Tipos conhecidos também podem ser necessários para o conteúdo de coleções. Por exemplo, se uma <xref:System.Collections.ArrayList> realmente contiver instâncias `Type1` de `Type2`e, ambos os tipos devem ser adicionados a tipos conhecidos.

O exemplo a seguir mostra um grafo de objeto construído corretamente usando coleções e tipos conhecidos. O exemplo é um pouco forçado, porque em um aplicativo real normalmente você não definiria os seguintes membros de `Object`dados como e, portanto, não tem nenhum problema de tipo/polimorfismo conhecido.
...
Quando a <xref:System.Runtime.Serialization.NetDataContractSerializer> classe está em uso, os tipos de coleção não personalizados (sem <xref:System.Runtime.Serialization.CollectionDataContractAttribute> o atributo) que não são matrizes perdem seu significado especial.

Os tipos de coleção não personalizados marcados com <xref:System.SerializableAttribute> o atributo ainda podem ser serializados <xref:System.Runtime.Serialization.NetDataContractSerializer> pela classe de acordo <xref:System.SerializableAttribute> com o atributo <xref:System.Runtime.Serialization.ISerializable> ou as regras de interface.

Tipos de coleção personalizados, interfaces de coleção e matrizes ainda são tratados como coleções, mesmo <xref:System.Runtime.Serialization.NetDataContractSerializer> quando a classe está em uso.
...
No entanto, para coleções personalizadas, tipos separados são gerados, marcados <xref:System.Runtime.Serialization.CollectionDataContractAttribute> com o atributo. (Um tipo de coleção personalizado no esquema é aquele que não usa o namespace padrão, o nome, o nome do elemento repetitivo ou os nomes de elemento de chave/valor.) Esses tipos são tipos vazios que derivam de <xref:System.Collections.Generic.List%601> genérico para tipos de lista e dicionário genérico para tipos de dicionário.
...
Talvez você queira usar tipos diferentes no código gerado do que os padrões. Por exemplo, talvez você queira usar matrizes <xref:System.ComponentModel.BindingList%601> genéricas em vez de regulares para seus membros de dados para facilitar sua vinculação a componentes de interface do usuário.

Para escolher os <xref:System.Runtime.Serialization.ImportOptions.ReferencedCollectionTypes%2A> tipos de coleção a serem gerados, passe uma lista de tipos de coleção que você deseja usar na <xref:System.Runtime.Serialization.ImportOptions> Propriedade do objeto ao importar o esquema. Esses tipos são chamados de *tipos de coleção referenciados*.
...
Você pode especificar tipos de interface de coleção como parte dos tipos de coleção referenciados, mas não pode especificar tipos de coleção inválidos `Add` (como aqueles sem nenhum método ou construtor público).
...
- Se o tipo implementar genérico <xref:System.Collections.Generic.IEnumerable%601> (ou suas interfaces derivadas) várias vezes para tipos diferentes, o tipo não será considerado um tipo de coleção referenciado válido e será ignorado. Isso é verdadeiro mesmo que algumas implementações sejam inválidas ou usem genéricos abertos. Por exemplo, um tipo que implementa generic <xref:System.Collections.Generic.IEnumerable%601> de `int` T e <xref:System.Collections.Generic.IEnumerable%601> genérico nunca seria usado como uma coleção referenciada de `int` ou qualquer outro tipo, independentemente de o tipo ter um `Add` método aceitando `int` ou um`Add` método que aceita um parâmetro do tipo T, ou ambos.

- Se o tipo implementar uma interface de coleção genérica, bem <xref:System.Collections.IList>como, o tipo nunca será usado como um tipo de coleção referenciado, a menos que a interface de coleção <xref:System.Object>genérica seja um genérico fechado do tipo.
...
- Matrizes de bytes e matrizes são tipos de <xref:System.Xml.XmlNode> matriz especiais que são tratados como primitivos, não coleções. A serialização de uma matriz de resultados de bytes em um único elemento XML que contém um bloco de dados codificados em base64, em vez de um elemento separado para cada byte. Para obter mais informações sobre como uma matriz <xref:System.Xml.XmlNode> de é tratada, consulte [tipos XML e ADO.net em contratos de dados](../../../../docs/framework/wcf/feature-details/xml-and-ado-net-types-in-data-contracts.md). É claro que esses tipos especiais podem participar de coleções: uma matriz de resultados de bytes em vários elementos XML, com cada um contendo uma parte de dados codificados em base64.
...
- Se um tipo de coleção implementa <xref:System.Xml.Serialization.IXmlSerializable> a interface, as regras a seguir se aplicam `myType:IList<string>, IXmlSerializable`, dado um tipo:
...
|<xref:System.Collections.Generic.IList%601>genérico|Indexador genérico <xref:System.Collections.Generic.IList%601>|Adição genérica|
...
A tabela anterior lista as interfaces de coleção em ordem decrescente de precedência. Isso significa, por exemplo, que se um tipo implementa <xref:System.Collections.IList> e genérico <xref:System.Collections.Generic.IEnumerable%601>, a coleção é serializada e desserializada de acordo com <xref:System.Collections.IList> as regras:

- Na desserialização, todas as coleções são desserializadas primeiro criando uma instância do tipo chamando o construtor sem parâmetros, que deve estar presente para que o serializador trate um tipo de coleção como uma coleção durante a serialização e desserialização.

- Se a mesma interface de coleção genérica for implementada mais de uma vez (por exemplo, se um tipo <xref:System.Collections.Generic.ICollection%601> implementar `Integer` Generic of <xref:System.Collections.Generic.ICollection%601> e <xref:System.String>Generic of) e nenhuma interface de precedência mais alta for encontrada, a coleção será Não tratado como uma coleção válida.

- Os tipos de coleção podem <xref:System.SerializableAttribute> ter o atributo aplicado a eles e podem <xref:System.Runtime.Serialization.ISerializable> implementar a interface. Ambos são ignorados. No entanto, se o tipo não atender totalmente aos requisitos de tipo de coleção ( `Add` por exemplo, se o método estiver ausente), o tipo não será considerado um tipo <xref:System.SerializableAttribute> de coleção e <xref:System.Runtime.Serialization.ISerializable> , portanto, o atributo e a interface serão usados para determinar Se o tipo pode ser serializado.

- Aplicar o <xref:System.Runtime.Serialization.CollectionDataContractAttribute> atributo a uma coleção para personalizá-lo remove <xref:System.SerializableAttribute> o mecanismo de fallback anterior. Em vez disso, se uma coleção personalizada não atender aos requisitos de tipo <xref:System.Runtime.Serialization.InvalidDataContractException> de coleção, uma exceção será lançada. A cadeia de caracteres de exceção geralmente contém informações que explicam por que um determinado tipo não é considerado `Add` uma coleção válida (nenhum método, nenhum construtor sem parâmetros e assim por diante), portanto, geralmente <xref:System.Runtime.Serialization.CollectionDataContractAttribute> é útil aplicar o atributo para depuração metas.

### <a name="collection-naming"></a>Nomenclatura de coleção

Veja a seguir uma lista de regras de nomenclatura de coleção:

- O namespace padrão para todos os contratos de dados de coleção de dicionário, bem como para os contratos de dados de coleção de `http://schemas.microsoft.com/2003/10/Serialization/Arrays` lista que contêm tipos primitivos, é a menos que seja substituído usando o namespace. Tipos que são mapeados para tipos XSD internos, bem como `char` `Timespan`tipos, e `Guid` , são considerados primitivos para essa finalidade.
...
- O nome padrão para contratos de dados de coleção de lista, a menos que seja substituído usando Name, é a cadeia de caracteres "ArrayOf" combinada com o nome do contrato de dados do tipo contido na coleção. Por exemplo, o nome do contrato de dados para uma lista genérica de inteiros é "ArrayOfint". Tenha em mente que o nome do contrato de `Object` dados é "anyType", portanto, o nome do contrato de dados de listas <xref:System.Collections.ArrayList> não genéricas como é "ArrayOfanyType".

O nome padrão para contratos de dados de coleção de dicionário, `Name`a menos que substituído usando, é a cadeia de caracteres "ArrayOfKeyValueOf" combinada com o nome do contrato de dados do tipo de chave seguido pelo nome do contrato de dados do tipo de valor. Por exemplo, o nome do contrato de dados para um dicionário genérico de cadeia de caracteres e inteiro é "ArrayOfKeyValueOfstringint". Além disso, se os tipos de chave ou de valor não forem tipos primitivos, um hash de namespace dos namespaces de contrato de dados dos tipos de chave e valor será anexado ao nome. Para obter mais informações sobre hashes de namespace, consulte [nomes de contrato de dados](../../../../docs/framework/wcf/feature-details/data-contract-names.md).
...
Regras de nomenclatura de tipo genérico, conforme descrito em [nomes de contrato de dados](../../../../docs/framework/wcf/feature-details/data-contract-names.md), se aplicam totalmente aos tipos de coleção; ou seja, você pode usar chaves dentro do nome para indicar parâmetros de tipo genérico. No entanto, os números dentro das chaves referem-se aos parâmetros genéricos e não aos tipos contidos na coleção.
...
- Aplicando <xref:System.Runtime.Serialization.DataContractAttribute> o atributo a um tipo ao qual <xref:System.Runtime.Serialization.CollectionDataContractAttribute> o atributo foi aplicado ou a um de seus tipos derivados.

- Aplicando <xref:System.Runtime.Serialization.CollectionDataContractAttribute> o atributo a um tipo que implementa <xref:System.Xml.Serialization.IXmlSerializable> a interface.

- Aplicando <xref:System.Runtime.Serialization.CollectionDataContractAttribute> o atributo a um tipo que não é de coleção.
...
Nesse caso, uma instância do `Marks1` pode ser atribuída a. `testMarks` No entanto, `Marks2` não deve ser usado porque seu contrato de dados não é considerado `IList<int>` equivalente ao contrato de dados. O nome do contrato de dados é "Marks2" e não "ArrayOfint", e o nome do elemento\<repetitivo é "Mark >\<" e não "int >".
...
Uma matriz de um tipo derivado pode ser atribuída a uma matriz de um tipo base. Nesse caso, o nome do contrato para o tipo derivado é serializado para cada elemento repetitivo. Por exemplo, se um tipo `Book` derivar do tipo `LibraryItem`, você poderá atribuir uma matriz de `Book` a uma matriz de `LibraryItem`. Isso não se aplica a outros tipos de coleção. Por exemplo, você não pode atribuir `Generic List of Book` um a `Generic List of LibraryItem`um. No entanto, você pode atribuir `Generic List of LibraryItem` um que `Book` contém instâncias. Na matriz e no caso não matriz, `Book` o deve estar na lista de tipos conhecidos.
