...
O filtro AND não filtra diretamente um valor dentro de uma mensagem, mas permite que você combine dois outros filtros para criar uma condição de `AND` em que ambos os filtros devem corresponder à mensagem antes que o filtro AND seja avaliado como `true`. Isso permite que você crie filtros complexos que só correspondem se todos os subfiltros corresponderem. O exemplo a seguir define um filtro de endereço e um filtro de ação e, em seguida, define um filtro AND que avalia uma mensagem em relação aos filtros de endereço e de ação. Se os filtros de endereço e de ação forem correspondentes, o filtro e retornará `true`.
