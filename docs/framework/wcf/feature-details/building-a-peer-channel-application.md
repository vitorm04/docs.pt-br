---
title: Compilando um aplicativo de canal par
ms.date: 03/30/2017
ms.assetid: a69a1b8f-8e15-45c5-87b8-bdb71e1c7ab8
ms.openlocfilehash: b6f856eed1aa011bd21bcf01172e9322df70925b
ms.sourcegitcommit: 5b6d778ebb269ee6684fb57ad69a8c28b06235b9
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 04/08/2019
ms.locfileid: "59108531"
---
# <a name="building-a-peer-channel-application"></a>Compilando um aplicativo de canal par
As seções a seguir fornecem orientação sobre como desenvolver aplicativos de canal par.  
  
## <a name="in-this-section"></a>Nesta seção  
 [Convertendo um aplicativo NetTcpBinding em um aplicativo de canal par](../../../../docs/framework/wcf/feature-details/converting-a-nettcpbinding-application-to-a-peer-channel-application.md)  
 Descreve quente para converter um aplicativo NetTcpBinding em um aplicativo de canal par.  
  
 [Limitando a distribuição de mensagens](../../../../docs/framework/wcf/feature-details/limiting-message-distribution.md)  
 Discute o uso de contagem de salto, um filtro de propagação de mensagem, um filtro local ou uma conexão direta para controlar como as mensagens são encaminhadas em toda a malha e fornece diretrizes gerais para escolher uma abordagem.  
  
 [Adicionando status online e offline](../../../../docs/framework/wcf/feature-details/adding-online-and-offline-status.md)  
 Descreve como usar o <xref:System.ServiceModel.IOnlineStatus> interface para monitorar os detalhes específicos sobre o status online e offline de uma conexão de canal par.  
  
 [Protegendo aplicativos de canal par](../../../../docs/framework/wcf/feature-details/securing-peer-channel-applications.md)  
 Discute baseados em mensagem e transporte de segurança e práticas recomendadas para proteger aplicativos de canal par.  
  
## <a name="see-also"></a>Consulte também

- [Conceitos de canal par](../../../../docs/framework/wcf/feature-details/peer-channel-concepts.md)
