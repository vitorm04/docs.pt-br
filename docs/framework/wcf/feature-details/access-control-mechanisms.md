...
## <a name="principalpermissionattribute"></a>PrincipalPermissionAttribute  
 O <xref:System.Security.Permissions.PrincipalPermissionAttribute> é usado para restringir o acesso a um método de serviço. Quando o atributo é aplicado a um método, ele pode ser usado para solicitar a identidade do chamador um específico ou associação em um grupo do Windows ou a função do ASP.NET. Se o cliente é autenticado usando um certificado X.509, ele recebe uma identidade primária que consiste o nome da entidade e a impressão digital do certificado.  
...
## <a name="aspnet-membership-provider"></a>Provedor de associação do ASP.NET  
 Um recurso do ASP.NET é o provedor de associação. Mesmo que o provedor de associação não seja tecnicamente um mecanismo de controle de acesso, ele permite controlar o acesso ao serviço, limitando o conjunto de identidades possíveis que podem acessar o ponto de extremidade do serviço. O recurso de associação inclui um banco de dados que pode ser populado com combinações de nome/senha de usuário que permitem aos usuários de um site da Web estabelecer contas com o site. Para acessar um serviço que usa o provedor de associação, um usuário faça logon com seu nome de usuário e senha.  
  
> [!NOTE]
>  Primeiro, você deve preencher o banco de dados usando o recurso do ASP.NET para que um serviço WCF possa ser usado para fins de autorização.  
  
 Você também pode usar o recurso de associação, se você já tiver um banco de dados de associação de um site da Web ASP.NET existente e você deseja habilitar os mesmos que os usuários usem seu serviço, autorizado com os mesmos nomes de usuário e senhas.  
...
## <a name="aspnet-role-provider"></a>Provedor de função ASP.NET  
 Outro recurso do ASP.NET é a capacidade de gerenciar a autorização usando funções. O provedor de função do ASP.NET permite que um desenvolvedor para criar funções para usuários e atribuir a cada usuário a uma função ou funções. Assim como acontece com o provedor de associação, as funções e atribuições são armazenadas em um banco de dados e podem ser preenchidas usando as ferramentas fornecidas por uma implementação específica do provedor de função do ASP.NET. Como com o recurso de associação, os desenvolvedores do WCF podem usar as informações no banco de dados para autorizar os usuários do serviço pelas funções. Eles podem, por exemplo, usar o provedor de função em combinação com o `PrincipalPermissionAttribute` descrito acima de mecanismo de controle de acesso.  
  
 Se você tiver um banco de dados de provedor de função ASP.NET existente e quiser usar o mesmo conjunto de regras e as atribuições de usuário em seu serviço WCF, você também pode usar o provedor de função do ASP.NET.  
...
## <a name="authorization-manager"></a>Gerenciador de autorização  
 Outro recurso combina o Gerenciador de autorização (AzMan) com o provedor de função ASP.NET para autorizar os clientes. Quando o ASP.NET hospeda um serviço Web, AzMan pode ser integrado ao aplicativo para que a autorização para o serviço é feita por meio de AzMan. Gerenciador de funções do ASP.NET fornece uma API que permite que você gerencie as funções de aplicativo, adicionar e remover usuários das funções e verificar a associação de função, mas não permite a consulta se um usuário está autorizado a executar uma tarefa nomeada ou a operação. AzMan permite que você defina operações individuais e combiná-los em tarefas. Com o AZMan, além das verificações de função, você também pode verificar se um usuário pode executar uma tarefa. Autorização de tarefa e atribuição de função pode ser configurada fora do aplicativo ou podem ser executada de forma programática dentro do aplicativo. A administração de AzMan snap-in do Console de gerenciamento Microsoft (MMC) permite aos administradores alterar as tarefas de que uma função pode executar no tempo de execução e para gerenciar a associação de cada usuário de funções.  
  
 Você também pode usar o provedor de função do ASP.NET e AzMan se você já tiver acesso a uma instalação existente do AzMan e deseja autorizar seus usuários de serviço usando os recursos da combinação de provedor de função/AzMan.  
  
 Para obter mais informações sobre o AzMan e o provedor de função do ASP.NET, consulte [How To: Usar Gerenciador de autorização (AzMan) com o ASP.NET 2.0](https://go.microsoft.com/fwlink/?LinkId=88951). Para obter mais informações sobre como usar o provedor de função e AzMan para os serviços WCF, consulte [como: Usar o provedor de função do Gerenciador de autorização do ASP.NET com um serviço](../../../../docs/framework/wcf/feature-details/how-to-use-the-aspnet-authorization-manager-role-provider-with-a-service.md).  
