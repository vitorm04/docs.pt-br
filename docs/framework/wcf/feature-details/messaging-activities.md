---
title: Atividades de mensagem - WCF
ms.date: 03/30/2017
...
Ao chamar um serviço de fluxo de trabalho de um aplicativo de fluxo de trabalho, o Visual Studio 2012 gera atividades de mensagem personalizadas que encapsulam o usual <xref:System.ServiceModel.Activities.Send> e <xref:System.ServiceModel.Activities.ReceiveReply> atividades usadas em uma MEP de solicitação/resposta. Para usar esse recurso, clique com botão direito no projeto cliente no Visual Studio e selecione **Add** > **referência de serviço**. Digite o endereço base do serviço na caixa de endereço e clique em Ir. Os serviços disponíveis são exibidos na **Services:** caixa. Expanda o nó de serviço para exibir os contratos de suporte. Selecione o contrato que você deseja chamar e a lista de operações disponíveis é exibida na **operações** caixa. Você pode, em seguida, especifique o namespace para a atividade gerada e clique em **Okey**. Você verá uma caixa de diálogo que diz que a operação foi concluída com êxito e que as atividades personalizados geradas estão na caixa de ferramentas depois que você tiver recriado o projeto. Há uma atividade para cada operação definida no contrato de serviço. Depois de recompilar o projeto de arrastar e soltar atividades personalizadas para seu fluxo de trabalho e defina as propriedades na janela Propriedades.

## <a name="messaging-activity-templates"></a>Modelos de atividade do sistema de mensagens

Para facilitar a configuração de uma solicitação/resposta MEP no cliente e o serviço, o Visual Studio 2012 fornece dois modelos de atividade de mensagens. `System.ServiceModel.Activities.Design.ReceiveAndSendReply` é usado no serviço e `System.ServiceModel.Activities.Design.SendAndReceiveReply` é usada no cliente. Em ambos os casos os modelos de adicionam as atividades de mensagem apropriadas para seu fluxo de trabalho. No serviço, o `System.ServiceModel.Activities.Design.ReceiveAndSendReply` adiciona um <xref:System.ServiceModel.Activities.Receive> atividade seguido por um <xref:System.ServiceModel.Activities.SendReply> atividade. O <xref:System.ServiceModel.Activities.SendReply.Request> propriedade é definida automaticamente como o <xref:System.ServiceModel.Activities.Receive> atividade. No cliente, o `System.ServiceModel.Activities.Design.SendAndReceiveReply` adiciona um <xref:System.ServiceModel.Activities.Send> atividade seguido por um <xref:System.ServiceModel.Activities.ReceiveReply>. O <xref:System.ServiceModel.Activities.ReceiveReply.Request%2A> propriedade é definida automaticamente como o <xref:System.ServiceModel.Activities.Send> atividade. Para usar esses modelos, simplesmente arraste e solte o modelo apropriado para seu fluxo de trabalho.
