...
3. Implemente o contrato de serviço e <xref:System.ServiceModel.ServiceBehaviorAttribute.InstanceContextMode> especifique <xref:System.ServiceModel.InstanceContextMode.PerSession?displayProperty=nameWithType>um de. Isso instancia o serviço apenas uma vez para cada sessão.  
...
5. Configure um ponto de extremidade que usa a associação `NetMsmqBinding` fornecida pelo sistema.  
...
7. Crie um host de serviço para o serviço usando <xref:System.ServiceModel.ServiceHost>o.  
...
2. Crie o cliente WCF usando a ferramenta do [Utilitário de metadados ServiceModel (svcutil. exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) .  
...
### <a name="description"></a>Descrição  
 O exemplo a seguir fornece o código para `IProcessOrder` o serviço e para um cliente que usa esse serviço. Ele mostra como o WCF usa sessões enfileiradas para fornecer o comportamento de agrupamento.  
