...
## <a name="backward-compatibility-and-the-net-framework-45"></a>Compatibilidade com versões anteriores e o .NET Framework 4.5  
 O .NET Framework 4.5 e versões posteriores são compatíveis com versões anteriores com aplicativos que foram criados com versões anteriores do .NET Framework. Em outras palavras, aplicativos e componentes criados com versões anteriores irá funcionar sem modificação no .NET Framework 4.5 e versões posteriores. No entanto, por padrão, os aplicativos executados na versão do common language runtime para o qual eles foram desenvolvidos, então talvez seja necessário fornecer um arquivo de configuração para habilitar seu aplicativo para ser executado no .NET Framework 4.5 ou versões posteriores. Para saber mais, confira a seção [Compatibilidade de versão para aplicativos](#Apps), anteriormente neste artigo.  
 ...
 Se seu aplicativo ou componente não funcionar conforme esperado no .NET Framework 4.5 (incluindo versões seu ponto, o [!INCLUDE[net_v451](../../../includes/net-v451-md.md)], 4.5.2, 4.6, 4.6.1, 4.6.2, 4.7 ou 4.7.1, use as listas de verificação a seguir:  
  
-  Se seu aplicativo foi desenvolvido para executar qualquer versão do .NET Framework, começando com o .NET Framework 4.0, consulte [compatibilidade de aplicativos no .NET Framework](application-compatibility.md) para gerar listas de alterações entre a versão de destino do .NET Framework e a versão em que seu aplicativo é executado.  

- Se você tiver um aplicativo do .NET Framework 3.5, consulte também [problemas de migração do .NET Framework 4](../../../docs/framework/migration-guide/net-framework-4-migration-issues.md).

- Se você tiver um aplicativo do .NET Framework 2.0, consulte também [alterações no .NET Framework 3.5 SP1](http://go.microsoft.com/fwlink/?LinkId=186989).

- Se você tiver um aplicativo do .NET Framework 1.1, consulte também [alterações no .NET Framework 2.0](http://go.microsoft.com/fwlink/?LinkID=125263).  
  
-   Se estiver recompilar o código-fonte existente para ser executado no .NET Framework 4.5 ou seu ponto de versões, ou se você estiver desenvolvendo uma nova versão de um aplicativo ou componente que tem como alvo o [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] seu ponto de versões de uma verificação de base de código fonte existente ou [ O que está obsoleto na biblioteca de classes](../../../docs/framework/whats-new/whats-obsolete.md) para obsoleto tipos e membros e aplicar a solução descrita. (O código compilado anteriormente continuará sendo executado em tipos e membros marcados como obsoletos.)  
