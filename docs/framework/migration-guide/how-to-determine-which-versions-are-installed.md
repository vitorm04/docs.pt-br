...
A versão do .NET Framework (4,5 e posterior) instalada em um computador é listada no registro em **HKEY_LOCAL_MACHINE\\SOFTWARE\\instalação do Microsoft\\NET Framework\\NDP\\v4\\Full**. Se a subchave **completa** estiver ausente, .NET Framework 4,5 ou superior não estará instalado.
...
| .NET Framework 4.8     | No Windows 10 maio 2019 atualização e Windows 10 de novembro de 2019 atualização: 528040<br/>No Windows 10 e no Windows Server, versão 1909:528209<br/>Em todos os outros sistemas operacionais Windows (incluindo outros sistemas operacionais Windows 10): 528049 |
...
01. No editor do registro, abra a seguinte subchave: **HKEY_LOCAL_MACHINE\\SOFTWARE\\instalação do Microsoft\\NET Framework\\NDP\\v4\\Full**. Se a subchave **Full** não estiver presente, você não terá o .NET Framework 4.5 ou posterior instalado.
...
Use os comandos do PowerShell para verificar o valor da entrada de **versão** do **HKEY_LOCAL_MACHINE\\software\\a instalação do Microsoft\\NET Framework\\a subchave completa do NDP\\v4\\** .
...
01. Use os métodos <xref:Microsoft.Win32.RegistryKey.OpenBaseKey%2A?displayProperty=nameWithType> e <xref:Microsoft.Win32.RegistryKey.OpenSubKey%2A?displayProperty=nameWithType> para acessar a **HKEY_LOCAL_MACHINE\\SOFTWARE\\a instalação do Microsoft\\NET Framework\\NDP\\v4\\a** subchave completa no registro do Windows.

    > [!IMPORTANT]
    > Se o aplicativo que você está executando for de 32 bits e estiver em execução no Windows de 64 bits, os caminhos do registro serão diferentes dos listados anteriormente. O registro de 64 bits está disponível na subchave **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\** . Por exemplo, a subchave do registro para .NET Framework 4,5 é **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\a instalação do Microsoft\\NET Framework\\NDP\\v4\\Full**.
...
| ------------------ | --------------- | ----- |
| 1.0                | **HKLM\\software\\Microsoft\\. NETFramework\\Policy\\v 1.0\\3705**     | **Instalar** o REG_SZ é igual a `1` |
| 1,1                | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v 1.1.4322**   | **Instalar** o REG_DWORD é igual a `1` |
| 2,0                | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v 2.0.50727**  | **Instalar** o REG_DWORD é igual a `1` |
| 3.0                | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v 3.0\\setup** | **InstallSuccess** REG_DWORD é igual a `1` |
| 3,5                | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v 3.5**        | **Instalar** o REG_DWORD é igual a `1` |
| 4,0 perfil do cliente | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v4\\Client**  | **Instalar** o REG_DWORD é igual a `1` |
| 4,0 perfil completo   | **HKLM\\software\\instalação do Microsoft\\NET Framework\\NDP\\v4\\Full**    | **Instalar** o REG_DWORD é igual a `1` |

> [!IMPORTANT]
> Se o aplicativo que você está executando for de 32 bits e estiver em execução no Windows de 64 bits, os caminhos do registro serão diferentes dos listados anteriormente. O registro de 64 bits está disponível na subchave **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\** . Por exemplo, a subchave do registro para .NET Framework 3,5 é **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\instalação do Microsoft\\NET Framework\\NDP\\v 3.5**.
...
Use a classe <xref:Microsoft.Win32.RegistryKey?displayProperty=nameWithType> para acessar o **HKEY_LOCAL_MACHINE\\SOFTWARE\\a instalação do Microsoft\\NET Framework\\** subchave NDP no registro do Windows.
...
- **A ferramenta CLRVer. exe**
  ...
  Use a [ferramenta de versão do CLR (CLRVer. exe)](../tools/clrver-exe-clr-version-tool.md) para determinar quais versões do CLR estão instaladas em um computador. Abra o [prompt de comando do desenvolvedor para o Visual Studio](../tools/developer-command-prompt-for-vs.md) e insira `clrver`.
...
- **A classe `Environment`**
  ...
  01. Consulte a propriedade <xref:System.Environment.Version?displayProperty=nameWithType> para recuperar um objeto <xref:System.Version>.
      ...
      Para .NET Framework versões 4, 4,5, 4.5.1 e 4.5.2, a representação de cadeia de caracteres do objeto de <xref:System.Version> retornado tem o formato 4.0.30319. *xxxxx*, em que *xxxxx* é menor que 42000. Para .NET Framework 4,6 e versões posteriores, ele tem o formato 4.0.30319.42000.
