...
---
# <a name="how-to-implement-copytodatatablet-where-the-generic-type-t-is-not-a-datarow"></a>Como: Implementar CopyToDataTable\<T> onde o tipo genérico T não é um DataRow
...
1. Implementar a classe de `ObjectShredder<T>` para criar <xref:System.Data.DataTable> de uma fonte de <xref:System.Collections.Generic.IEnumerable%601> :  
    ...
    O exemplo anterior assume que `DataColumn` as propriedades dos tipos de valor não são anuladas. Para lidar com propriedades com tipos de valor anulados, use o seguinte código:
