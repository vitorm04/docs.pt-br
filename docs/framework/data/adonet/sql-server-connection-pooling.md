---
title: Pool de conexão do servidor SQL
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
> [!CAUTION]
> É altamente recomendável sempre fechar a conexão quando você terminar de usá-la para que a conexão seja retornada ao pool. Você pode fazer isso `Close` `Dispose` usando ou `Connection` métodos do objeto, ou `using` abrindo todas as `Using` conexões dentro de uma declaração em C#, ou uma declaração no Visual Basic. As conexões que não são fechadas explicitamente não podem ser adicionadas nem retornadas ao pool. Para obter mais informações, consulte [usando A Declaração](../../../csharp/language-reference/keywords/using-statement.md) ou [Como: Descartar um recurso do sistema](../../../visual-basic/programming-guide/language-features/control-flow/how-to-dispose-of-a-system-resource.md) para o Visual Basic.  
...
Para obter mais informações sobre os eventos associados às conexões de abertura e fechamento, consulte [Audit Login Event Class](/sql/relational-databases/event-classes/audit-login-event-class) e Audit [Logout Event Class](/sql/relational-databases/event-classes/audit-logout-event-class) na documentação do SQL Server.  
  
## <a name="removing-connections"></a>Removendo conexões  
 O pooler de conexão remove uma conexão da piscina depois de estar ociosa por aproximadamente 4-8 minutos, ou se o pooler detectar que a conexão com o servidor foi cortada. Observe que uma conexão interrompida pode ser detectada somente após uma tentativa de comunicação com o servidor. Se for encontrada uma conexão que não esteja mais conectada ao servidor, ela será marcada como inválida. As conexões inválidas são removidas de pool de conexões somente quando são fechadas ou recuperadas.  
...
## <a name="clearing-the-pool"></a>Limpando o pool  
 ADO.NET 2.0 introduziu dois novos métodos <xref:System.Data.SqlClient.SqlConnection.ClearAllPools%2A> <xref:System.Data.SqlClient.SqlConnection.ClearPool%2A>para limpar a piscina: e . `ClearAllPools` limpa os pools de conexões de um provedor específico e `ClearPool` limpa o pool de conexões associado a uma conexão específica. Se houver conexões em uso no momento da chamada, elas serão devidamente marcadas. Quando fechadas, serão descartadas, em vez de retornadas ao pool.  
