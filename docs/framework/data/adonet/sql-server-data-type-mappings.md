...
<sup>1</sup> você não pode definir a `DbType` propriedade de um `SqlParameter` para `SqlDbType.Date` .  
<sup>2</sup> use um acessador com tipo específico se você souber o tipo subjacente do `sql_variant` .  
