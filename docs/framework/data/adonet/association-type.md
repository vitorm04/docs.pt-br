...
Um *tipo de associação* (também chamado de associação) é o bloco de construção fundamental para descrever relações no modelo de dados de entidade (EDM). Em um modelo conceitual, uma associação representa uma relação entre dois [tipos de entidade](../../../../docs/framework/data/adonet/entity-type.md) ( `Customer` como e `Order`). Em um aplicativo, uma instância de uma associação representa uma associação específica (como uma associação entre uma instância de `Customer` e uma instância de `Order`). As instâncias de associação são agrupadas logicamente em um [conjunto de associação](../../../../docs/framework/data/adonet/association-set.md).  
  
 Uma definição de associação contém as informações a seguir:  
  
- Um nome exclusivo. (Necessário)  
  
- Duas [associações terminam](../../../../docs/framework/data/adonet/association-end.md), uma para cada tipo de entidade na relação. (Necessário)  
