...
## <a name="performing-a-non-transacted-bulk-copy-operation"></a>Executando uma operação não transacionada de cópia em massa  
 ...
 No exemplo, a tabela de origem e a tabela de destino incluem `Identity` uma coluna chamada **ProductID**. O código primeiro prepara a tabela de destino excluindo todas as linhas e, em seguida, inserindo uma única linha cujo **ProductID** é conhecido como existir na tabela de origem. Por padrão, um novo valor para a coluna `Identity` é gerado na tabela de destino para cada linha adicionada. Neste exemplo, uma opção é definida quando a conexão é aberta, o que força o processo de carregamento em massa usar os valores de `Identity` da tabela de origem.  
