...
## <a name="how-linq-to-sql-invokes-your-business-logic"></a>Como o LINQ to SQL chama sua lógica de negócios  
 Quando você gera uma classe de entidade em tempo de design, manualmente ou usando o SQLMetal ou Object Relational Designer, ele é definido como uma classe parcial. Isso significa que, em um arquivo de código separado, você pode definir outra parte da classe de entidade que contém a lógica de negócios personalizada. Em tempo de compilação, as duas partes são mescladas em uma única classe. Mas, se você precisar regenerar as classes de entidade usando o SQLMetal ou Object Relational Designer, você poderá fazer isso e sua parte da classe não será modificado.  
...
## <a name="a-closer-look-at-the-extensibility-points"></a>Uma visão mais profunda dos pontos de extensibilidade  
 O exemplo a seguir mostra parte do código gerado por Object Relational Designer para o `DataContext` classe que tem duas tabelas: `Customers` e `Orders`. Observe que os métodos Insert, Update e Delete são definidos para cada tabela da classe.  
...
```  
 ...
 Na sua parte da classe, você escreve uma definição de implementação do método. No Visual Studio, depois de digitar `partial` você verá o IntelliSense para as definições do método na outra parte da classe.  
