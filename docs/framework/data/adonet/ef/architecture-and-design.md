...
Esta seção aborda os tipos usados no provedor de [exemplo](https://code.msdn.microsoft.com/windowsdesktop/Entity-Framework-Sample-6a9801d0) que você usa para criar uma instrução SQL.
...
TopClause representa a expressão TOP em um SqlSelectStatement. A propriedade de TopCount indica quantas linhas SUPERIORES devem ser selecionadas.  Quando WithTies é true, o TopClause foi criado a partir de um DbLimitExpression.
...
As funções definidas pelo usuário são convertidas em NamespaceName. nomedafunção (arg1, arg2,..., argN).
