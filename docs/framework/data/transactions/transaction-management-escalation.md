...
- Pelo menos dois recursos duráveis que oferecem suporte a notificações de fase única estão inscritos na transação. Por exemplo, inscrever uma única conexão com SQL Server 2005 não faz com que uma transação seja promovida. No entanto, sempre que você abrir uma segunda conexão com um SQL Server banco de dados 2005 fazendo com <xref:System.Transactions> que o banco de dados se inscreva, a infraestrutura detectará que ele é o segundo recurso durável na transação e o escalonará para uma transação MSDTC.  
