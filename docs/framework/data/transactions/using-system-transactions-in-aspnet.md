...
## <a name="dynamic-compilation"></a>Compilação dinâmica
 Se você quiser importar e usar <xref:System.Transactions> em um aplicativo ASP.NET compilado dinamicamente no Access, você deve fazer uma referência ao assembly <xref:System.Transactions> no arquivo de configuração. Especificamente, a referência deve ser adicionada na seção **compilação** /**assemblies** do arquivo de configuração **Web. config** de raiz padrão ou um arquivo de configuração do aplicativo Web específico. O exemplo a seguir demonstra isso.
