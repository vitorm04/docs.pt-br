...
Este tópico descreve considerações de segurança específicas para desenvolver, implantar e executar serviços e aplicativos de dados WCF que acessam serviços que suportam o OData (Open Data Protocol, protocolo de dados abertos). Você também deve seguir recomendações para criar aplicativos .NET Framework seguros.  
  
Ao planejar como proteger um serviço OData baseado em Serviços de Dados WCF, você deve abordar tanto a autenticação, o processo de descobrir e verificar a identidade de um diretor, quanto a autorização, o processo de determinar se um principal autenticado é autorizado a acessar os recursos solicitados. Você também deve considerar a criptografia da mensagem usando o protocolo SSL.  
  
## <a name="authenticating-client-requests"></a>Autenticando solicitações de clientes  
O WCF Data Services não implementa nenhum tipo de autenticação própria, mas depende das disposições de autenticação do host de serviço de dados. Isso significa que o serviço assume que qualquer solicitação que recebe já foi autenticada pelo host de rede e que o host identificou corretamente o princípio da solicitação apropriadamente através das interfaces fornecidas pelos WCF Data Services. Essas opções e abordagens de autenticação são detalhadas na [série Multipart OData and Authentication](https://devblogs.microsoft.com/odata/?s=%22OData+and+authentication%22).  
...
```  
 ...
 Para obter mais informações, consulte [Como especificar credenciais do cliente para uma solicitação de serviço de dados.](specify-client-creds-for-a-data-service-request-wcf.md)  
  
 Quando o serviço de dados exige credenciais de logon que não podem ser especificadas com um objeto <xref:System.Net.NetworkCredential>, como um cookie ou um token baseado em declarações, você deve definir manualmente os cabeçalhos na solicitação HTTP, geralmente os cabeçalhos `Authorization` e `Cookie`. Para obter mais informações sobre esse tipo de cenário de autenticação, consulte o post do blog [OData and Authentication – Parte 3 – ClientSide Hooks](https://devblogs.microsoft.com/odata/odata-and-authentication-part-3-clientside-hooks/). Para obter um exemplo de como definir cabeçalhos HTTP em uma mensagem de solicitação, consulte [Como: Definir cabeçalhos na solicitação do cliente](how-to-set-headers-in-the-client-request-wcf-data-services.md).  
  
## <a name="impersonation"></a>Representação  
 Em geral, o serviço de dados acessa os recursos necessários, como arquivos no servidor ou em um banco de dados, usando as credenciais do processo de trabalho que está hospedando o serviço de dados. Ao usar a personificação, ASP.NET aplicativos podem ser executados com a identidade do Windows (conta de usuário) do usuário que faz a solicitação. A representação é normalmente usada em aplicativos que dependem do IIS para autenticar o usuário, e as credenciais desse princípio são usadas para acessar os recursos necessários. Para obter mais informações, consulte [ASP.NET personificação](https://docs.microsoft.com/previous-versions/aspnet/xh507fc5(v=vs.100)).  
...
- Limite o tamanho dos cabeçalhos HTTP e seus valores e forneça acesso aos Serviços de Dados WCF aos dados de cabeçalho.  
...
### <a name="considerations-for-client-applications"></a>Considerações para aplicativos cliente  
 As seguintes considerações de segurança se aplicam aos aplicativos que usam o cliente WCF Data Services para acessar os serviços OData:  
