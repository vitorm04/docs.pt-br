...
Este tópico fornece informações sobre como desenvolver e implantar WCF Data Services. Para obter informações mais básicas sobre WCF Data Services, consulte [introdução](getting-started-with-wcf-data-services.md) e [visão geral](wcf-data-services-overview.md).

## <a name="develop-wcf-data-services"></a>Desenvolver WCF Data Services

Ao usar WCF Data Services para criar um serviço de dados que ofereça suporte [!INCLUDE[ssODataFull](../../../../includes/ssodatafull-md.md)]ao, você deve executar as seguintes tarefas básicas durante o desenvolvimento:
...
Este tópico aborda principalmente o desenvolvimento e a implantação de serviços de dados usando o Visual Studio. Para obter informações sobre a flexibilidade fornecida pelo WCF Data Services para expor seus dados como feeds OData, consulte [definindo WCF Data Services](defining-wcf-data-services.md).
...
2. **Visual Studio Development Server**
    ...
    - Esse servidor não pode tratar fluxos HTTP em partes, que são enviados por padrão pelo cliente WCF Data Services ao acessar dados binários grandes do serviço de dados. Para obter mais informações, consulte [streaming Provider](streaming-provider-wcf-data-services.md).
...
3. **Ambiente de desenvolvimento do Windows Azure**

     As ferramentas do Windows Azure para Visual Studio incluem um conjunto integrado de ferramentas para o desenvolvimento de serviços do Windows Azure no Visual Studio. Com essas ferramentas, você pode desenvolver um serviço de dados que pode ser implantado no Microsoft Azure e você pode testar o serviço de dados no computador local antes da implantação. Use essas ferramentas ao usar o Visual Studio para desenvolver um serviço de dados executado na plataforma Windows Azure. Você pode baixar as ferramentas do Windows Azure para Visual Studio no [centro de download da Microsoft](https://go.microsoft.com/fwlink/?LinkID=201848). Para obter mais informações sobre como desenvolver um serviço de dados que é executado no Windows Azure, consulte o post [implantando um serviço OData no Windows Azure](https://go.microsoft.com/fwlink/?LinkId=201847).

### <a name="development-tips"></a>Dicas de desenvolvimento

Você deve considerar o seguinte ao desenvolver um serviço de dados:

- Determinar os requisitos de segurança de seu serviço de dados, se planejar autenticar usuários ou restringir o acesso para usuários específicos. Para obter mais informações, consulte [securing WCF Data Services](securing-wcf-data-services.md).
...
- **Servidor Web hospedado pelo IIS**
  ...
  - **Tecnologias de implantação para sites da ASP.NET**
    ...
    > [!TIP]
    > Antes de tentar implantar o serviço de dados para o IIS, verifique se você testou a implantação para um servidor Web que esteja executando o IIS. Para obter mais informações, confira [Como: Desenvolva um serviço de dados WCF em](how-to-develop-a-wcf-data-service-running-on-iis.md)execução no IIS.

- **Windows Azure**

     Você pode implantar um serviço de dados no Windows Azure usando as ferramentas do Windows Azure para Visual Studio. Você pode baixar as ferramentas do Windows Azure para Visual Studio no [centro de download da Microsoft](https://go.microsoft.com/fwlink/?LinkID=201848). Para obter mais informações sobre como implantar um serviço de dados no Windows Azure, consulte o post [implantando um serviço OData no Windows Azure](https://go.microsoft.com/fwlink/?LinkId=201847).
