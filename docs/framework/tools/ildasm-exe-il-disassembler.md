...
|**/Project**|Exibe metadados da maneira como são exibidos para o código gerenciado, e não da maneira como são exibidos no Windows Runtime nativo. Se `PEfilename` não for um arquivo de metadados do Windows ( *. winmd* ), essa opção não terá nenhum efeito. Consulte [Suporte do .NET Framework para aplicativos da Windows Store e Windows Runtime](../cross-platform/support-for-windows-store-apps-and-windows-runtime.md).|
