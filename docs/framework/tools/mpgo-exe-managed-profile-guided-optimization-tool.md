...
|`-Timeout` \<*segundos*>|The amount of time to allow the Windows 8.x Store app to run before the app exits.|
...
- Não é possível usar caminhos entre aspas com marcas de barra à direita porque as macros do Visual Studio também usam marcas de barra à direita por padrão. (For example, `–OutDir "C:\Output Folder\"` is invalid.) To work around this restriction, you can escape the trailing slash. (Por exemplo, use `-OutDir "$(OutDir)\"` em vez disso.)  
