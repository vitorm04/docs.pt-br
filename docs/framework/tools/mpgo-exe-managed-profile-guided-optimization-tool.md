...
|`-Timeout` \<*segundos*>|A quantidade de tempo para permitir que o aplicativo da loja do Windows 8. x seja executado antes de o aplicativo ser encerrado.|
...
|`-ExeConfig` \<*nome do arquivo*>|Especifica o arquivo de configuração usado pelo cenário para fornecer as informações de versão e carregador.|
