...
- Sobreposição de decorações visuais em um <xref:System.Windows.UIElement>.
- Mascarar visualmente ou substituir parte ou todas as <xref:System.Windows.UIElement>.
...
A estrutura de adornos fornecida por [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] destina-se principalmente a dar suporte à criação de adornos personalizados. Um adorno personalizado é criado pela implementação de uma classe que herda da classe abstrata <xref:System.Windows.Documents.Adorner>.

> [!NOTE]
> O pai de um <xref:System.Windows.Documents.Adorner> é o <xref:System.Windows.Documents.AdornerLayer> que renderiza a <xref:System.Windows.Documents.Adorner>, não o elemento que está sendo adornado.
...
Para habilitar o teste de clique de passagem de elementos em um adorno, defina o teste de clique <xref:System.Windows.UIElement.IsHitTestVisible%2A> Propriedade como **false** no adorno.  Para obter mais informações sobre testes de clique, consulte [teste de clique na camada Visual](../graphics-multimedia/hit-testing-in-the-visual-layer.md).
...
1. Chame o método estático <xref:System.Windows.Documents.AdornerLayer.GetAdornerLayer%2A> para obter um objeto <xref:System.Windows.Documents.AdornerLayer> para que o <xref:System.Windows.UIElement> seja adornado. <xref:System.Windows.Documents.AdornerLayer.GetAdornerLayer%2A> percorre a árvore visual, começando pelo <xref:System.Windows.UIElement>especificado e retorna a primeira camada de adorno que encontrar. (Se nenhuma camada de adorno for encontrada, o método retornará nulo.)

2. Chame o método <xref:System.Windows.Documents.AdornerLayer.Add%2A> para associar o adorno ao <xref:System.Windows.UIElement>de destino.
...
1. Chame o método `static` <xref:System.Windows.Documents.AdornerLayer.GetAdornerLayer%2A> para encontrar uma camada de adorno para o elemento cujos filhos devem ser adornados.

2. Enumere os filhos do elemento pai e chame o método <xref:System.Windows.Documents.AdornerLayer.Add%2A> para associar um adorno a cada elemento filho.
