---
title: Interoperabilidade do WPF e do Direct3D9
titleSuffix: ''
ms.date: 03/30/2017
dev_langs:
- cpp
...
## <a name="display-buffers"></a>Buffers de exibição  
 A classe <xref:System.Windows.Interop.D3DImage> gerencia dois buffers de exibição, que são chamados de *buffer de fundo* e o *buffer frontal*. O buffer de fundo é a superfície do Direct3D9. As alterações no buffer de fundo são copiadas para o buffer frontal quando você chama o método <xref:System.Windows.Interop.D3DImage.Unlock%2A>.  
...
## <a name="creating-the-back-buffer-render-target"></a>Criando o destino de renderização do buffer de fundo  
 Para exibir o conteúdo de Direct3D9 em um <xref:System.Windows.Interop.D3DImage>, você cria uma superfície de Direct3D9 e a atribui chamando o método <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A>.  
...
### <a name="wddm"></a>WDDM  
 No Windows Vista e em sistemas operacionais posteriores, que são configurados para usar o WDDM, você pode criar uma textura de destino de renderização e passar a superfície de nível 0 para o método de <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A>. Essa abordagem não é recomendada no Windows XP, porque você não pode criar uma textura de destino de renderização bloqueável e o desempenho será reduzido.  
  
## <a name="handling-device-state"></a>Manipulação de estado de dispositivo  
 A classe <xref:System.Windows.Interop.D3DImage> gerencia dois buffers de exibição, que são chamados de *buffer de fundo* e o *buffer frontal*. O buffer de fundo é a superfície do Direct3D.  As alterações no buffer de fundo são copiadas para o buffer frontal quando você chama o método <xref:System.Windows.Interop.D3DImage.Unlock%2A>, onde ele é exibido no hardware. Ocasionalmente, o buffer frontal se torna indisponível. Essa falta de disponibilidade pode ser causada por bloqueio de tela, aplicativos Direct3D de uso exclusivo em tela inteira, troca de usuário ou outras atividades do sistema. Quando isso ocorre, seu aplicativo do WPF é notificado tratando o evento <xref:System.Windows.Interop.D3DImage.IsFrontBufferAvailableChanged>.  A maneira que seu aplicativo responde à indisponibilidade do buffer frontal depende de se o WPF está habilitado para voltar à renderização de software. O método <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A> tem uma sobrecarga que usa um parâmetro que especifica se o WPF volta para a renderização de software.  
  
 Quando você chama a sobrecarga de <xref:System.Windows.Interop.D3DImage.SetBackBuffer%28System.Windows.Interop.D3DResourceType%2CSystem.IntPtr%29> ou chama a sobrecarga de <xref:System.Windows.Interop.D3DImage.SetBackBuffer%28System.Windows.Interop.D3DResourceType%2CSystem.IntPtr%2CSystem.Boolean%29> com o parâmetro `enableSoftwareFallback` definido como `false`, o sistema de renderização libera sua referência para o buffer de fundo quando o buffer frontal fica indisponível e nada é exibido. Quando o buffer frontal está disponível novamente, o sistema de renderização gera o evento <xref:System.Windows.Interop.D3DImage.IsFrontBufferAvailableChanged> para notificar seu aplicativo WPF.  Você pode criar um manipulador de eventos para o evento <xref:System.Windows.Interop.D3DImage.IsFrontBufferAvailableChanged> para reiniciar a renderização com uma superfície Direct3D válida. Para reiniciar a renderização, você deve chamar <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A>.  
  
 Quando você chama a sobrecarga de <xref:System.Windows.Interop.D3DImage.SetBackBuffer%28System.Windows.Interop.D3DResourceType%2CSystem.IntPtr%2CSystem.Boolean%29> com o parâmetro `enableSoftwareFallback` definido como `true`, o sistema de renderização retém sua referência ao buffer de fundo quando o buffer frontal fica indisponível, portanto, não há necessidade de chamar <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A> quando o buffer frontal está disponível novamente.  
  
 Quando a renderização de software é habilitada, pode haver situações em que o dispositivo do usuário fica indisponível, mas o sistema de renderização retém uma referência à superfície do Direct3D. Para verificar se um dispositivo de Direct3D9 está indisponível, chame o método `TestCooperativeLevel`. Para verificar um dispositivo Direct3D9Ex, chame o método `CheckDeviceState`, porque o método `TestCooperativeLevel` foi preterido e sempre retorna êxito. Se o dispositivo de usuário ficar indisponível, chame <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A> para liberar a referência do WPF para o buffer de fundo.  Se você precisar redefinir seu dispositivo, chame <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A> com o parâmetro `backBuffer` definido como `null`e, em seguida, chame <xref:System.Windows.Interop.D3DImage.SetBackBuffer%2A> novamente com `backBuffer` definido como uma superfície Direct3D válida.  
...
## <a name="handling-resizing"></a>Lidando com o redimensionamento  
 Se uma <xref:System.Windows.Interop.D3DImage> for exibida em uma resolução diferente de seu tamanho nativo, ela será dimensionada de acordo com a <xref:System.Windows.Media.RenderOptions.BitmapScalingMode%2A>atual, exceto que <xref:System.Windows.Media.Effects.SamplingMode.Bilinear> é substituída por <xref:System.Windows.Media.BitmapScalingMode.Fant>.  
  
 Se você precisar de maior fidelidade, deverá criar uma nova superfície quando o contêiner do <xref:System.Windows.Interop.D3DImage> alterar o tamanho.  
...
- Crie um <xref:System.Windows.Threading.DispatcherTimer> que verifica as dimensões do contêiner várias vezes por segundo.  
  
## <a name="multi-monitor-optimization"></a>Otimização de vários monitores  
 O desempenho significativamente reduzido pode resultar quando o sistema de renderização move um <xref:System.Windows.Interop.D3DImage> para outro monitor.  
...
1. Encontre um ponto de <xref:System.Windows.Interop.D3DImage> no espaço da tela com o método `Visual.ProjectToScreen`.  
...
4. Se o adaptador não for o mesmo que o adaptador com o buffer de fundo, crie um novo buffer de fundo no novo monitor e atribua-o ao buffer de back <xref:System.Windows.Interop.D3DImage>.  
  
> [!NOTE]
> Se a <xref:System.Windows.Interop.D3DImage> se espalhar por monitores, o desempenho será lento, exceto no caso do WDDM e `IDirect3D9Ex` no mesmo adaptador. Não há nenhuma maneira de melhorar o desempenho nessa situação.  
  
 O exemplo de código a seguir mostra como localizar o monitor atual.  
  
 [!code-cpp[System.Windows.Interop.D3DImage#RendererManager_SetAdapter](~/samples/snippets/cpp/VS_Snippets_Wpf/System.Windows.Interop.D3DImage/cpp/renderermanager.cpp#renderermanager_setadapter)]  
  
 Atualize o monitor quando o tamanho ou a posição do contêiner de <xref:System.Windows.Interop.D3DImage> for alterado ou atualize o monitor usando uma `DispatcherTimer` que atualiza algumas vezes por segundo.  
...
- <xref:System.Windows.Media.Imaging.RenderTargetBitmap>  
 ...
 Quando uma dessas situações ocorre, o sistema de renderização chama o método <xref:System.Windows.Interop.D3DImage.CopyBackBuffer%2A> para copiar o buffer de hardware para o software. A implementação padrão chama o método `GetRenderTargetData` com sua superfície. Como essa chamada ocorre fora do padrão de bloqueio/desbloqueio, ela poderá falhar. Nesse caso, o método `CopyBackBuffer` retorna `null` e nenhuma imagem é exibida.  
  
 Você pode substituir o método <xref:System.Windows.Interop.D3DImage.CopyBackBuffer%2A>, chamar a implementação base e, se ele retornar `null`, você poderá retornar um espaço reservado <xref:System.Windows.Media.Imaging.BitmapSource>.  
  
 Você também pode implementar sua própria renderização de software em vez de chamar a implementação de base.  
  
> [!NOTE]
> Se o WPF estiver processando completamente no software, <xref:System.Windows.Interop.D3DImage> não será mostrado porque o WPF não tem um buffer frontal.  
