...
## <a name="data-objects"></a>Objetos de dados  
 Todos os objetos de dados devem implementar o <xref:System.Windows.IDataObject> interface, que fornece o seguinte conjunto padrão de métodos que permitem e facilitam a transferência de dados.  
...
|<xref:System.Windows.IDataObject.SetData%2A>|Armazena os dados especificados neste objeto de dados.|  
 ...
 Normalmente, objetos de dados incluem um recurso para converter automaticamente dados armazenados em um formato para um formato diferente ao extrair os dados; esse recurso é conhecido como conversão automática. Ao consultar os formatos de dados disponíveis em um objeto de dados, os formatos de dados auto conversíveis podem ser filtrados de formatos de dados nativos chamando o <xref:System.Windows.DataObject.GetFormats%28System.Boolean%29> ou <xref:System.Windows.DataObject.GetDataPresent%28System.String%2CSystem.Boolean%29> método e especificando o `autoConvert` parâmetro como `false`.  Ao adicionar dados a um objeto de dados com o <xref:System.Windows.DataObject.SetData%28System.String%2CSystem.Object%2CSystem.Boolean%29> método, a conversão automática de dados pode ser proibido definindo o `autoConvert` parâmetro `false`.  
...
### <a name="creating-new-data-objects"></a>Criando novos objetos de dados  
 ...
 O exemplo de código a seguir cria um novo objeto de dados e usa um dos construtores sobrecarregados <xref:System.Windows.DataObject.%23ctor%2A>(<xref:System.Windows.DataObject.%23ctor(System.String,System.Object)>) para inicializar o objeto de dados com uma cadeia de caracteres e um formato de dados especificado.  Nesse caso, o formato de dados é especificado por uma cadeia de caracteres; o <xref:System.Windows.DataFormats> classe fornece um conjunto de cadeias de caracteres de tipo pré-definidas. A conversão automática dos dados armazenados é permitida por padrão.  
  
 [!code-csharp[DragDrop_DragDropMiscCode#_DragDrop_CreateDataObject_TypeString](../../../../samples/snippets/csharp/VS_Snippets_Wpf/DragDrop_DragDropMiscCode/CSharp/Window1.xaml.cs#_dragdrop_createdataobject_typestring)]
 [!code-vb[DragDrop_DragDropMiscCode#_DragDrop_CreateDataObject_TypeString](../../../../samples/snippets/visualbasic/VS_Snippets_Wpf/DragDrop_DragDropMiscCode/visualbasic/window1.xaml.vb#_dragdrop_createdataobject_typestring)]  
  
 Para obter mais exemplos de código que cria um objeto de dados, consulte [Criar um objeto de dados](../../../../docs/framework/wpf/advanced/how-to-create-a-data-object.md).  
...
### <a name="querying-a-data-object-for-available-formats"></a>Consultando um objeto de dados para os formatos disponíveis  
 ...
 O código de exemplo a seguir usa o <xref:System.Windows.DataObject.GetFormats%2A> sobrecarga para obter uma matriz de cadeias de caracteres que indica todos os formatos de dados disponíveis em um objeto de dados (nativo e por conversão automática).  
...
### <a name="retrieving-data-from-a-data-object"></a>Recuperando dados de um objeto de dados  
 ...
 Para obter mais exemplos de códigos que recuperam dados de um objeto de dados, consulte [Recuperar dados em um formato de dados específico](../../../../docs/framework/wpf/advanced/how-to-retrieve-data-in-a-particular-data-format.md).  
...
2.  "Copie" os dados desejados do objeto de dados antigo para o novo objeto de dados.  Para copiar os dados, use uma da <xref:System.Windows.DataObject.GetData%2A> métodos para recuperar um <xref:System.Object> que contém os dados brutos e, em seguida, use um do <xref:System.Windows.DataObject.SetData%2A> métodos para adicionar os dados para o novo objeto de dados.  
  
3.  Substitua o objeto de dados antigo pelo novo.  
  
> [!NOTE]
>  O <xref:System.Windows.DataObject.SetData%2A> métodos apenas adicionam dados a um objeto de dados; eles não substituem os dados, mesmo se os dados e o formato de dados são exatamente o mesmo que uma chamada anterior. Chamando <xref:System.Windows.DataObject.SetData%2A> duas vezes para os mesmos dados e dados de formato resultará no formato de dados/dados sendo presente duas vezes no objeto de dados.
