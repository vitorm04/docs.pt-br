...
- `RelativeSource` fornece informações de origem para um <xref:System.Windows.Data.Binding> que pode navegar por várias relações possíveis na árvore de objetos de tempo de execução. São oferecidas fontes especializadas para associações criadas em modelos multiuso ou criadas em código sem conhecimento completo da árvore de objetos adjacente. Para ver os detalhes, consulte [RelativeSource MarkupExtension](relativesource-markupextension.md).  
...
## <a name="extension-classes"></a>\*classes de extensão  
 ...
 O padrão de nomenclatura do `*Extension` é para conveniência e consistência. Não é necessário para que um processador XAML possa identificar essa classe como suporte para uma extensão de marcação. Contanto que sua codebase inclua System. XAML e use .NET Framework implementações de serviços XAML, tudo o que é necessário para ser reconhecido como uma extensão de marcação XAML é derivar de <xref:System.Windows.Markup.MarkupExtension> e para dar suporte a uma sintaxe de construção. O WPF define a extensão de marcação – habilitando classes que não seguem o padrão de nomenclatura de `*Extension`, por exemplo <xref:System.Windows.Data.Binding>. Normalmente, o motivo disso é que a classe dá suporte a cenários além do suporte à extensão de marcação puro. No caso de <xref:System.Windows.Data.Binding>, essa classe dá suporte ao acesso em tempo de execução para métodos e propriedades do objeto em cenários que não têm nada a ver com XAML.  
...
- Se houver um resultado paralelo entre o comportamento do construtor e o comportamento de configuração da propriedade em uma extensão de marcação, não importará qual comportamento será utilizado. O mais comum é usar os pares *property*`=`*value* para extensões de marcação que têm mais de uma propriedade configurável, pelo menos porque torna a marcação mais intencional e diminui a probabilidade de transpor acidentalmente os parâmetros do construtor. (Quando você especifica pares de propriedade = valor, essas propriedades podem estar em qualquer ordem.) Além disso, não há nenhuma garantia de que uma extensão de marcação forneça um parâmetro de construtor que defina cada uma de suas propriedades de tabela. Por exemplo, <xref:System.Windows.Data.Binding> é uma extensão de marcação, com muitas propriedades que são configuradas por meio da extensão na *propriedade*`=`formulário *valor* , mas <xref:System.Windows.Data.Binding> só dá suporte a dois construtores: um construtor sem parâmetros e outro que define um caminho inicial.  
