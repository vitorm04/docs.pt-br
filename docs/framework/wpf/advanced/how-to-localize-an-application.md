...
## <a name="requirements"></a>Requisitos  
 Ao longo desta discussão, você usará o Microsoft build engine (MSBuild), que é um compilador que funciona a partir da linha de comando.  
  
 Além disso, você será instruído a usar um arquivo de projeto. Para obter instruções sobre como usar arquivos msbuild e projeto, consulte [Build and Deploy](../app-development/building-and-deploying-wpf-applications.md).  
...
## <a name="create-a-sample-application"></a>Criar um aplicativo de exemplo  
 Nesta etapa, você preparará seu aplicativo para a localização. Nas [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] amostras, é fornecida uma amostra de HelloApp que será usada para os exemplos de código nesta discussão. Se você quiser usar esta amostra, baixe os [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] arquivos da Amostra de Ferramenta [LocBaml](https://github.com/microsoft/WPF-Samples/tree/master/Tools/LocBaml).  
  
1. Desenvolva seu aplicativo até o ponto em que você deseja iniciar a localização.  
  
2. Especifique a linguagem de desenvolvimento no arquivo do projeto para que o MSBuild gere uma montagem principal e um conjunto por satélite (um arquivo com a extensão .resources.dll) para conter os recursos de linguagem neutra. O arquivo de projeto no exemplo HelloApp é o HelloApp.csproj. Nesse arquivo, você encontrará a linguagem de desenvolvimento identificada da seguinte maneira:  
  
     `<UICulture>en-US</UICulture>`  
  
3. Adicione Uids [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] aos seus arquivos. Os Uids são usados para controlar as alterações nos arquivos e para identificar os itens que devem ser traduzidos. Para adicionar Uids aos seus arquivos, execute **updateuid** no arquivo do projeto:  
  
     **msbuild -t:updateuid helloapp.csproj**  
  
     Para verificar se você não tem Uids faltando ou duplicado, execute **checkuid**:  
...
1. Compile o HelloApp para criar uma biblioteca de links dinâmicos (DLL):  
...
1. Todos os arquivos necessários para construir LocBaml estão localizados nas [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] amostras. Baixe os arquivos C# da amostra de [ferramentas locBaml](https://github.com/microsoft/WPF-Samples/tree/master/Tools/LocBaml).  
...
4. As opções que você pode especificar ao executar a LocBaml são as seguintes:  
    ...
    - **analisar** ou **-p:** Analisa arquivos Baml, resources ou DLL para gerar um arquivo .csv ou .txt.  
  
    - **gerar** ou **-g:** Gera um arquivo binário localizado usando um arquivo traduzido.  
  
    - **fora** ou **-o** {*diretório de arquivos*] : **Nome** do arquivo de saída.  
  
    - **cultura** ou **-cul** {*cultura*] **:** Locale de conjuntos de saída.  
  
    - **tradução** ou **-trans** {*translation.csv*] **:** Arquivo traduzido ou localizado.  
  
    - **asmpath** ou **-asmpath:** {*filedirectory*] **:** Se o seu [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] código contiver controles personalizados, você deve fornecer o **asmpath** para o conjunto de controle personalizado.  
...
3. Ao executar a LocBaml para analisar arquivos, a saída consistirá em sete campos delimitados por vírgulas (arquivos .csv) ou tabulações (arquivos .txt). A seguir está o arquivo .csv analisado do HelloApp.resources.dll:
   ...
   2. **Chave de recursos**. O identificador do recurso localizado.  
  
   3. **Categoria**. O tipo de valor. Veja [Atributos e Comentários de Localização](localization-attributes-and-comments.md).  
  
   4. **Legibilidade**. Se o valor pode ser lido por um localizador. Veja [Atributos e Comentários de Localização](localization-attributes-and-comments.md).  
  
   5. **Modificabilidade**. Se o valor pode ser modificado por um localizador. Veja [Atributos e Comentários de Localização](localization-attributes-and-comments.md).  
  
   6. **Comentários**. Descrição adicional do valor para ajudar a determinar como um valor é localizado. Veja [Atributos e Comentários de Localização](localization-attributes-and-comments.md).  
   ...
   Observe que todos os valores para o campo **Comentários** não contêm valores; se um campo não tem um valor, ele está vazio. Observe também que o item na primeira linha não é legível nem modificável, e tem "Ignorar" como seu valor de **categoria,** tudo indica que o valor não é localizado.  
  
4. Para facilitar a descoberta de itens localizados em arquivos analisados, particularmente em arquivos grandes, você pode classificar ou filtrar os itens por **categoria,** **leitura**e **modificabilidade.** Por exemplo, você pode filtrar valores e ilegíveis e não modificáveis.  
  
<a name="translate_loc_content"></a>
## <a name="translate-the-localizable-content"></a>Traduzir o conteúdo localizável  
 Use qualquer ferramenta que você tiver disponível para traduzir o conteúdo extraído. Uma boa maneira de fazer isso é escrever os recursos em um arquivo .csv e visualizá-los no Microsoft Excel, fazendo alterações de tradução na última coluna (valor).  
  
<a name="merge_translations"></a>
## <a name="use-locbaml-to-generate-a-new-resourcesdll-file"></a>Usar a LocBaml para gerar um novo arquivo .resources.dll  
 O conteúdo que foi identificado ao analisar o HelloApp.resources.dll com a LocBaml foi traduzido e deve ser mesclado de volta no aplicativo original. Use a opção **gerar** ou **-g** para gerar um novo arquivo .resources.dll.  
...
7. Para testar o novo assembly satélite, você precisa alterar a cultura na qual o aplicativo será executado. É possível fazer isso de duas formas:  
    ...
    - Altere as configurações regionais do sistema operacional **(Inicie** &#124; **Painel de Controle** &#124; **Opções Regionais e de Idiomas**).  
...
- Todos os assemblies dependentes que definem controles personalizados devem ser copiados para o diretório local da LocBaml ou instalados no GAC. Isso é necessário porque a API de localização deve ter acesso aos conjuntos dependentes quando lê o Binário XAML (BAML).  
