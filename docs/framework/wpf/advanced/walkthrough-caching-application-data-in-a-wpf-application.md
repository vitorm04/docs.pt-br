...
## <a name="targeting-the-net-framework-and-adding-a-reference-to-the-caching-assemblies"></a>Direcionamento ao .NET Framework e adição de uma referência aos assemblies de cache  
 ...
 Portanto, a próxima etapa é alterar o destino do .NET Framework e adicione uma referência para o <xref:System.Runtime.Caching> namespace.  
...
-   Criar uma instância da classe cache — ou seja, você instanciará uma nova <xref:System.Runtime.Caching.MemoryCache> objeto.  
...
3.  No manipulador de eventos, adicione o seguinte código para instanciar o objeto de cache:  
    ...
    ```  
     ...
     O <xref:System.Runtime.Caching.ObjectCache> classe é uma classe interna que fornece um cache de objetos na memória.  
...
6.  No `if/then` em bloco, adicione o seguinte código para criar um novo <xref:System.Runtime.Caching.CacheItemPolicy> objeto que especifica que a entrada de cache expire após 10 segundos.  
    ...
    ```  
     ...
     Se nenhuma informação de remoção ou expiração for fornecida, o padrão é <xref:System.Runtime.Caching.ObjectCache.InfiniteAbsoluteExpiration>, que significa que as entradas de cache nunca expirar com base apenas em um tempo absoluto. Em vez disso, as entradas de cache expiram somente quando há pressão de memória. Como uma melhor prática, você deve sempre fornecer explicitamente uma expiração absoluta ou alternativa.  
...
8.  Após o código que você adicionou na etapa anterior, adicione o seguinte código para adicionar um novo <xref:System.Runtime.Caching.HostFileChangeMonitor> monitora o objeto à coleção de alteração para a entrada de cache:  
...
10. Após o código que você adicionou na etapa anterior, adicione o seguinte código para inserir o conteúdo do arquivo para o objeto de cache como um <xref:System.Runtime.Caching.CacheItem> instância:  
    ...
    ```  
     ...
     Especifique informações sobre como a entrada de cache deve ser removida, passando o <xref:System.Runtime.Caching.CacheItemPolicy> objeto que você criou anteriormente como um parâmetro.  
...
## <a name="see-also"></a>Consulte também  
 ...
 [Cache em aplicativos do .NET Framework](../../../../docs/framework/performance/caching-in-net-framework-applications.md)
