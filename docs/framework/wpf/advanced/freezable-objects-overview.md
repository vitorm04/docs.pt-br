...
## <a name="what-is-a-freezable"></a>O que é um Congelável?  
 Um <xref:System.Windows.Freezable> é um tipo especial de objeto que tem dois estados: descongelada e congelado. Quando descongelada, um <xref:System.Windows.Freezable> parece se comportar como qualquer outro objeto. Quando congelado, um <xref:System.Windows.Freezable> não pode mais ser modificado.  
  
 Um <xref:System.Windows.Freezable> fornece um <xref:System.Windows.Freezable.Changed> eventos para notificar observadores de quaisquer modificações do objeto. Congelar um <xref:System.Windows.Freezable> podem melhorar seu desempenho, porque ele não precisa gastar recursos em notificações de alteração. Um congelada <xref:System.Windows.Freezable> também podem ser compartilhadas entre threads, enquanto um descongelada <xref:System.Windows.Freezable> não é possível.  
  
 Embora o <xref:System.Windows.Freezable> classe tem muitos aplicativos, mais <xref:System.Windows.Freezable> objetos no [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] relacionados ao subsistema de elementos gráficos.  
  
 O <xref:System.Windows.Freezable> classe torna mais fácil de usar determinados objetos do sistema gráfico e pode ajudar a melhorar o desempenho do aplicativo. Exemplos de tipos que herdam de <xref:System.Windows.Freezable> incluem o <xref:System.Windows.Media.Brush>, <xref:System.Windows.Media.Transform>, e <xref:System.Windows.Media.Geometry> classes. Como eles contêm recursos não gerenciados, o sistema deve monitorar as modificações a esses objetos e, em seguida, atualizar seus recursos não gerenciados correspondentes, quando houver uma alteração no objeto original. Mesmo se você não realmente modificar um objeto do sistema de elementos gráficos, o sistema ainda deverá gastar um pouco de seus recursos monitorando o objeto, para o caso de você alterá-lo.  
  
 Por exemplo, suponha que você crie um <xref:System.Windows.Media.SolidColorBrush> de pincel e usá-lo para pintar o plano de fundo de um botão.  
 ...
 Um freezable <xref:System.Windows.Freezable.Freeze%2A> método permite que você desative essa capacidade auto-atualização. Você pode usar esse método para fazer com que o pincel se torne "congelado" ou não modificável.  
  
> [!NOTE]
>  Nem todo objeto Congelável pode ser congelado. Para evitar que seja um <xref:System.InvalidOperationException>, verifique o valor do objeto Freezable <xref:System.Windows.Freezable.CanFreeze%2A> propriedade para determinar se ele pode ser congelado antes de tentar congelar a ele.  
...
## <a name="using-freezables"></a>Usando Congeláveis  
 O uso de um congelável descongelado é como o uso de qualquer outro tipo de objeto. No exemplo a seguir, a cor de um <xref:System.Windows.Media.SolidColorBrush> é alterada de amarelo para vermelho depois que ele é usado para pintar o plano de fundo de um botão. O sistema de elementos gráficos funciona nos bastidores para alterar automaticamente o botão de amarelo para vermelho na próxima vez que a tela for atualizada.  
...
### <a name="freezing-a-freezable"></a>Congelando uma congelável  
 Para fazer um <xref:System.Windows.Freezable> não modificável, você chama seu <xref:System.Windows.Freezable.Freeze%2A> método. Quando você congela um objeto que contém objetos congeláveis, esses objetos também são congelados. Por exemplo, se você congelar uma <xref:System.Windows.Media.PathGeometry>, as ilustrações e os segmentos que ele contém seriam congelados também.  
...
-   Ele contém <xref:System.Windows.Freezable> subobjetos não podem ser congelados.  
 ...
 Se essas condições forem falsas, e você não pretende modificar o <xref:System.Windows.Freezable>, em seguida, você deve congelar para obter os benefícios de desempenho descritos anteriormente.  
  
 Depois de você chamar um freezable <xref:System.Windows.Freezable.Freeze%2A> método, ele não pode mais ser modificado. Tentativa de modificar um congelada objeto faz com que um <xref:System.InvalidOperationException> seja gerada. O código a seguir gera uma exceção, pois tenta modificar o pincel depois de ele ter sido congelado.  
  
 [!code-csharp[freezablesample_procedural#ExceptionExample](../../../../samples/snippets/csharp/VS_Snippets_Wpf/freezablesample_procedural/CSharp/freezablesample.cs#exceptionexample)]
 [!code-vb[freezablesample_procedural#ExceptionExample](../../../../samples/snippets/visualbasic/VS_Snippets_Wpf/freezablesample_procedural/visualbasic/freezablesample.vb#exceptionexample)]  
  
 Para evitar gerar essa exceção, você pode usar o <xref:System.Windows.Freezable.IsFrozen%2A> método para determinar se um <xref:System.Windows.Freezable> está congelada.  
 ...
 **Observação** porque um congelada freezable não pode ser animado, o sistema de animação criará automaticamente clones modificáveis de congelada <xref:System.Windows.Freezable> objetos quando você tentar animá-los com um <xref:System.Windows.Media.Animation.Storyboard>. Para eliminar a sobrecarga de desempenho causada pela clonagem, deixe um objeto descongelado se você pretende animá-lo. Para obter mais informações sobre animação com storyboards, consulte a [Visão geral de storyboards](../../../../docs/framework/wpf/graphics-multimedia/storyboards-overview.md).  
...
### <a name="unfreezing-a-freezable"></a>"Descongelando" um Congelável  
 Após congelado, um <xref:System.Windows.Freezable> nunca pode ser modificado ou descongelada; no entanto, você pode criar um clone descongelado usando o <xref:System.Windows.Freezable.Clone%2A> ou <xref:System.Windows.Freezable.CloneCurrentValue%2A> método.  
...
>  Independentemente de qual método de clonagem usar, animações nunca são copiadas para o novo <xref:System.Windows.Freezable>.  
 ...
 O <xref:System.Windows.Freezable.Clone%2A> e <xref:System.Windows.Freezable.CloneCurrentValue%2A> métodos produzem cópias profundas do freezable. Se o congelável contém outros objetos congeláveis congelados, eles também são clonados e tornados modificáveis. Por exemplo, se você clonar um congelada <xref:System.Windows.Media.PathGeometry> para torná-lo modificável, as ilustrações e os segmentos que ele contém são também copiados e tornados modificáveis.  
...
-   No início de qualquer [!INCLUDE[TLA#tla_api](../../../../includes/tlasharptla-api-md.md)] que lê membros de dados de propriedades não-dependentes, chame o <xref:System.Windows.Freezable.ReadPreamble%2A> método.  
  
-   No início de API que escreve membros de dados de propriedades não-dependentes, chame o <xref:System.Windows.Freezable.WritePreamble%2A> método. (Depois de você ter chamado <xref:System.Windows.Freezable.WritePreamble%2A> em uma [!INCLUDE[TLA2#tla_api](../../../../includes/tla2sharptla-api-md.md)], você não precisa fazer uma chamada adicional para <xref:System.Windows.Freezable.ReadPreamble%2A> se você também pode ler membros de dados de propriedade de dependência não.)  
  
-   Chamar o <xref:System.Windows.Freezable.WritePostscript%2A> método antes de sair de métodos que gravam membros de dados de propriedade de dependência não.  
  
 Se sua classe contém membros de dados de propriedade de dependência não são <xref:System.Windows.DependencyObject> objetos, você também deve chamar o <xref:System.Windows.Freezable.OnFreezablePropertyChanged%2A> método sempre que alterar em seus valores, mesmo se você estiver definindo o membro de `null`.  
...
>  É muito importante que você comece cada <xref:System.Windows.Freezable> método substituir com uma chamada para a implementação base.  
 ...
 Para obter um exemplo de um personalizado <xref:System.Windows.Freezable> de classe, consulte o [exemplo animação personalizado](http://go.microsoft.com/fwlink/?LinkID=159981).  
