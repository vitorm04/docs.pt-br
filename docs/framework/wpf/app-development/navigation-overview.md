...
No [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)], você pode navegar para vários tipos de conteúdo que incluem objetos .NET Framework, objetos personalizados, valores de enumeração, controles de usuário, arquivos [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] e arquivos HTML. No entanto, você descobrirá que a maneira mais comum e conveniente de empacotar conteúdo é usando <xref:System.Windows.Controls.Page>. Além disso, <xref:System.Windows.Controls.Page> implementa recursos específicos de navegação para aprimorar sua aparência e simplificar o desenvolvimento.

Usando <xref:System.Windows.Controls.Page>, você pode implementar declarativamente uma página navegável de conteúdo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] usando marcação como a seguinte.

[!code-xaml[NavigationOverviewSnippets#Page1XAML](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/Page1.xaml#page1xaml)]

Um <xref:System.Windows.Controls.Page> implementado na marcação [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] tem `Page` como seu elemento raiz e requer a declaração de namespace [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] @ no__t-4. O elemento `Page` contém o conteúdo que você deseja navegar e exibir. Você adiciona conteúdo definindo o elemento de propriedade `Page.Content`, conforme mostrado na marcação a seguir.
...
Os elementos filho de um elemento `Page` são considerados como o conteúdo de um <xref:System.Windows.Controls.Page> e, consequentemente, você não precisa usar a declaração `Page.Content` explícita. A marcação a seguir é o equivalente declarativo da amostra anterior.

[!code-xaml[NavigationOverviewSnippets#Page3XAML](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/Page3.xaml#page3xaml)]

Nesse caso, `Page.Content` é automaticamente definido com os elementos filho do elemento `Page`. Para obter mais informações, consulte [Modelo de conteúdo do WPF](../controls/wpf-content-model.md).

Um <xref:System.Windows.Controls.Page> somente marcação é útil para exibir o conteúdo. No entanto, um <xref:System.Windows.Controls.Page> também pode exibir controles que permitem que os usuários interajam com a página, e ele pode responder à interação do usuário manipulando eventos e chamando a lógica do aplicativo. Um @no__t interativo-0 é implementado usando uma combinação de marcação e code-behind, conforme mostrado no exemplo a seguir.
...
Quando você tiver um <xref:System.Windows.Controls.Page>, poderá navegar até ele. Para especificar o primeiro <xref:System.Windows.Controls.Page> ao qual um aplicativo navega, você precisa configurar o @no__t inicial-1.
...
Uma definição de aplicativo geralmente é implementada usando marcação e code-behind, com o arquivo de marcação configurado como um item do MSBuild @ no__t-0. A seguir está uma definição de aplicativo para um [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)].
...
Um típico [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] é composto por várias páginas. A maneira mais simples de navegar de uma página para outra é usar um <xref:System.Windows.Documents.Hyperlink>. Você pode adicionar declarativamente um <xref:System.Windows.Documents.Hyperlink> a um <xref:System.Windows.Controls.Page> usando o elemento `Hyperlink`, que é mostrado na marcação a seguir.
...
- O pacote [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] do <xref:System.Windows.Controls.Page> para navegar, conforme especificado pelo atributo `NavigateUri`.
...
![Página com hiperlink](./media/navigation-overview/xbap-with-a-page-with-a-hyperlink.png "mostra um XBAP com uma página com um hiperlink.")
...
![Botões voltar e avançar](./media/navigation-overview/back-and-forward-navigation.png "navegam com os botões voltar e avançar.")

Além de dar suporte à navegação de um <xref:System.Windows.Controls.Page> para outro, o <xref:System.Windows.Documents.Hyperlink> também oferece suporte à navegação de fragmento.
...
- O [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] do <xref:System.Windows.Controls.Page> com o fragmento de conteúdo para o qual navegar.
...
Veja a seguir um exemplo de um `Hyperlink` configurado para navegar até um fragmento de conteúdo.
...
> [!IMPORTANT]
> Você pode navegar para fragmentos em páginas flexíveis [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] (somente marcação [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] arquivos com `Page` como o elemento raiz) somente se as páginas puderem ser navegadas via HTTP.
>
...
Embora <xref:System.Windows.Documents.Hyperlink> permita que um usuário inicie a navegação para um determinado <xref:System.Windows.Controls.Page>, o trabalho de localizar e baixar a página é executado pela classe <xref:System.Windows.Navigation.NavigationService>. Essencialmente, <xref:System.Windows.Navigation.NavigationService> fornece a capacidade de processar uma solicitação de navegação em nome do código do cliente, como o <xref:System.Windows.Documents.Hyperlink>. Além disso, <xref:System.Windows.Navigation.NavigationService> implementa o suporte de nível superior para rastrear e influenciar uma solicitação de navegação.

Quando um <xref:System.Windows.Documents.Hyperlink> é clicado, [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] chama <xref:System.Windows.Navigation.NavigationService.Navigate%2A?displayProperty=nameWithType> para localizar e baixar o <xref:System.Windows.Controls.Page> no pacote especificado [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)]. O <xref:System.Windows.Controls.Page> baixado é convertido em uma árvore de objetos cujo objeto raiz é uma instância do <xref:System.Windows.Controls.Page> baixado. Uma referência ao objeto raiz <xref:System.Windows.Controls.Page> é armazenada na propriedade <xref:System.Windows.Navigation.NavigationService.Content%2A?displayProperty=nameWithType>. O pacote [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para o conteúdo que navegou é armazenado na propriedade <xref:System.Windows.Navigation.NavigationService.Source%2A?displayProperty=nameWithType>, enquanto o <xref:System.Windows.Navigation.NavigationService.CurrentSource%2A?displayProperty=nameWithType> armazena o pacote [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para a última página à qual foi navegada.
...
Você não precisará saber sobre <xref:System.Windows.Navigation.NavigationService> se a navegação for implementada declarativamente na marcação usando <xref:System.Windows.Documents.Hyperlink>, porque <xref:System.Windows.Documents.Hyperlink> usa o <xref:System.Windows.Navigation.NavigationService> em seu nome. Isso significa que, desde que o pai direto ou indireto de um <xref:System.Windows.Documents.Hyperlink> seja um host de navegação (consulte [hosts de navegação](#Navigation_Hosts)), <xref:System.Windows.Documents.Hyperlink> poderá localizar e usar o serviço de navegação do host de navegação para processar uma solicitação de navegação.
...
Por motivos que são abordados na seção [hosts de navegação](#Navigation_Hosts), um aplicativo [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] pode ter mais de um <xref:System.Windows.Navigation.NavigationService>. Isso significa que seu código precisa de uma maneira de localizar um <xref:System.Windows.Navigation.NavigationService>, que geralmente é o <xref:System.Windows.Navigation.NavigationService> que navegou para o <xref:System.Windows.Controls.Page> atual. Você pode obter uma referência a um <xref:System.Windows.Navigation.NavigationService> chamando o método `static` @ no__t-2. Para obter o <xref:System.Windows.Navigation.NavigationService> que navegou para um determinado <xref:System.Windows.Controls.Page>, você passa uma referência para o <xref:System.Windows.Controls.Page> como o argumento do método <xref:System.Windows.Navigation.NavigationService.GetNavigationService%2A>. O código a seguir mostra como obter o <xref:System.Windows.Navigation.NavigationService> para o <xref:System.Windows.Controls.Page> atual.
...
> [!NOTE]
> Um <xref:System.Windows.Controls.Page> só pode obter uma referência a seu <xref:System.Windows.Navigation.NavigationService> quando <xref:System.Windows.Controls.Page> gera o evento <xref:System.Windows.FrameworkElement.Loaded>.
...
Como você viu, há várias maneiras de iniciar a navegação. Quando a navegação é iniciada e enquanto a navegação está em andamento, você pode acompanhar e influenciar a navegação usando os seguintes eventos que são implementados por <xref:System.Windows.Navigation.NavigationService>:
...
Gráfico de fluxo de(./media/navigation-overview/order-of-navigation-events.png "eventos de navegação da página") ![gráfico de fluxo de navegação de página]
...
Se você registrar um manipulador com um evento de navegação de um <xref:System.Windows.Controls.Page>, como o exemplo anterior, você também deverá cancelar o registro do manipulador de eventos. Se não houver, pode haver efeitos colaterais em relação a como a navegação [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] memoriza a navegação <xref:System.Windows.Controls.Page> usando o diário.

<a name="NavigationHistory"></a>

### <a name="remembering-navigation-with-the-journal"></a>Memorizar a navegação com o diário

[!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] usa duas pilhas para memorizar as páginas pelas quais você navegou: uma pilha voltar e uma pilha avançar. Ao navegar do @no__t atual-0 para um novo <xref:System.Windows.Controls.Page> ou encaminhar para um @no__t existente-2, o <xref:System.Windows.Controls.Page> atual é adicionado à *pilha voltar*. Quando você navega do atual <xref:System.Windows.Controls.Page> de volta para o <xref:System.Windows.Controls.Page> anterior, o @no__t atual é adicionado à *pilha de encaminhamento*. Nos referimos ao conjunto composto pela pilha voltar, a pilha avançar e a funcionalidade para gerenciá-las como o diário. Cada item na pilha voltar e a pilha de encaminhamento é uma instância da classe <xref:System.Windows.Navigation.JournalEntry> e é referenciado como uma *entrada de diário*.
...
Por padrão, o texto de cada <xref:System.Windows.Controls.Page> que aparece na lista **páginas recentes** do Internet Explorer é o [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para o <xref:System.Windows.Controls.Page>. Em muitos casos, isso não é especialmente significativo para o usuário. Felizmente, você pode alterar o texto usando uma das seguintes opções:

1. O valor do atributo `JournalEntry.Name` anexado.

2. O valor do atributo `Page.Title`.

3. O valor do atributo `Page.WindowTitle` e o [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para o <xref:System.Windows.Controls.Page> atual.

4. O [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para o <xref:System.Windows.Controls.Page> atual. (Padrão)
...
Por esse motivo, o comportamento padrão do diário é armazenar metadados de <xref:System.Windows.Controls.Page> em cada entrada de diário, em vez de uma referência a um objeto <xref:System.Windows.Controls.Page>. Quando uma entrada de diário é navegada, seus metadados <xref:System.Windows.Controls.Page> são usados para criar uma nova instância do <xref:System.Windows.Controls.Page> especificado. Como consequência, cada <xref:System.Windows.Controls.Page> que é navegada tem o tempo de vida ilustrado pela figura a seguir.
...
Embora o uso do comportamento de registro em diário padrão possa economizar no consumo de memória, o desempenho de renderização por página pode ser reduzido; a reinstanciação de um <xref:System.Windows.Controls.Page> pode ser demorada, especialmente se ele tiver muito conteúdo. Se você precisar manter uma instância <xref:System.Windows.Controls.Page> no diário, poderá desenhar duas técnicas para fazer isso. Primeiro, você pode navegar programaticamente para um objeto <xref:System.Windows.Controls.Page> chamando o método <xref:System.Windows.Navigation.NavigationService.Navigate%2A?displayProperty=nameWithType>.

Em segundo lugar, você pode especificar que [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] retenha uma instância de um <xref:System.Windows.Controls.Page> no diário definindo a propriedade <xref:System.Windows.Controls.Page.KeepAlive%2A> como `true` (o padrão é `false`). Conforme mostrado no exemplo a seguir, você pode definir <xref:System.Windows.Controls.Page.KeepAlive%2A> declarativamente na marcação.

[!code-xaml[NavigationOverviewSnippets#KeepAlivePageXAML](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/KeepAlivePage.xaml#keepalivepagexaml)]

O tempo de vida de um <xref:System.Windows.Controls.Page> mantido ativo é ligeiramente diferente de um que não é. Na primeira vez que um <xref:System.Windows.Controls.Page> mantido ativo é navegado para, ele é instanciado da mesma forma que um <xref:System.Windows.Controls.Page> que não é mantido ativo. No entanto, como uma instância do <xref:System.Windows.Controls.Page> é retida no diário, ela nunca é instanciada novamente pelo tempo que permanecer no diário. Consequentemente, se um <xref:System.Windows.Controls.Page> tiver uma lógica de inicialização que precise ser chamada toda vez que o <xref:System.Windows.Controls.Page> for navegado, você deverá movê-lo do construtor para um manipulador para o evento <xref:System.Windows.FrameworkElement.Loaded>. Conforme mostrado na figura a seguir, os eventos <xref:System.Windows.FrameworkElement.Loaded> e <xref:System.Windows.FrameworkElement.Unloaded> ainda são gerados cada vez que um <xref:System.Windows.Controls.Page> é navegado para e de, respectivamente.

![Quando os eventos carregados e]descarregados são(./media/navigation-overview/loaded-and-unloaded-events.png "carregados e os eventos descarregados são gerados quando uma página é navegada para e de.")
...
Fazer qualquer uma delas criará referências que forçam o <xref:System.Windows.Controls.Page> a ser retido na memória, mesmo após ela ter sido removida do diário.

Em geral, você deve preferir o comportamento padrão <xref:System.Windows.Controls.Page> de não manter um <xref:System.Windows.Controls.Page> Alive. No entanto, isso tem implicações de estado que são discutidas na próxima seção.
...
Felizmente, o diário fornece suporte para memorizar dados entre navegações <xref:System.Windows.Controls.Page>, incluindo dados de controle. Especificamente, a entrada de diário para cada <xref:System.Windows.Controls.Page> atua como um contêiner temporário para o estado <xref:System.Windows.Controls.Page> associado. As etapas a seguir descrevem como esse suporte é usado quando um <xref:System.Windows.Controls.Page> é navegado de:

1. Uma entrada para o <xref:System.Windows.Controls.Page> atual é adicionada ao diário.
...
[!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] usa automaticamente esse suporte quando os seguintes controles são usados em um <xref:System.Windows.Controls.Page>:
...
![A página com controles que lembram](./media/navigation-overview/data-remembered-across-page-navigations.png "os dados de estado inseridos é lembrada entre as navegações de página.")
...
- os aplicativos autônomos [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] e [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] podem criar e gerenciar cookies.
...
- Os cookies são expedidos quando [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] e as páginas [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] soltas fazem solicitações da Web.

- Os [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] de nível superior e o [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] hospedados em IFRAMEs podem acessar cookies.
...
Para incorporar a navegação em estilo de navegador em seus aplicativos autônomos, você pode usar a classe <xref:System.Windows.Navigation.NavigationWindow>. <xref:System.Windows.Navigation.NavigationWindow> deriva de <xref:System.Windows.Window> e a estende com o mesmo suporte para navegação que [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] fornece. Você pode usar <xref:System.Windows.Navigation.NavigationWindow> como a janela principal do seu aplicativo autônomo ou como uma janela secundária, como uma caixa de diálogo.

Para implementar um <xref:System.Windows.Navigation.NavigationWindow>, assim como a maioria das classes de nível superior no [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] (<xref:System.Windows.Window>, <xref:System.Windows.Controls.Page> e assim por diante), use uma combinação de marcação e code-behind. Isso é mostrado no exemplo a seguir.
...
![Uma](./media/navigation-overview/navigation-window-as-main-window.png "janela de navegação da janela principal como a janela principal")
...
Definir as propriedades <xref:System.Windows.Controls.Page.WindowWidth%2A> e <xref:System.Windows.Controls.Page.WindowHeight%2A> também afeta o <xref:System.Windows.Navigation.NavigationWindow>.

Normalmente, você implementa seu próprio <xref:System.Windows.Navigation.NavigationWindow> quando precisa personalizar seu comportamento ou sua aparência. Se nada disso é necessário, você pode usar um atalho. Se você especificar o pacote [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] de um <xref:System.Windows.Controls.Page> como o <xref:System.Windows.Application.StartupUri%2A> em um aplicativo autônomo, o <xref:System.Windows.Application> criará automaticamente um <xref:System.Windows.Navigation.NavigationWindow> para hospedar o <xref:System.Windows.Controls.Page>. A marcação a seguir mostra como habilitar isso.

[!code-xaml[IntroToNavNavigationWindowSnippets#AppLaunchPage](~/samples/snippets/csharp/VS_Snippets_Wpf/IntroToNavNavigationWindowSnippets/CSharp/AnotherApp.xaml#applaunchpage)]

Se desejar que uma janela de aplicativo secundária, como uma caixa de diálogo, seja um <xref:System.Windows.Navigation.NavigationWindow>, você poderá usar o código no exemplo a seguir para abri-lo.
...
![Uma](./media/navigation-overview/navigation-window-as-dialog-box.png "janela de navegação da caixa de diálogo como uma caixa de diálogo")
...
![Botões voltar e avançar em um NavigationWindow](./media/navigation-overview/back-and-forward-buttons-in-navigation-window.png "e botões Avançar em uma janela de navegação")
...
O exemplo a seguir mostra como adicionar um <xref:System.Windows.Controls.Frame> a um <xref:System.Windows.Controls.Page> de forma declarativa usando o elemento `Frame`.

[!code-xaml[NavigationOverviewSnippets#FrameHostPageXAML1](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/FrameHostPage.xaml#framehostpagexaml1)]
[!code-xaml[NavigationOverviewSnippets#FrameHostPageXAML2](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/FrameHostPage.xaml#framehostpagexaml2)]
[!code-xaml[NavigationOverviewSnippets#FrameHostPageXAML3](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/FrameHostPage.xaml#framehostpagexaml3)]

Essa marcação define o atributo `Source` do elemento `Frame` com um pacote [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] para o <xref:System.Windows.Controls.Page> para o qual o <xref:System.Windows.Controls.Frame> deve inicialmente navegar. A figura a seguir mostra um [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] com um <xref:System.Windows.Controls.Page> que tem um <xref:System.Windows.Controls.Frame> que navegou entre várias páginas.

![Um quadro que navegou entre várias páginas](./media/navigation-overview/frame-navigation-between-multiple-pages.png "exibe uma navegação de quadro entre várias páginas.")
...
Por padrão, <xref:System.Windows.Controls.Frame> usa apenas seu próprio diário na ausência de outro diário. Se um <xref:System.Windows.Controls.Frame> fizer parte do conteúdo hospedado dentro de um <xref:System.Windows.Navigation.NavigationWindow> ou um [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)], <xref:System.Windows.Controls.Frame> usará o diário que pertence ao <xref:System.Windows.Navigation.NavigationWindow> ou [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]. Às vezes, no entanto, um <xref:System.Windows.Controls.Frame> pode precisar ser responsável por seu próprio diário. Um motivo para fazer isso é permitir a navegação do diário dentro das páginas que são hospedadas por um <xref:System.Windows.Controls.Frame>. Isso é ilustrado pela figura a seguir.
...
![Um quadro que usa seu próprio diário](./media/navigation-overview/frame-uses-its-own-journal.png "mostra o efeito de navegar dentro de um quadro que usa seu próprio diário.")
...
> [!NOTE]
> Se um <xref:System.Windows.Controls.Frame> fizer parte do conteúdo hospedado em um <xref:System.Windows.Window>, <xref:System.Windows.Controls.Frame> usará seu próprio diário e, consequentemente, exibirá sua própria navegação [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)].
...
![Um diário em um quadro e em uma](./media/navigation-overview/navigation-window-and-frame.png "janela de navegação NavigationWindow e quadro")
...
Um arquivo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] solto é um arquivo com as seguintes características:

- Contém somente [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] (ou seja, sem código).

- Tem uma declaração de namespace apropriada.

- Tem a extensão de nome de arquivo .xaml.

Por exemplo, considere o conteúdo a seguir que é armazenado como um arquivo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] flexível, Person. XAML.
...
A ![exibição do conteúdo no arquivo Person. XAML](./media/navigation-overview/contents-of-person-xaml-file.png "mostra o conteúdo do arquivo Person. XAML.")

Você pode exibir um arquivo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] solto do seguinte:
...
Um arquivo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] solto pode ser adicionado aos favoritos do navegador ou ser o home page do navegador.

> [!NOTE]
> Para obter mais informações sobre como publicar e iniciar páginas [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] soltas, consulte [implantando um aplicativo WPF](deploying-a-wpf-application-wpf.md).

Uma limitação em relação à [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] flexível é que você só pode hospedar conteúdo que seja seguro para ser executado em confiança parcial. Por exemplo, `Window` não pode ser o elemento raiz de um arquivo [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] solto. Para obter mais informações, consulte [Segurança parcialmente confiável do WPF](../wpf-partial-trust-security.md).

<a name="Navigating_to_HTML_Files_Using_Frame"></a>

### <a name="navigating-to-html-files-by-using-frame"></a>Navegar para arquivos HTML usando o quadro

Como você pode esperar, você também pode navegar para HTML. Você simplesmente precisa fornecer um [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] que usa o esquema http. Por exemplo, o [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] a seguir mostra um <xref:System.Windows.Controls.Frame> que navega para uma página HTML.

[!code-xaml[NavigationOverviewSnippets#FrameHtmlNavMARKUP](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigationOverviewSnippets/CSharp/FrameHTMLNavPage.xaml#framehtmlnavmarkup)]

Navegar para HTML requer permissões especiais. Por exemplo, você não pode navegar de um [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] em execução na área de segurança de confiança parcial da zona da Internet. Para obter mais informações, consulte [Segurança parcialmente confiável do WPF](../wpf-partial-trust-security.md).
...
O controle <xref:System.Windows.Controls.WebBrowser> dá suporte à Hospedagem de documento HTML, navegação e interoperabilidade de código gerenciado/script. Para obter informações detalhadas sobre o controle <xref:System.Windows.Controls.WebBrowser>, consulte <xref:System.Windows.Controls.WebBrowser>.

Assim como <xref:System.Windows.Controls.Frame>, navegar para HTML usando <xref:System.Windows.Controls.WebBrowser> requer permissões especiais. Por exemplo, de um aplicativo de confiança parcial, você pode navegar apenas para HTML localizado no site de origem. Para obter mais informações, consulte [Segurança parcialmente confiável do WPF](../wpf-partial-trust-security.md).
...
Para navegar até ele, chame o método <xref:System.Windows.Navigation.NavigationWindow.Navigate%2A?displayProperty=nameWithType>, conforme demonstrado pelo código a seguir.
...
![Uma página que navega para uma classe](./media/navigation-overview/page-navigates-to-an-object.png "é um exemplo de uma página que navega para um objeto.")

Nessa figura, você pode ver que nada útil é exibido. Na verdade, o valor que é exibido é o valor de retorno do método `ToString` para o objeto **Person** ; Por padrão, esse é o único valor que [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] pode usar para representar seu objeto. Você pode substituir o método `ToString` para retornar informações mais significativas, embora ele ainda será um valor de cadeia de caracteres. Uma técnica que você pode usar que aproveita os recursos de apresentação do [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] é usar um modelo de dados. Você pode implementar um modelo de dados que [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] possa associar a um objeto de um tipo específico. O código a seguir mostra um modelo de dados para o objeto `Person`.

[!code-xaml[NavigateToObjectSnippets#DataTemplateMARKUP](~/samples/snippets/csharp/VS_Snippets_Wpf/NavigateToObjectSnippets/CSharp/App.xaml#datatemplatemarkup)]

Aqui, o modelo de dados é associado ao tipo `Person` usando a extensão de marcação `x:Type` no atributo `DataType`. Em seguida, o modelo de dados associa os elementos `TextBlock` (consulte <xref:System.Windows.Controls.TextBlock>) às propriedades da classe `Person`. A figura a seguir mostra a aparência atualizada do objeto `Person`.
