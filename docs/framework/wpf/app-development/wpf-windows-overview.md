...
Os usuários interagem com aplicativos autônomos do Windows Presentation Foundation (WPF) por meio do Windows. O objetivo principal de uma janela é hospedar conteúdo que visualiza dados e permite aos usuários interagir com os dados. Os aplicativos autônomos do WPF fornecem suas próprias janelas <xref:System.Windows.Window> usando a classe. Este tópico apresenta <xref:System.Windows.Window> antes de abranger os conceitos básicos da criação e gerenciamento de janelas em aplicativos autônomos.  
  
> [!NOTE]
> Aplicativos WPF hospedados em navegador, incluindo aplicativos de navegador XAML (XBAPs) [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] e páginas flexíveis, não fornecem suas próprias janelas. Em vez disso, eles são hospedados no Windows fornecido pelo Windows Internet Explorer. Consulte [visão geral de aplicativos de navegador XAML WPF](wpf-xaml-browser-applications-overview.md).  
...
### <a name="opening-a-window"></a>Abrir uma janela  
 ...
 Neste exemplo, o `MarkupAndCodeBehindWindow` é instanciado quando o aplicativo é iniciado, o que ocorre quando <xref:System.Windows.Application.Startup> o evento é gerado.  
...
- Um botão **fechar** , normalmente em uma caixa de diálogo sem janela restrita.  
 ...
 O <xref:System.Windows.Window.Closing> manipulador de eventos é passado <xref:System.ComponentModel.CancelEventArgs>a, que implementa `Boolean` <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> a propriedade que você definiu como `true` para impedir que uma janela seja fechada.  
...
## <a name="window-size"></a>Tamanho da janela  
 ...
 A altura da janela é <xref:System.Windows.FrameworkElement.MinHeight%2A>gerenciada <xref:System.Windows.FrameworkElement.Height%2A>pelo <xref:System.Windows.FrameworkElement.MaxHeight%2A>, e e é configurada conforme mostrado no exemplo a seguir.  
...
- <xref:System.Windows.SizeToContent.WidthAndHeight>. Ajustar à largura e à altura do conteúdo, que tem o mesmo efeito que <xref:System.Windows.FrameworkElement.MinHeight%2A> definir <xref:System.Windows.FrameworkElement.MaxHeight%2A> e com a altura do conteúdo, e definir <xref:System.Windows.FrameworkElement.MinWidth%2A> e <xref:System.Windows.FrameworkElement.MaxWidth%2A> com a largura do conteúdo.  
...
> [!NOTE]
> Os <xref:System.Windows.Window.Top%2A>valores das <xref:System.Windows.Window.Left%2A> <xref:System.Windows.FrameworkElement.Height%2A> Propriedades,, <xref:System.Windows.FrameworkElement.Width%2A>e de uma janela sempre representam os valores para o estado normal, mesmo quando a janela está atualmente maximizada ou minimizada.  
...
#### <a name="non-rectangular-window-style"></a>Estilo de janela não retangular  
 Há também situações em que os estilos de borda <xref:System.Windows.Window.WindowStyle%2A> que permitem que você tenha não sejam suficientes. Por exemplo, você pode querer criar um aplicativo com uma borda não retangular, como o Microsoft Windows Media Player usa.  
