...
> [!NOTE]
> Esta referência documenta três tipos de exceção que são exclusivos para .NET Native. Para obter a documentação de referência para a API de reflexão de núcleo .NET Framework, consulte os namespaces <xref:System.Reflection>, <xref:System.Reflection.Context> e <xref:System.Reflection.Emit>. Para ver a documentação de referência da API de interoperabilidade principal do .NET Framework, consulte <xref:System.Runtime.InteropServices>.  
