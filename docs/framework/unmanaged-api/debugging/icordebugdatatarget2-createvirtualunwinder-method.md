...
## <a name="return-value"></a>Valor retornado  
 `S_OK` se bem-sucedido. Qualquer outro `HRESULT` indica uma falha. Qualquer falha `HRESULT` recebida pelo MSCorDbi é considerada fatal e faz [ICorDebug](icordebug-interface.md) com que os métodos `CORDBG_E_DATA_TARGET_ERROR`ICorDebug sejam retornados.  
