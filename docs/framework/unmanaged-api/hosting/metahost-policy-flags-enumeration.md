...
|`METAHOST_POLICY_SHOW_ERROR_DIALOG`|Uma caixa de diálogo de erro será exibida se `GetRequestedRuntime` não consegue encontrar um tempo de execução que é compatível com os parâmetros de entrada. Começando com o .NET Framework 4.5, essa caixa de diálogo de erro pode levar a forma de uma caixa de diálogo de recurso do Windows que pergunta se o usuário gostaria de habilitar o recurso apropriado.|  
