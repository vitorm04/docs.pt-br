...
## <a name="parameters"></a>Parâmetros  
 ...
 no O nome do assembly no qual o tipo solicitado derivado de <xref:System.AppDomainManager> é implementado.  
  
 `pwzAppDomainManagerType`  
 no O nome do tipo implementado no parâmetro `pwzAppDomainManagerAssembly` que implementa os recursos de <xref:System.AppDomainManager>.  
