...
```  
 ...
 Determinados eventos do criador de perfil são imutáveis. Isso significa que, assim que esses eventos são definidos no retorno de chamada `ICorProfilerCallback::Initialize`, eles não podem ser desativados e novos eventos não podem ser ativados. As tentativas de alterar um evento imutável resultarão em `ICorProfilerInfo::SetEventMask` retornando um HRESULT com falha.  
