...
|<xref:System.Windows.Automation.Provider.IValueProvider.Value%2A>|Consulte as observações.|Essa propriedade retornará o conteúdo textual do controle de edição. Se o `IsPasswordProperty` é definido como `true`, esta propriedade deve lançar uma `InvalidOpertaionException` quando solicitado.|  
