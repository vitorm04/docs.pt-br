 ...
 **✓ CONSIDER** oferecer suporte a serialização de tempo de execução se seus tipos serão usados com a comunicação remota do .NET. Por exemplo, o namespace <xref:System.AddIn?displayProperty=nameWithType> usa a comunicação remota do .NET e, portanto, todos os tipos trocados entre `System.AddIn` suplementos precisam dar suporte à serialização de tempo de execução.  
