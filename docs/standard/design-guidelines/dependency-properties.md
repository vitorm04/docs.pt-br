...
## <a name="dependency-property-design"></a>Design de propriedade de dependência
 ...
 ✔️ fornecem uma propriedade CLR regular e um campo somente leitura estático público que armazena uma instância de <xref:System.Windows.DependencyProperty?displayProperty=nameWithType> para cada propriedade de dependência.
 ...
 ❌ não definir valores padrão de propriedades de dependência explicitamente no código; em vez disso, defina-os nos metadados.
...
## <a name="dependency-property-validation"></a>Validação de propriedade de dependência
 ...
 ❌ não coloque a lógica de validação da propriedade de dependência nos acessadores da propriedade. Em vez disso, passe um retorno de chamada de validação para `DependencyProperty.Register` método.
...
## <a name="dependency-property-value-coercion"></a>Coerção de valor de propriedade de dependência
 ...
 ❌ não implementar a lógica de coerção em acessadores de propriedade de dependência.
