...
As diretrizes de nomenclatura a seguir se aplicam à nomenclatura de tipo geral.
 ...
 ❌ não atribua nomes de classe a um prefixo (por exemplo, "C").
...
|`System.Delegate`|✔️ Adicionar o sufixo "EventHandler" aos nomes de delegados que são usados em eventos.<br /><br /> ✔️ Adicionar o sufixo "callback" a nomes de delegados diferentes daqueles usados como manipuladores de eventos.<br /><br /> ❌ não adicionar o sufixo "delegate" a um delegado.|
|`System.EventArgs`|✔️ Adicionar o sufixo "EventArgs".|
|`System.Enum`|❌ não derivam dessa classe; em vez disso, use a palavra-chave com suporte no seu idioma; por exemplo, no C#, use a palavra-chave `enum`.<br /><br /> ❌ não adicionar o sufixo "enum" ou "Flag".|
...
## <a name="naming-enumerations"></a>Enumerações de nomenclatura
 ...
 ❌ não usar um sufixo "enum" em nomes de tipo enum.

 ❌ não use os sufixos "Flag" ou "Flags" em nomes de tipo de enumeração.

 ❌ não use um prefixo nos nomes de valores de enumeração (por exemplo, "AD" para enums ADO, "RTF" para enums de Rich Text, etc.).
