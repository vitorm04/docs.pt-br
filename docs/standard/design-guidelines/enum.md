...
**✓ DO** se aplicam a <xref:System.FlagsAttribute?displayProperty=nameWithType> para enums de sinalizador. Não aplique esse atributo a enums simples.
...
**✓ DO** o nome do valor zero de sinalizador enums `None`. Para uma enumeração de sinalizador, o valor sempre deve significar "todos os sinalizadores são limpos".
