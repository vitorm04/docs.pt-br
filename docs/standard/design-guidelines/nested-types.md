 ...
 **X AVOID** exposto publicamente tipos aninhados. A única exceção a isso é se as variáveis do tipo aninhado precisam ser declaradas apenas em cenários raros, como subclasse ou outros cenários de personalização avançada.  
 ...
 **X DO NOT** usar tipos aninhados se eles precisam ser instanciado pelo código do cliente.  Se um tipo tiver um construtor público, ele provavelmente não deve ser aninhado.  
 ...
 **X DO NOT** definir um tipo aninhado como um membro de uma interface. Muitas linguagens não oferecem suporte a tal construção.  
