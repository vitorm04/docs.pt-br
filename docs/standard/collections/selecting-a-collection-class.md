...
- Você precisa acessar os elementos em uma ordem específica, como PEPS, UEPS ou aleatória?
  ...
  - A <xref:System.Collections.Queue> classe, bem como as <xref:System.Collections.Generic.Queue%601>classes genéricas <xref:System.Collections.Concurrent.ConcurrentQueue%601>,, <xref:System.Collections.Immutable.ImmutableQueue%601> e todas oferecem acesso PEPS. Para obter mais informações, consulte [Quando usar uma coleção thread-safe](../../../docs/standard/collections/thread-safe/when-to-use-a-thread-safe-collection.md).

  - A <xref:System.Collections.Stack> classe, bem como as classes <xref:System.Collections.Generic.Stack%601>genéricas <xref:System.Collections.Concurrent.ConcurrentStack%601>,, <xref:System.Collections.Immutable.ImmutableStack%601> e todas oferecem acesso de UEPS. Para obter mais informações, consulte [Quando usar uma coleção thread-safe](../../../docs/standard/collections/thread-safe/when-to-use-a-thread-safe-collection.md).
...
- Cada elemento conterá um valor, uma combinação de uma chave e um valor ou uma combinação de uma chave e diversos valores?
  ...
  - Uma chave e um valor: use qualquer uma das coleções baseadas na interface do <xref:System.Collections.IDictionary> ou na interface genérica do <xref:System.Collections.Generic.IDictionary%602>. Para uma opção imutável, considere as <xref:System.Collections.Immutable.IImmutableSet%601> interfaces genéricas <xref:System.Collections.Immutable.IImmutableDictionary%602> ou.
...
- Você precisa de coleções que aceitem apenas cadeias de caracteres?
  ...
  - Além disso, é possível usar qualquer uma das classes genéricas de coleção no namespace <xref:System.Collections.Generic> como coleções de cadeias de caracteres fortemente tipadas especificando a classe <xref:System.String> para seus argumentos de tipo genérico. Por exemplo, você pode declarar que uma variável é do tipo [List\<Cadeia de caracteres>](xref:System.Collections.Generic.List%601) ou [Dictionary<Cadeia de caracteres,Cadeia de caracteres>](xref:System.Collections.Generic.Dictionary%602).
