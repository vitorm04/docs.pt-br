...
Se seu aplicativo depende de um fuso horário que deve estar presente, primeiro você deve chamar o <xref:System.TimeZoneInfo.FindSystemTimeZoneById%2A> método para recuperar as informações de fuso horário do registro. Se a chamada de método falhar, o manipulador de exceção deve, em seguida, criar uma nova instância do fuso horário ou recriá-la desserializando serializados <xref:System.TimeZoneInfo> objeto. Confira [Como Restaurar fusos horários de um recurso inserido](../../../docs/standard/datetime/restore-time-zones-from-an-embedded-resource.md) para obter um exemplo.
