...
6. Chame o `static` método`Shared` (no Visual Basic .NET <xref:System.DateTime.SpecifyKind%2A> ) para <xref:System.DateTime.Kind%2A> definir a propriedade de valor de data e hora UTC <xref:System.DateTimeKind.Utc?displayProperty=nameWithType>como.
...
O núcleo do código de exemplo usa uma matriz de <xref:System.TimeSpan> objetos para indicar possíveis deslocamentos do tempo ambíguo do UTC. No entanto, esses deslocamentos provavelmente não serão significativos para o usuário. Para esclarecer o significado dos deslocamentos, o código também observa se um deslocamento representa o horário padrão do fuso horário local ou seu horário de verão. O código determina qual é a hora padrão e em qual horário é o horário de verão comparando o <xref:System.TimeZoneInfo.BaseUtcOffset%2A> deslocamento com o valor da propriedade. Essa propriedade indica a diferença entre o UTC e o horário padrão do fuso horário.
