---
title: Executar, gerenciar e monitorar ambientes de produção do Docker
description: Containerized Docker Application Lifecycle with Microsoft Platform and Tools (Ciclo de vida de aplicativo do Docker em contêineres com a plataforma e as ferramentas da Microsoft)
ms.prod: .net
author: CESARDELATORRE
ms.author: wiwagn
ms.date: 09/22/2017
ms.workload:
- dotnet
- dotnetcore
ms.openlocfilehash: 1bef0acc4a01a035a02c726800a643e0bcbdfde6
ms.sourcegitcommit: 9a4fe1a1c37b26532654b4bbe22d702237950009
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 04/16/2018
---
# <a name="run-manage-and-monitor-docker-production-environments"></a>Executar, gerenciar e monitorar ambientes de produção do Docker

Visão: aplicativos corporativos precisam ser executados com alta disponibilidade e escalabilidade; as operações de TI precisam ser capazes de gerenciar e monitorar os ambientes e os próprios aplicativos.

Este último pilar no ciclo de vida de aplicativos do Docker em contêineres se concentra na maneira pela qual você pode executar, gerenciar e monitorar seus aplicativos em ambientes de produção escalonáveis e de HA (alta disponibilidade).

A maneira como você executa seus aplicativos em contêineres na produção (arquitetura de infraestrutura e tecnologias de plataforma) também é muito relacionadas e completamente fundada na arquitetura e nas plataformas de desenvolvimento escolhidas que vimos no Capítulo 1 deste livro eletrônico. Este capítulo examina os produtos e tecnologias da Microsoft e outros fornecedores que você pode usar para executar aplicativos altamente dimensionável e distribuídos com HA com eficácia, bem como gerenciá-los e monitorá-los da perspectiva de TI.


>[!div class="step-by-step"]
[Previous] (../docker-devops-workflow/docker-application-outer-loop-devops-workflow.md) [Next] (run-microservices-based-applications-in-production.md)
