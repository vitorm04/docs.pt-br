...
.NET define um formato de arquivo binário, *montagem,* que é usado para descrever e conter totalmente programas .NET. Os assemblies são usados para os programas em si, bem como todas as bibliotecas dependentes. Um programa .NET pode ser executado como um ou mais assemblies, sem a necessidade de nenhum outro artefato, além da implementação do .NET apropriado. As dependências nativas, incluindo as APIs do sistema operacional, são uma preocupação separada e não estão contidas no formato de montagem .NET, embora às vezes sejam descritas com esse formato (por exemplo, WinRT).
...
## <a name="process-the-assemblies"></a>Processe as assembléias
