...
- Se o *assembly A* não for de nome forte, o nome do assembly Friend deverá consistir apenas no nome do assembly. Para obter mais informações, confira [Como: Criar assemblies Friend não assinados](create-unsigned-friend.md).

- Se *AssemblyB* tiver um nome forte, você deverá especificar a chave de nome forte para *AssemblyB* usando a configuração do projeto ou a opção de linha de comando `/keyfile` compilador. Para obter mais informações, confira [Como: Criar](create-signed-friend.md)de assemblies Friend assinados.
