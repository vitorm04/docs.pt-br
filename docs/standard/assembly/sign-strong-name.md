...
- *ModuleName* é o nome de um módulo de código .NET Framework (um arquivo *. netmodule* ) que inclui um ou mais tipos. Você pode criar um arquivo *. netmodule* compilando seu código com a opção `/target:module` no C# ou Visual Basic.
...
O exemplo a seguir usa o atributo <xref:System.Reflection.AssemblyKeyFileAttribute> com um arquivo de chave chamado *keyfile. SNK*, que está localizado no diretório em que o assembly é compilado.  
