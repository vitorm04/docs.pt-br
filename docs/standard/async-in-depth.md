...
`CalculateResult()` executa no thread no qual foi chamado.  Quando ele chama `Task.Run`, enfileira a operação vinculada à CPU, `DoExpensiveCalculation()`, no pool de threads e recebe um identificador `Task<int>`.  `DoExpensiveCalculation()` é finalmente executado simultaneamente no próximo thread disponível, provavelmente em outro núcleo da CPU.  É possível fazer o trabalho simultâneo enquanto `DoExpensiveCalculation()` está ocupado em outro thread, pois o thread que chamou `CalculateResult()` ainda está em execução.
