...
Às vezes, as regras de marshalling padrão para estruturas não são exatamente o que você precisa. Os tempos de execução do .NET fornecem alguns pontos de extensão para você personalizar o layout de sua estrutura e como os campos têm o marshaling realizado.
...
**✔️ USE** somente `LayoutKind.Explicit` no marshalling quando seu struct nativo também tiver um layout explícito, como uma união.

**❌ EVITE** usar `LayoutKind.Explicit` ao realizar marshal de estruturas em plataformas que não sejam Windows. O tempo de execução do .NET Core não dá suporte à passagem de estruturas explícitas por valor para funções nativas em sistemas Intel ou AMD de 64 bits que não sejam Windows. No entanto, o tempo de execução dá suporte à passagem de estruturas explícitas por referência em todas as plataformas.

## <a name="customizing-boolean-field-marshalling"></a>Personalização do marshalling de campo booliano
...
Valores booleanos padrão para realizar marshal como um valor [`BOOL`](/windows/desktop/winprog/windows-data-types#BOOL) Win32 nativo de 4 bytes conforme mostrado no exemplo a seguir:
...
## <a name="customizing-array-field-marshalling"></a>Personalização do marshalling de campo de matriz
...
Se você precisar realizar marshal da matriz in-loco, use o valor <xref:System.Runtime.InteropServices.UnmanagedType.ByValArray?displayProperty=nameWithType> para informar o marshaler para realizar marshal da matriz in-loco. Ao usar esse marshalling, você também deve fornecer um valor ao campo <xref:System.Runtime.InteropServices.MarshalAsAttribute.SizeConst?displayProperty=nameWithType> para o número de elementos na matriz, para que o tempo de execução possa alocar espaço corretamente para a estrutura.
...
## <a name="customizing-string-field-marshalling"></a>Personalização do marshalling de campo de cadeia de caracteres
...
Se sua API requer que você passe a cadeia de caracteres in-loco na estrutura, use o valor <xref:System.Runtime.InteropServices.UnmanagedType.ByValTStr?displayProperty=nameWithType>. A codificação de uma cadeia de caracteres com marshal realizado por `ByValTStr` é determinada por meio do atributo `CharSet`. Além disso, ela requer que o comprimento da cadeia de caracteres seja passado pelo campo <xref:System.Runtime.InteropServices.MarshalAsAttribute.SizeConst?displayProperty=nameWithType>.
...
## <a name="customizing-decimal-field-marshalling"></a>Personalização do marshalling de campo decimal
...
## <a name="marshalling-systemobjects"></a>`System.Object` de marshalling
