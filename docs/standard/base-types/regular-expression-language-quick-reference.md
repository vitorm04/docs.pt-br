...
|---------------------|-----------------|-------------|-------------|
|`[` *grupo_de_caracteres* `]`|Corresponde a qualquer caractere único em *grupo_de_caracteres*. Por padrão, a correspondência diferencia maiúsculas de minúsculas.|`[ae]`|`"a"` in `"gray"`<br /><br /> `"a"` e `"e"` em `"lane"`|
|`[^` *grupo_de_caracteres* `]`|Negação: corresponde a qualquer caractere único que não esteja em *character_group*. Por padrão, caracteres em *grupo_de_caracteres* diferenciam maiúsculas de minúsculas.|`[^aei]`|`"r"`, `"g"` e `"n"` em `"reign"`|
...
|------------------------|-----------------|-------------|-------------|
|`(` *subexpressão* `)`|Captura a subexpressão correspondente e atribui a ela um número ordinal com base um.|`(\w)\1`|`"ee"` in `"deep"`|
...
|`(?:` *subexpressão* `)`|Define um grupo de não captura.|`Write(?:Line)?`|`"WriteLine"` in `"Console.WriteLine()"`<br /><br /> `"Write"` in `"Console.Write(value)"`|
...
|-----------------------------|-----------------|-------------|-------------|
|`\` *número*|Referência inversa. Corresponde ao valor de uma subexpressão numerada.|`(\w)\1`|`"ee"` in `"seek"`|
|`\k<` *nome* `>`|Referência inversa nomeada. Corresponde ao valor de uma expressão nomeada.|`(?<char>\w)\k<char>`|`"ee"` in `"seek"`|
