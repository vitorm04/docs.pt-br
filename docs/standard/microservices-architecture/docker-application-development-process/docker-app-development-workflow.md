...
Quando você usar o Visual Studio, muitas dessas etapas serão feitas para você, melhorando significativamente sua produtividade. Isso é especialmente verdadeiro quando você usa o Visual Studio 2017 e tem aplicativos de vários contêineres como destino. Por exemplo, com apenas um clique do mouse, o Visual Studio adiciona os arquivos *Dockerfile* e *docker-compose.yml* aos projetos com a configuração correta para o aplicativo. Quando você executa o aplicativo no Visual Studio, ele cria a imagem do Docker e executa o aplicativo de vários contêineres diretamente no Docker. Ele até mesmo permite que você depure vários contêineres de uma só vez. Essas funcionalidades aceleram sua velocidade de desenvolvimento.

Na diretriz a seguir, explicaremos o que está acontecendo com o Docker "nos bastidores".
...
Você precisa de um Dockerfile para cada imagem personalizada que deseja criar. Também precisa de um Dockerfile para cada contêiner a ser implantado, seja automaticamente por meio do Visual Studio ou manualmente usando a CLI do Docker (comandos docker run e docker-compose). Se seu aplicativo contém um único serviço personalizado, é necessário um único Dockerfile. Se seu aplicativo contém vários serviços (como em uma arquitetura de microsserviços), é necessário um Dockerfile para cada serviço.
...
Com as Ferramentas do Visual Studio para Docker, essa tarefa exige apenas alguns cliques do mouse. Quando você cria um projeto no Visual Studio 2017, há uma opção chamada **Habilitar Suporte do Docker**, conforme mostrado na Figura 5-3.
...
Você também pode habilitar o suporte ao Docker em um projeto de aplicativo Web .NET Core existente clicando com o botão direito do mouse no projeto no **Gerenciador de Soluções** e selecionando **Adicionar** > **Suporte ao Docker**, conforme mostrado na Figura 5-4.
...
Essa ação adiciona um *Dockerfile* ao projeto com a configuração necessária e só está disponível em projetos de aplicativo Web .NET Core.

Para adicionar um arquivo *docker-compose.yml* a toda a solução, clique com o botão direito do mouse no projeto no **Gerenciador de Soluções** e selecione **Adicionar** > **Suporte ao Orquestrador de Contêineres**, conforme mostrado na Figura 5-5.
...
**Figura 5-5**. Adicionando o suporte ao orquestrador de contêineres a um projeto existente no Visual Studio 2017.

Nas seções a seguir, descrevemos as informações que vão em cada um desses arquivos. O Visual Studio realiza esse trabalho para você, mas é útil entender o que vai dentro de um Dockerfile.
...
Geralmente, você cria uma imagem personalizada para seu contêiner sobre uma imagem base que você pode obter de um repositório oficial, no registro do [Hub do Docker](https://hub.docker.com/). É exatamente isso o que acontece nos bastidores quando você habilita o suporte do Docker no Visual Studio. O Dockerfile usa uma imagem aspnetcore existente.

Anteriormente, explicamos quais imagens do Docker e quais repositórios você poderia usar, dependendo da estrutura e do sistema operacional que você escolheu. Por exemplo, se você quer usar o ASP.NET Core (Linux ou Windows), a imagem a ser usada é microsoft/aspnetcore:2.0. Assim, basta especificar qual imagem base do Docker será usada para o seu contêiner. Isso é feito com a adição de FROM microsoft/aspnetcore:2.0 ao seu Dockerfile. Isso é realizado automaticamente pelo Visual Studio, mas se você precisar atualizar a versão, atualize esse valor.
...
Mas (e essa é uma novidade desde meados de 2017) se você especificar o mesmo nome de imagem e até a mesma marcação, as novas imagens de várias arquiteturas (como a imagem aspnetcore, que dá suporte a várias arquiteturas) usarão a versão do Windows ou do Linux, dependendo do sistema operacional host do Docker que você está implantando, conforme mostrado no seguinte exemplo:
...
As imagens do Docker são criadas automaticamente para você no Visual Studio. As etapas a seguir são necessárias apenas para o fluxo de trabalho de editor/CLI e são explicadas com a finalidade de esclarecer o que acontece nos bastidores.
...
Para criar uma imagem personalizada no ambiente local usando a CLI do Docker e o Dockerfile, use o comando docker build, como na Figura 5-5.
...
O arquivo docker-compose.yml de exemplo define quatro serviços: o serviço webmvc (um aplicativo Web), dois microsserviços (catalog.api e ordering.api) e um contêiner de fonte de dados, sql.data, com base no SQL Server para Linux, em execução como um contêiner. Cada serviço é implantado como um contêiner, assim, é necessário uma imagem do Docker para cada serviço.
...
Quando você adiciona o suporte ao orquestrador de contêineres a um projeto de aplicativo Web, conforme mostrado na Figura 5-7, o Visual Studio adiciona uma seção de serviço (projeto) à solução que contém um arquivo docker-compose.yml. Essa é uma maneira fácil de iniciar a composição de uma solução de vários contêineres.
...
A adição de suporte ao orquestrador de contêineres adiciona o Dockerfile ao projeto (caso ainda não exista). Ele também adiciona informações de configuração a um arquivo docker-compose.yml global no nível da solução. Você verá um novo nó de projeto (o arquivo de projeto *docker-compose.dcproj*) no **Gerenciador de Soluções** que contém o arquivo docker-compose.yml, conforme mostrado na Figura 5-8.
...
Depois, você pode abrir o arquivo docker-compose.yml e atualizá-lo com outras funcionalidades.

Você pode implantar um aplicativo de vários contêineres com um único arquivo docker-compose.yml usando o comando `docker-compose up`.
...
Se seu aplicativo tem apenas um contêiner, você pode executá-lo implantando-o no host do Docker (VM ou servidor físico). No entanto, se o aplicativo contém vários serviços, você pode implantá-lo como um aplicativo composto usando um único comando de CLI (docker-compose up) ou com o Visual Studio, que usará esse mesmo comando nos bastidores. Vamos examinar as diferentes opções.

### <a name="option-a-run-a-single-container-app"></a>Opção A: executar um aplicativo de contêiner único

#### <a name="docker-cli"></a>CLI do Docker
...
Caso ainda não tenha adicionado o suporte ao orquestrador de contêineres, você também poderá executar um aplicativo de contêiner único no Visual Studio pressionando **F5**. O contêiner é executado localmente usando o docker run.

### <a name="option-b-run-a-multi-container-app"></a>Opção B: executar um aplicativo de vários contêineres
...
Depois que o comando docker-compose up é executado, o aplicativo e os contêineres relacionados são implantados no host do Docker.

#### <a name="visual-studio"></a>Visual Studio

A execução de um aplicativo de vários contêineres com o Visual Studio 2017 é simples. Você não somente pode executar o aplicativo de vários contêineres, mas também depurar todos os respectivos contêineres diretamente no Visual Studio, definindo pontos de interrupção regulares.

Conforme mencionado anteriormente, sempre que você adiciona o suporte ao orquestrador de contêineres a um projeto em uma solução, esse projeto é configurado no arquivo docker-compose.yml global (nível da solução), o que permite que você execute ou depure toda a solução de uma só vez. O Visual Studio iniciará um contêiner para cada projeto que tenha o suporte habilitado para a solução do Docker e executará todas as etapas internas para você (dotnet publish, docker build, etc.).
...
Essa URL no navegador usa a porta 80 para o exemplo de contêiner específico que está sendo abordado. No entanto, internamente as solicitações estão sendo redirecionadas para a porta 5000, pois é assim que ele foi implantado com o comando docker run, conforme explicado em uma etapa anterior.
...
- **Depurando aplicativos em um contêiner local do Docker**
...
- **Steve Lasker. Desenvolvimento do Docker no .NET com o Visual Studio 2017**
...
[Anterior](index.md)
[Próximo](../net-core-single-containers-linux-windows-server-hosts/index.md)