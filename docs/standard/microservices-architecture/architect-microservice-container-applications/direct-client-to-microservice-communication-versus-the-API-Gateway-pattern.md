...
description: Arquitetura de microsserviços .NET para aplicativos .NET em contêineres | Padrão de Gateway de API versus comunicação direta de cliente com microsserviço
...
![Diagrama mostrando arquitetura de comunicação direta de cliente com microsserviço](./media/image12.png)
...
- **Problemas de segurança**: sem um gateway, todos os microsserviços devem ser expostos ao "mundo externo", tornando a superfície de ataque maior do que se você ocultasse microsserviços internos não usados ​​diretamente pelos aplicativos clientes. Quanto menor a superfície de ataque, mais segura sua aplicação pode ser.
...
![Diagrama mostrando um Gateway de API implementado como um serviço personalizado](./media/image13.png)
...
Pode haver muitos outros interesses paralelos oferecidos pelos produtos de Gateways de API, dependendo de cada implementação. Por exemplo, o [Gerenciamento de API do Azure](https://azure.microsoft.com/services/api-management/) (conforme mostrado na Figura 4-14) não apenas soluciona as necessidades do seu Gateway de API, mas também fornece recursos como a coleta de informações de suas APIs. Se você estiver usando uma solução de gerenciamento de API, um Gateway de API será apenas um componente nessa solução completa de gerenciamento de API.

![Diagrama mostrando o Gateway de API com arquitetura de gerenciamento de API do Azure](./media/image14.png)
...
**Ocelot.** Para abordagens mais simples, recomenda-se um gateway de API leve como o Ocelot. O [Ocelot](https://github.com/ThreeMammals/Ocelot) é um gateway de API baseado em .NET Core de código aberto, feito especialmente para a arquitetura de microsserviços que precisa de pontos de entrada unificados em seu sistema. Ele é leve, rápido, escalonável e fornece roteamento e autenticação entre muitos outros recursos.

A principal razão pela qual o Ocelot foi usado no [aplicativo de referência eShopOnContainers](https://github.com/dotnet-architecture/eShopOnContainers) é porque Ocelot é um Gateway de API leve do .NET Core que você pode implantar no mesmo ambiente de implantação de aplicativos em que você está implantando seus microsserviços/contêineres, como um Docker Host, Kubernetes, Service Fabric etc. E, como ele é baseado no .NET Core, é uma plataforma cruzada que permite a implantação no Linux ou no Windows.
...
- **Charles Richardson. Padrão: Gateway de API / Back- end para front-end** [*https://microservices.io/patterns/apigateway.html*](https://microservices.io/patterns/apigateway.html)
...
- **Gateway de API neste mundo insano** (série de tutoriais da Gateway de API do ASP.NET Core)    [*http://www.pogsdotnet.com/2018/08/api-gateway-in-nutshell.html*](http://www.pogsdotnet.com/2018/08/api-gateway-in-nutshell.html)
