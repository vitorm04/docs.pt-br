...
Na maioria dos casos, a instância gerenciada do Azure SQL Database será a melhor opção deve ser considerado ao migrar seus dados para o Azure. Se você estiver migrando bancos de dados do SQL Server e precisar quase 100% garantia que não será necessário refazer a arquitetura de seu aplicativo ou fazer alterações em seus dados ou o código de acesso a dados, escolha o recurso de instância gerenciada do banco de dados do SQL Azure.
...
Devido a seus recursos completos de PaaS e melhor preço- e, finalmente, reduzir o custo-você deve mover o banco de dados do SQL Azure padrão sua escolha"por padrão" Se você tiver um aplicativo que usa basic, standard bancos de dados SQL e nenhum recurso de instância adicional. Não há suporte para recursos do SQL Server como a integração CLR do SQL, SQL Server Agent e consulta de bancos de dados no banco de dados padrão do SQL Azure. Esses recursos estão disponíveis apenas no modelo de instância gerenciada do Azure SQL Database.
...
[Próximo](lift-and-shift-existing-apps-devops/index.md)