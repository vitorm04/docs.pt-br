...
-   Comunicar-se com operações assíncronas pendentes usando o modelo familiar de eventos e representantes. Para obter mais informações sobre o uso de manipuladores de eventos e delegados, consulte [eventos](../../../docs/standard/events/index.md).  
  
 Uma classe que suporta o padrão assíncrono baseado em evento terá um ou mais métodos chamados *MethodName*`Async`. Esses métodos podem espelhar versões síncronas, que realizam a mesma operação no thread atual. A classe também pode ter um *MethodName* `Completed` eventos e podem ter um *MethodName* `AsyncCancel` (ou simplesmente `CancelAsync`) método.  
...
> [!CAUTION]
>  É possível que o download termine assim que a solicitação <xref:System.Windows.Forms.PictureBox.CancelAsync%2A> seja enviada, por isso, <xref:System.ComponentModel.AsyncCompletedEventArgs.Cancelled%2A> pode não refletir a solicitação de cancelamento. Isso é chamado de um *condição de corrida* e é um problema comum em programação multithread. Para obter mais informações sobre problemas de programação multithread, consulte [gerenciados Threading práticas recomendadas](../../../docs/standard/threading/managed-threading-best-practices.md).  
  
## <a name="characteristics-of-the-event-based-asynchronous-pattern"></a>Características do Padrão Assíncrono baseado em Evento  
 O Padrão Assíncrono baseado em Evento pode assumir diversos formatos, dependendo da complexidade das operações compatíveis com uma classe específica. As classes simples podem ter um único *MethodName* `Async` método e correspondente *MethodName* `Completed` eventos. Classes mais complexa pode ter vários *MethodName* `Async` métodos, cada um com um correspondente *MethodName* `Completed` eventos, bem como versões síncronas desses métodos. As classes podem opcionalmente oferecer suporte a cancelamento, relatórios de progresso e resultados incrementais para cada método assíncrono.  
...
### <a name="asynchronous-method-overloads"></a>Sobrecargas de Método Assíncronas  
 ...
 O parâmetro `userState` para as sobrecargas de invocação múltipla permite que você faça a distinção entre as operações assíncronas. Forneça um valor exclusivo (por exemplo, um código hash ou GUID) para cada chamada para `Method1Async(string param, object userState)`, e quando cada operação estiver concluída, o manipulador de eventos pode determinar qual instância da operação gerou o evento de conclusão.  
  
### <a name="tracking-pending-operations"></a>Acompanhando Operações Pendentes  
 Se você usar as sobrecargas de invocação múltipla, seu código precisará monitorar os objetos `userState` (IDs de tarefa) para as tarefas pendentes. Cada chamada para `Method1Async(string param, object userState)`, normalmente você irá gerar um novo exclusivo `userState` de objeto e adicioná-lo a uma coleção. Quando a tarefa correspondente a esse objeto `userState` eleva o evento de conclusão, sua implementação do método de conclusão irá examinar <xref:System.ComponentModel.AsyncCompletedEventArgs.UserState%2A?displayProperty=nameWithType> e removê-lo da coleção. Usado dessa forma, o parâmetro `userState` assume a função de uma ID de tarefa.  
...
### <a name="canceling-pending-operations"></a>Cancelando Operações Pendentes  
 É importante poder cancelar operações assíncronas a qualquer momento antes de sua conclusão. As classes que implementam o padrão assíncrono baseado em evento terá um `CancelAsync` método (se houver apenas um método assíncrono) ou um *MethodName* `AsyncCancel` método (se houver vários métodos assíncronos).  
  
 Métodos que permitem várias invocações assumem um parâmetro `userState`, que pode ser usado para acompanhar o tempo de vida de cada tarefa. `CancelAsync`leva um `userState` parâmetro, que permite que você cancele específico de tarefas pendentes.  
...
### <a name="receiving-progress-updates-and-incremental-results"></a>Recebendo Atualizações de Andamento e Resultados Incrementais  
 Uma classe que adere ao Padrão Assíncrono baseado em Evento pode opcionalmente oferecer um evento para acompanhar o andamento e os resultados incrementais. Isso geralmente será nomeado `ProgressChanged` ou *MethodName*`ProgressChanged`, e seu manipulador de eventos correspondente levará um <xref:System.ComponentModel.ProgressChangedEventArgs> parâmetro.  
  
 O manipulador de eventos para o `ProgressChanged` evento pode examinar a <xref:System.ComponentModel.ProgressChangedEventArgs.ProgressPercentage%2A?displayProperty=nameWithType> propriedade para determinar a porcentagem de uma tarefa assíncrona foi concluída. Essa propriedade estará entre 0 e 100, e pode ser usada para atualizar a propriedade <xref:System.Windows.Forms.ProgressBar.Value%2A> de um <xref:System.Windows.Forms.ProgressBar>. Se várias operações assíncronas estiverem pendentes, você poderá usar a propriedade <xref:System.ComponentModel.ProgressChangedEventArgs.UserState%2A?displayProperty=nameWithType> para distinguir qual operação está relatando o andamento.  
