...
description: Saiba como migrar um aplicativo web .NET ou serviço de entrada para o Azure App Service.
ms.topic: conceptual
ms.date: 08/11/2018
...
[O App Service](https://docs.microsoft.com/azure/app-service/overview) é um serviço de plataforma de computação totalmente gerenciado que é otimizado para hospedar sites escaláveis e aplicativos web. Este artigo fornece informações sobre como levantar e deslocar um aplicativo existente para o Azure App Service, modificações a considerar e recursos adicionais para [se mover para a nuvem](https://azure.microsoft.com/migration/web-applications/). A maioria dos sites ASP.NET (formulários da Web, MVC) e serviços (API da Web, WCF) podem migrar diretamente para o Serviço de Aplicativo do Azure sem alterações. Alguns podem precisar de pequenas alterações enquanto outros talvez precisem de alguma refatoração.
...
* Use ofertas de plataforma como serviço na nuvem para reduzir dependências. Por exemplo, em vez de se conectar a um servidor de email local, considere o uso de [SendGrid](https://docs.microsoft.com/azure/sendgrid-dotnet-how-to-send-email).
...
Não há suporte para isso. Considere copiar os conjuntos necessários para a pasta *\bin* do aplicativo. Arquivos *.msi personalizados instalados* no servidor (por exemplo, geradores PDF) não podem ser usados.

### <a name="iis-settings"></a>Configurações do IIS
Tudo configurado tradicionalmente via applicationHost.config em seu aplicativo, agora pode ser configurado com o portal do Azure. Isso se aplica à bitness do AppPool, habilitar/desativar WebSockets, versão gerenciada do pipeline, versão .NET Framework (2.0/4.0) e assim por diante. Para modificar as [configurações do aplicativo](https://docs.microsoft.com/azure/app-service/web-sites-configure), navegue até o [portal do Azure](https://portal.azure.com), abra a folha de seu aplicativo Web, em seguida, selecione a guia **Configurações do Aplicativo**.

#### <a name="iis5-compatibility-mode"></a>Modo de Compatibilidade do IIS5
Não há suporte para o Modo de Compatibilidade do IIS5. No Azure App Service, cada aplicativo web e todos os aplicativos sob ele são executados no mesmo processo do trabalhador com um conjunto específico de [pools](https://technet.microsoft.com/library/cc735247(v=WS.10).aspx)de aplicativos .
...
No Azure App Service, cada aplicativo web e todos os aplicativos sob ele são executados no mesmo pool de aplicativos. Considere estabelecer um único pool de aplicativos com configurações comuns ou criar um aplicativo web separado para cada aplicativo.

### <a name="com-and-com-components"></a>Componentes COM e COM+  
O Serviço de Aplicativo do Azure permite o registro de componentes COM na plataforma. Se o aplicativo fizer uso de quaisquer componentes COM, estes precisam ser reescritos em código gerenciado e implantados com o site ou aplicativo.

### <a name="physical-directories"></a>Diretórios físicos
O Serviço de Aplicativo do Azure não permite acesso à unidade física. Você pode precisar usar [arquivos do Azure](https://docs.microsoft.com/azure/storage/files/storage-files-introduction) para acessar arquivos via SMB. [O armazenamento de Blobs do Azure](https://docs.microsoft.com/azure/storage/blobs/storage-blobs-introduction) pode armazenar arquivos para acesso via HTTPS.
...
As vinculações HTTPS não são migradas, nem os certificados SSL estão associados aos seus sites. [Certificados SSL podem ser carregados manualmente](https://docs.microsoft.com/azure/app-service/app-service-web-tutorial-custom-ssl), no entanto, somente após a migração do site.
...
Sites gratuitos têm um limite de tamanho de 1 GB de conteúdo. Se seu site for maior que 1 GB, você deve atualizar para uma SKU paga. Consulte [os preços do Serviço de Aplicativo](https://azure.microsoft.com/pricing/details/app-service/windows/).
...
O AAD não funciona com aplicativos gratuitos. Para usar o AAD, você deve atualizar o SKU de aplicativo. Consulte [os preços do Serviço de Aplicativo](https://azure.microsoft.com/pricing/details/app-service/windows/).
...
* [Detalhes e restrições do aplicativo web do Azure](https://github.com/projectkudu/kudu/wiki/Azure-Web-App-sandbox)
