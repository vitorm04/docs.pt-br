...
---
# <a name="train-a-regression-model-to-predict-a-value-using-mlnet"></a>Treinar um modelo de regressão para prever um valor usando ML.NET

Em geral, há três etapas para o treinamento de modelo do ML.NET:

1. Obter os dados de treinamento na forma de uma `IDataView`
2. Criar o “pipeline de aprendizado”' como uma sequência de “operadores” elementares (estimadores).
3. Chamar `Fit` no pipeline para obter o modelo treinado.

Neste [arquivo de exemplo](https://github.com/dotnet/machinelearning/tree/master/test/data/generated_regression_dataset.csv), o rótulo previsto (`target`) é a última coluna (12ª), e todo o resto são recursos:
